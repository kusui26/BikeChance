 ...
 * **1 分あたりに投げてよい「地点」の数。**
 *
 * Open-Meteo の無料枠は**地点を 1 呼び出しとして数える**。実測の境界はきれいに 600 で、
 * `job_runs` にそのまま残っている（W5 プラン §12 の 173）。
 *
 * | 格子数 | 結果 |
 * |---|---|
 * | 600 | **ok 144 回中 144 回**（2026-09-11 04:17 〜 09-17 03:17） |
 * | 601 | **failed 24 回中 24 回**（09-17 04:17 〜 09-18 03:17） |
 * | 602 | failed 9 回中 9 回 |
 *
 * 601 になった瞬間に 7 分割目（残り 1〜2 地点）が 429 で落ちた。7 分割を約 5 秒で
 * 連射していたので、**1 分の枠を 1 地点ぶん超えた**のである。
 *
 * **500 にするのは、境界ちょうどに張り付けないため。** 100 地点の余裕があれば、
 * Open-Meteo 側の数え方が多少変わっても落ちない。
 */
export const WEATHER_LOCATIONS_PER_MINUTE = 500;

/**
 * 上の予算を数える窓。**実測より長めに取る。**
 *
 * 相手の窓が「直近 60 秒」なのか「毎分 0 秒でリセット」なのかは分からない。前者なら
 * 60 秒待てば足りるが、後者なら**待ち始めた時刻によっては 60 秒では足りない**。
 * 5 秒足しておけば、どちらでも枠をまたぐ。
 */
export const WEATHER_RATE_WINDOW_MS = 65_000;
