...
- `lightgbm` は **`apps/ml` の本番依存**に入れた（W4-15）。上限 500 MB に対して 334 MB
...
| モデル成果物 | `model_versions` の `status='active'` を毎サイクル確認し、版が変われば Storage から取得してモジュール変数にキャッシュ（ウォーム時は再取得しない）。**2026-09-10 に実装した**（W4 の PR E、migration 0038）。環境変数 `BASELINE_MODEL_VERSION` は使わない——正を 2 つ作らないため、**登録が無ければ 500 で止める**（黙って古い版を配らない）。`?model=` で候補を名指しできるが、**`active` でない版は試し打ち**になり `station_forecasts` にも `inference_log` にも書かない（W4-18）。実測：成果物は 3.2 MB で、5 分毎に取り直すと 1 日 900 MB の転送になる |
