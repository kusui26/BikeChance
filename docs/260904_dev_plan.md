...
| # | 中身 | 状態（2026-09-20） |
|---|---|---|
| **§12 の 168** | **28 日ぶんの当てはめが 25.1 GB**（実測の回帰。山 ＝ 2.37 GB ＋ 1.39 GB × 百万行） | **✅ 1 段目を 9/19 に実施**（23.7 → 14.2 GB）、**✅ 測る場所も 9/19**（`measure-fit.yml`）、**✅ 9/20 に Linux で実測**（4 水準）。**結果は「載るが足りない」**：8 日＝28 日ぶんで**常駐 14.67 GiB ＋ 退避 0.96 GiB**、機械は **15.61 GiB**（余り 1.4%）。**的（28 日・一様）は 15.96 GiB で 0.35 GiB 超過**、**PR J 後は 16.9〜17.6 GiB**。**→ 2 段目（日ごとに流す。約 11.4 GiB）をやる**（下の (d) 3） |
...
3. **✅ §12 の 168 の検算（9/20 完了）→ 2 段目をやる** ——`ubuntu-latest` で
   ...
   **的（28 日・一様）は 15.96 GiB ＝ 0.35 GiB 超過**、**PR J（66〜69 列）で
   ...
   → **2 段目（`load_days` を日ごとに流して「読んだ表」4.59 GiB を外す。約 11.4 GiB、
   PR J 後も約 12 GiB で余裕 23〜27%）をやる**。**同じ場所で測り直して効いたことを
   確かめる**（§12 の 168 の「Linux で測った」）
