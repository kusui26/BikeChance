...
- **リーク防止の原則**
  ...
  - **天気の予報ファイルは `hour_epoch + 1 時間` から使う**（2026-09-08 の点検で追加）。パスの `hour_epoch` は取得時刻を「時」に丸めた値で、実際に保存されるのは cron の分（`17 * * * *`）＝ **常に `hour_epoch + 18 分**（実測でばらつき 0）。「`hour_epoch <= t` の最新」で引くと、`t` が毎正時から 18 分の間にあるとき**まだ発行されていない予報**を使う。**5 分グリッド点の 30% が該当する。** 厳密に扱うなら `storage.objects.created_at` か `job_runs.detail` と突き合わせる。
...
| モデル成果物 | `model_versions` の `status='active'` を毎サイクル確認し、版が変われば Storage から取得してモジュール変数にキャッシュ（ウォーム時は再取得しない）。**W3 の段 8 では `model_versions` がまだ無いので、配る版は Vercel の環境変数 `BASELINE_MODEL_VERSION` で指定する**（実測：成果物は 3.2 MB で、5 分毎に取り直すと 1 日 900 MB の転送になる） |
...
| W4 | **9/10〜9/19**（W3 が 10 日早く終わったぶん前倒し。**PR F だけ 9/16 の測り直しを待つ**） | **アーカイブした予報 JSON を `weather_hourly` に落とす**。**`jma_msm` 一本に決めた**（水平 180 分に対し覆域 76 時間で余り、30〜40 度では `best_match` との差が 0.00%。W4 プラン §4 の W4-05）。アーカイブ開始前の 32.4 時間は**天気付き版の学習期間から外す**（サンプルは消さない） | LightGBM v0（配線確認。**`candidate` 止まり**）、`model_versions`、モデルカード雛形 | **`/v1/stations` に予測を追加（`base_observed_at` が 15 分より古ければ返さない）**、`/v1/trip-check`。**推論側の特徴量経路（`build_now`）と、学習との一致のゴールデン** | 到着時刻ピッカー、確率表示 | **M3 予測が地図に出る（ベースライン）。前倒しで 9/17 見込み** |
