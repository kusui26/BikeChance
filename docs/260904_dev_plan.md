...
**費用**：LightGBM を配ると 1 周期（両系合計）は 32 → **133 秒**になり、月 77 → **319 CPU 時**、$16 → **$65**。**$20 のクレジットを超える**ので、`shadow` に上げる前に下の「費用の抑え方」から選ぶ（W6、W4-11 と合わせて判断）。候補は配信していないので、いまこの費用は出ていない。
