...
| **9/24（木）** | **⭐ 当てはめ直して昇格**（下限 3。D-30）→ **PR J1 に着手** | **08:30 JST 以降**（`profile(09-23)`）。**学習窓の 5 日が連休**なので 9/25 朝に新しい版の平日の成績を見る。**J1 の作り直しは当てはめ直しの後**（v3 と v4 が混ざった窓は当てはめが拒む。契約 8）。**PR J は 9/23 に J1・J2 に分けた**（D-31） |
| 9/26（土） | W5 完了時点の点検（§14 の 9 問） | — |
...
8. **9/24** ——~~PR J をマージ（実装は 3・4 と並行して先に済ませておける）~~ → **当てはめ直して昇格（D-30）し、PR J1 に着手**。**J2 は W6、v1 の `shadow` の前に §12 の 176 と一緒に**（D-31）
