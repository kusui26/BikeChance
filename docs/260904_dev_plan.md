...
| GitHub Actions | $0 | $0 | **公開リポジトリなので無制限・無料**（2026-09-11 に確認。D-12 の「私有」と食い違っている）。実績は CI 1.0 分・Python 0.6 分・SQL 2.6 分／回、iOS のみ macOS ランナー |
...
| D-12 | リポジトリ公開範囲 | ~~私有（GitHub Actions 2,000 分/月で足りる）~~ → **実際は公開になっている** | 公開 | **要確認（2026-09-11）**。`gh api` で `"visibility": "public"`、認証なしの GET も 200。**決定と実態が食い違っている。** 費用の面では公開のほうが有利で、**GitHub Actions は公開リポジトリでは無制限・無料**（私有の 2,000 分/月という制約が消える）。履歴を全走査したが**秘密の混入は 0 件**、`.env` もコミットされていない。**ただし、W4 の PR J で GitHub Secrets を入れるなら設計上の注意が要る**：スケジュール実行のワークフローは `schedule` / `workflow_dispatch` だけにし、**`pull_request` では動かさない**（fork からの PR に秘密は渡らないが、経路を作らない）。**私有に戻すか、この決定を公開に書き換えるかを決める** |
