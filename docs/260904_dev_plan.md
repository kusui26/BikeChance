...
| # | 中身 | 状態（2026-09-19） |
|---|---|---|
| **§12 の 168** | **28 日ぶんの当てはめが 25.1 GB**（実測の回帰。山 ＝ 2.37 GB ＋ 1.39 GB × 百万行） | **✅ 1 段目を 9/19 に実施。23.7 → 14.2 GB**（同じものを 2 つ持つのをやめ、float32 にした）。`ubuntu-latest`（16 GB）に**載るが余裕は 1.8 GB**。**✅ 測る場所も 9/19 に作った**（`measure-fit.yml`。**28 日ぶんの規模は実データで測れる**——層化の日が 3.4 倍濃いので 09-08〜09-18 が的の 98.6%）。**残るは Linux での実測と、足りなければ 2 段目**（日ごとに流す、見込み約 9 GB） |
...
**どれも日付に縛られていない。** **3 つのうち 1 つ半が片付いた**（166 は完了、168 は
1 段目まで）。**残るのは 167 と、168 の Linux 検算 → 2 段目**である。
...
3. **§12 の 168 の検算 → 2 段目** ——**14.2 GB は手元 macOS（8 GB、スワップしながら）の
   2 点からの 5.6 倍の外挿**で、**Linux では一度も測っていない**。`ubuntu-latest` の余裕が
   **1.8 GB しかない**ので、**まず実測で検算する**。**測る場所は 9/19 に作った**
   （`.github/workflows/measure-fit.yml`、`workflow_dispatch` のみ・何も書かない）。
   **28 日ぶんの規模は実データで測れる**——層化の日が 3.4 倍濃いので、09-08〜09-18 が
   的の 98.6% になる。足りなければ 2 段目（`load_days` を日ごとに流して「読んだ表」
   4.81 GB を外す。見込み約 9 GB）——**同じ場所で測り直して効いたことを確かめる**
...
**9/19 までに 1・2 が済んだので、残る障害は「載るかどうかの検算」だけである。**
166 が片付いたことで**測る土俵が本番と揃った**——以後の当てはめは、報告書の数字を
そのまま門の判定に使える。
