...
| **§12 の 167** | `MIN_CELL_SAMPLES = 2` が一様 1% では低すぎ、**セルが埋まるほど B2 が悪くなる** | **未着手。** 166 を直したので**本番の経路は通らなくなった**が、**プロファイルが無い日は今でもここに落ちる**（`--no-profile` の退避路） |

**どれも日付に縛られていない。** **166 と 168 は完了した**（168 は 1 段目 → 検算 →
2 段目 → 検算まで。9/20 に**退避 0・余裕 28%** を実測）。**残るのは 167 だけ**である。
...
4. **§12 の 167（`MIN_CELL_SAMPLES`）** ——プロファイルが無い日の退避路。**本番の経路は
   166 で通らなくなった**ので急がないが、**壊れたままである**
