...
**W6 の変更ぶん**（書式 v2・J2・shadow・合成器・特徴量の経路 B）の費用と容量は、**`docs/260924_week6_model_v1_and_release_prep.md` §4.9 に先に書いた**（CLAUDE.md §6）。**要点は 3 つ**：①書式 v2 で成果物が 20.2 → 2.4 MB になり、冷えた起動の下りは月 17 → 約 1.4 GB、②**J2 で下りが月 +24 GB**（1 周期 1.51 MB）、`profiles/` の `profile.parquet` は 15 → **44 MB/日**（35 日の掃除は W7。§4.5b の `profiles/` の行は W7 に直す）、③shadow の 4 日は全行ぶんの森（100 本なら約 $2.6）
...
| D-34 | ベースラインの成果物の書式 | **版 2：gzip JSON のまま、B2 の鍵（昇順の差分・`<u4`）と率（×10^6 の整数・`<u4`）を base64 の生バッファで持つ。読み手は版 1 と 2 の両方** | Parquet（MIME の追加が要り、小さい欄を JSON のまま読めない）／版 1 のまま | **決定（2026-09-24）**。W6 プラン §4 の W6-01、PR B。**配信中の成果物から作って測った**：20.2 → 2.4 MB、開く山 1.23 → 0.52 GB、6.3 → 1.2 秒、**中身は完全一致**（W6 プラン §13.2）。W5 プラン §12 の 176 の答え |
