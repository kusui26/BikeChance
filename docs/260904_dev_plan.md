...
>
> **⚠️⚠️ メモリは一度も検討していない**（2026-09-22 に気づいた。W5 プラン §12 の 176）。**`shadow` は 1 回の呼び出しで 2 つのモデルを読む。** 本番の推論はいま **P95 1.64 GB / 枠 2 GB**（余り約 360 MB）で、**その 96% はベースラインの成果物（28.5 MB の gzip JSON）を Python の辞書に開く 1 行**が使っている。**`shadow` に入る前に成果物の書式を直す**（鍵と率を生のバッファにする。見込み 1.58 → 0.5 GB 前後）。**これは所要の話ではなく、枠に入るかの話である。**
...
| ~~9/23（水・祝）~~ | ~~PR K（校正の判断）~~ | **✅ 済：入れない**（D-29）・**下限を 3 日に**（D-30）。**9/25 のうちに当てはめ直して昇格すれば、9/26（土）は B1 に落ちる**（いまの成果物の `sat` は 2 日） |
| 9/24（木） | PR J をマージ・配信 | 実装は先行可。マージだけ連休明け |
| 9/26（土） | W5 完了時点の点検（§14 の 9 問） | — |
| 10/05（月） | W4-29（ビルド本数）・9 月の Vercel 使用量 | 請求サイクルの切れ目 |
| 10/07（火） | 09-07〜09-11 の天気が `weather_hourly` の 30 日保持を抜ける | **PR J の作り直しが 9/24 に済んでいれば自動的に解消** |
...
8. **9/24** ——PR J をマージ（実装は 3・4 と並行して先に済ませておける）
