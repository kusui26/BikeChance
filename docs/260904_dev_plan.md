...
| W6 | 10/12–10/16 | ~~`forecast-log/` を実装する~~ → **✅ 9/13（PR A）**。~~`maxDuration` を 240 秒~~ → **✅ 9/13** | **LightGBM v1**（学習 4 週）→ ゲート → shadow。**未着手。§13.1 の 3 つを先に片付ける** | ~~予測ログ・日次評価 `/ml/evaluate`~~ → **✅ 9/16（PR L）** | クレジット/免責画面、プライバシーマニフェスト、~~Developer Program 登録~~ → **✅ 登録済み（2026-09-18 確認）** | **M4 LightGBM v1 active**（**費用の判断が先**。§13.1） |
...
| 9/25（金） | 新しい版の平日の成績（日次評価） | **観測だけ。** B2 の重みが約 3 割下がった（0.109/0.126 → 0.077/0.084）ぶんの**平日の代償**を、前の版の平日と比べる（所見 179） |
| 9/26（土） | W5 完了時点の点検（§14 の 9 問） | — **`sat` は B1 に落ちて `confidence = 2` が正しい**（成果物の `sat` は 0） |
| **9/28（月）** | **9/27（日）の実測で、5 日ぶんの `sun_holiday` の B2 が「B2 を落とした確率」より当たったか**（EDA #7 付録の方法）→ **下限の形を決め、所見 180 の直しと 1 つの PR に** | **期限：`sat` がちょうど 3 日になる当てはめ（`profile(09-26)` 以降）の前。** 決めるまで当てはめ直さない（鮮度の警報は 10/01）。W5 プラン §12 の 179 の候補 1〜3 |
| 10/05（月） | W4-29（ビルド本数）・9 月の Vercel 使用量 | 請求サイクルの切れ目 |
| 10/07（火） | 09-07〜09-11 の天気が `weather_hourly` の 30 日保持を抜ける | **PR J1 の作り直しが 10/07 より前に済んでいれば自動的に解消**（J1 をこれより後に送らない） |
| 10/08（木） | 「必要なら作り直せる」の実証（D-22 の前提） | これが済むまで `features/` を捨てる判断はできない |
| 11/30（月） | 再構築スクリプトの実証＋9 月パーティションの期限 | 生 JSON から戻せることの証明（R17） |
...
| D-22 | 学習サンプル（`features/`）の保持 | **当面ためる**（~~13~~ → **5.4 GB/年**。§4.5b。2026-09-16 に一様抽出へ変えて 3 分の 1 になった。D-25）。**捨てる前提が揃っていない**——「必要なら作り直せる」の確認が PR L（期限 2026-10-08）で、**確かめていないものを前提に捨てる決定はできない**。急ぐ理由も無い（100 GB まで約 2.3 年、先に効くのは `gbfs-raw` の 24 GB/年）。**貯めておけばモデルを作った正確なバイト列が残る** | N か月で捨てて必要なら作り直す | **決定（2026-09-11）**。見直す合図は Storage 合計 **60 GB**、または W6 の週次再学習を組むとき |
