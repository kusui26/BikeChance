...
- **属性の無いポートを許容する**：`station_status` にしか現れず `station_information` に無いポートが実在する（ドコモで実測 11 件）。`station_attributes` に行が無いので、**名称・座標・容量が NULL でも 200 を返す**設計にする（`/v1/stations` の要素で属性を optional にし、iOS の `Codable` も optional で生成する）。座標が無いポートは地図に出せないため、`bbox` 検索の結果からは自然に外れる。
