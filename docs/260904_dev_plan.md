...
- **変更履歴**：**v1.80（2026-09-25）D-37 に注記を 1 つ足した（W6 の PR A の実装）。** 学習側の下限を 1 日下げるのは**プロファイルから作るとき**で、学習サンプルから作る退避路（`FromSamples`）は下げない——引くのが行 1 つで日数が減らないので、同じ下限で「配るセル ＝ 学習の行が引けるセル」になる。**v1.79（2026-09-25）D-37 を足した（B3 の気候値の下限の形と、その決め方。W6 の PR A）。** **形は先に決め、土日祝の配る側の下限だけを 9/27 の同じ行で 3・4・5 日を並べて選ぶ**。W6 プランの前の決め方（9/27 に 5 日ぶんだけを測り、効けば「学習側の下限を 1 低く」だけ）では配る下限が 3 のまま残り、**見せても 3 日の B2 は「B2 を落とした確率」より悪い**（EDA #8 §0.4 C）のに、9/29 から 3 日の土曜を配ることになっていた（W6 プランの所見 202）。基準（全体で効き、ポート単位のブートストラップの 95% 区間の下限も正、4 組のうち 3 組以上）は**9/27 のデータを見る前に書いた**。**9/27 を測れなくても当てはめ直し #1 は止めない**（土日祝の B2 を配らない側で進む）。**v1.78（2026-09-25）D-34 を直した（W6 の PR B の実装）。** **B2 の鍵を昇順の差分・`<u4` で持つ形をやめ、「使えるか」を 1 セル 1 ビットの列で持つ**——差分の列は使えるセルが増えるほど長くなり、**ほぼ埋まった成果物（600 万セル）では開く山が 0.76 GB になって PR B の完了条件（0.6 GB）を超える**。ビット列なら 0.39 GB（W6 プランの所見 200）。あわせて、**率の整数は `round(x, 6)` を通してから作る**（`np.rint(x × 10^6)` から直接作ると、半分の点の近くで版 1 と 1 違う。所見 201）。**v1.77（2026-09-24）W6 の実装プランを書いた（`docs/260924_week6_model_v1_and_release_prep.md`。§4.5・§8.4・§11.1・§13・§13.1・§15 の D-33〜D-36）。** **期間は 9/25〜10/16**（前提づくりを前倒し。**9/25〜10/16 の日程の正は W6 プラン §7**、10/17 以降は M4 の文書 §2）。**v1 を配る前提 6 つを PR に割り、平日の朝に 1 日 1 つ入れる**：書式 v2（9/28）→ 当てはめ直し #1（9/29）→ J2（9/29〜30）→ shadow の経路（10/1）→ v1 のワークフロー（10/2）→ 特徴量の経路 B（10/5）→ 当てはめ直し #2（10/6）→ 予行演習（10/7）→ 合成器（10/8）→ v1 の shadow（10/9。3 連休を含む 4 日）→ **M4（10/13）**。**下調べで確かめたこと**：①**書式 v2 は中身が完全一致のまま 20.2 → 2.4 MB・開く山 1.23 → 0.52 GB**（D-34）、②**本番の Storage は Range 要求に応える**（J2 の部分読みが成り立つ。D-35）、③**推論の CPU の上位は前処理**（JSON の型検査・長形式化・as-of・ポートごとのループ・5 分ごとの参照スナップショットの組み立て直し）、④**iOS はサーバーのセル応答を読めない**（0.09 度で `aggregation: cell`。W5-19 の比べ方が逆だった）。**見つかった問題 13 件（187〜199）**：J2 の「1.51 MB」は手元のファイルで数えた値だった／J2 は 00:00〜06:00 に前日の版が無い／キャッシュが 1 つしか持たない／shadow を下ろす口が無い／書式の照合が完全一致／**§8.4・§11.1 の自動ロールバックは CLAUDE.md §6 に反する**（D-36 で取り下げた）／前処理が重い／iOS のセル応答／クレジットの書式と天気・祝日・行政区域の出典／予測時刻の表示の漏れ／意思決定の指標とデータの版が未実装／門の相手の B3 が本番と違う作り方／**v1 は active になる日に 8 日前のデータ**（合成器の `train_days` を 2 つの和にする。D-33）。**v1.76（2026-09-24）M4 の費用判断を整理し（D-32。上限は利用者が「公開後の定常で $20 の内側」を選んだ）、9/25〜11/30 の日程を 1 か所にまとめた（`docs/260924_m4_cost_and_schedule.md`。§4.5・§7.2・§8.2・§13・§13.1・§14 の R4・§15 の D-22・D-32）。** **v1 を手元で予備に当てはめて、止まっていた問いに答えた**（学習 09-08〜09-15 の 8 日・一様 1%・v4、検証 09-17〜18。**読むだけ**）：**100 本（学習率 0.15）で 300 本の改善の 96%、50 本で 93%、門を越える組は同じ 31 / 40**（9/20 の v3 の見通しは 21 / 40。**上がったのは `prof_*` による**——抜くと 18 / 40、**配るときだけ抜くと 6 / 40**で、J2 と契約 29 の門が前提になる）。**費用は本数に比例し**（手元で 1 行 1 木 0.20〜0.23 µs、本数と学習率によらない）、**門をセル単位で当てると森を歩く行は 60% 前後**になる。**$20 の内側に置けるのは `hnd1`＋特徴量の経路 B で約 50 本、`iad1`＋B で約 120 本**で、**上限は「公開後の定常で $20 の内側」に決まった**（利用者が選んだ）。**見積りの前提の訂正が 3 つ**：①スキップ規則は全行の **0.81%** しか当たらず、しかも門で B3 に回る水平の中にある（効かない）、②デプロイは 309 回ではなく **270 回**（GitHub の deployments に Actions の `training` 環境 39 件が混ざる。直近 7 日は 1 日 6.0 回、うち 45% が文書だけ）、③§13.1 (c) の 10/08（D-22 の前提）は **9/12 に済んでいた**。**v1 を配る前提が 6 つ残る**（§12 の 176・J2・**shadow の経路**・**v1 を置くワークフロー**・合成器・特徴量の経路 B。太字の 2 つは未記載だった）。**Spend Management の「Pause Production Deployments」は収集器ごと止めるので使わない**（R4）。**門の粒度が CLAUDE.md（水平）と §7.1（水平, バケツ）で違う**ことも見つけた（変えずに 10/09 の本番の窓で比べて諮る）。**v1.75（2026-09-24）PR J1 をマージし（13:58 JST、#126）、`features/` の 09-07〜09-23 を v4 で作り直した（14:05〜14:16 JST）。J1 の完了条件 7 本がそろった（W5 プラン §6.10 の「作り直した」。§13.1）。** **先に本番を確かめた**——デプロイ後の推論は両系統とも ok・`feature_set` v4・`profile_date` null・予測ログ ok、配った確率の相異なる値と `confidence = 3` の割合はマージ前と同じ幅（**B3 は変わっていない**）、収集はデプロイの前後で 26 / 26 分抜けなし。**作り直しは 17 / 17 日が合格**：一様の日は前からある列が作り直す前の版と完全一致、層化だった日は 9/16 に手元で一様で作った版と完全一致、置いたファイルは予行演習の出力と **17 / 17 日バイト一致**、抽出に依らない `pairs_total`・`pairs_kept`・`excluded` は前の記録の在る 14 日すべてで一致。**全日が一様抽出になり**（混ざった窓が消えた）、`features/` は **484 → 280 MB**。退避した v3 は手元に残し、v4 で最初の当てはめが通ってから消す。**次に見るのは 9/25 06:00 JST の日次ジョブ**（09-24 を v4 で作り `profile(09-23)` を読む）。**v1.74（2026-09-24）PR J1 を実装した：ポートプロファイルの 7 列を `FEATURE_SET` v4 で足した（W5 プラン §6.10 の J1 の実装・§9 の契約 11。§6.3・§13.1、§15 の D-25・D-27・D-28）。** **7 列はどれも目標時刻のセル `(ポート, 目標の曜日種別, 目標の 15 分枠)` を前日の版から引き、B2 と同じ表・同じ規則**——枠の規則を `profile.target_slot` の 1 か所にし、B2 の `slot15` もそこを通るようにした（`prof_p_*` と B2 の率が**ビット単位で一致**することを検査で固定）。**セルが無ければ 6 列は NULL、`prof_n_days` は 0。** **推論はまだプロファイルを渡さない**（`inference_log.detail.profile_date` が null）ので**配る確率は変わらない**——`prof_*` をでたらめな値で埋めても B3 の確率はバイト単位で同じ（完了条件 4）。**決めたこと 3 つ**：①流量の 2 列は**時間ではなく枠**で引く（値は 60 分の窓の和なので 1 時間あたり。時間で切ると J2 の 1 周期の下りが 2 MB を超える）②`weight`・`stratum` は**残す**（D-25）③`bikes_same_time_7d` は **v4 に入れない**（学習は格子を 7 日さかのぼる作り直し、推論は 3 つ目の窓が要り、J1 の「配信を変えない」を崩す）。**作り直しの予行演習（本番には置かずに 17 日ぶん）は 17 / 17 日が合格**——一様の日は前からある列が置いてある v3 と完全一致、層化の日は 9/16 に手元で一様で作った版と完全一致（作り直しは「抽出を揃えて列を足す」以外のことをしていない）。最初は 3 日が不一致と出たが、**`lat` / `lon` の NaN どうしを `Table.equals` が等しいとしない検算の穴**だった（直した。座標の無いポートは NULL ではなく NaN を書いている——v3 からの書き方で実害は無い）。**最初の週末（09-12・09-13）は `prof_*` がほぼ入らない**（0% と 3.9%）——v1 の窓を決めるときに被覆も並べて見る（W6 への申し送り）。`sum_dailies` の入口（`build_profiles --from-dailies --compare`）は**本番の 3 日でバイト一致**（D-28 の②）し、**山を 11.7 → 5.7 GB** にした（1 日ずつ畳む）。日次の `build_features` は手元で 33 → 43 秒・ファイル +15%。検査 938 → 983 件、壊して 14 通りすべて捕まる。**作り直し（09-07〜前日を v4 で）はマージ当日**に、確認を取ってから行う。**本番はまだ何も変えていない**。**v1.73（2026-09-24）下限 3 日で当てはめ直して昇格した（`baseline-b3-v0-20260923`、09:48 JST）。評価で「3 日ぶんの B2 も祝日にはまだ害」と分かり、害の約 8 割が「混合が学習で見ていない B2 を配った」ことから来ていたと切り分けた。あわせて leave-one-out の穴を見つけた（W5 プラン §8.3・§12 の 179・180、EDA #8 `docs/260924_eda_08_floor_three_days.md`。§13.1、§15 の D-30）。** **成果物**：`sat` 0 セル（2 日で下限に届かない＝正しい。`inspect-artifact.py` が「0 が正しい」で合格）・`sun_holiday` 5 日・`weekday` 10 日、28.5 → **20.2 MB**、開く山 1.71 → **1.16 GB**（所見 176 の余りが広がる向き）。**昇格後の点検はすべて通った**（両系統が次の周期で載り替わり、`confidence = 3` 99.7% / 99.6%、失敗 0、`forecast_log` ok、鮮度の警報 0）。**所見 179**：検証日を祝日 09-23 に置き 3 日ぶんの B2 で当てはめた版を測ると、**B2 を落とすと Brier −10.40%**（40 組中 36 組で良くなる。2 日のときは −7.59%）——**D-30 の「3 で足りるか」は、祝日については「足りない」**。**害だった 2 例（2 日・3 日）は、どちらも「日数がちょうど下限」だった**——leave-one-out は学習の行を 1 日薄くするので、**下限ちょうどの曜日種別は、混合が一度も見ないまま配られる**。**学習側の下限だけ 2 にして見せると、B1 との差は +11.98% → +2.57%（約 8 割減）**。それでも 40 組中 30 組で B1 より悪い。**配った版で B2 の重みが約 3 割下がった**（0.109/0.126 → 0.077/0.084）のも同じ仕組みで、**祝日の行の B2 を B1 に落として当てはめ直すと 0.125/0.172 に戻る**——この 1 つの `w₂` は平日にも掛かる（平日の代償は 9/25 に見る）。**次の当てはめで `sat` がちょうど 3 日になる**（`profile(09-26)` 以降＝見ていない形）ので、**9/27 の実測（9/28 朝）で下限の形を決めるまで当てはめ直さない**（候補：配るセルを混合が見たセルに揃える／下限を上げる。鮮度の警報は 10/01）。**所見 180**：`own_day` は基準時刻の日の寄与を引くが、セルは到着時刻で引くので、**日をまたぐ行（約 4%）では自分の答えがセルに残る**。直すと配った版の重みは 0.077/0.084 → 0.074/0.080 と小さく、**入れ替えずに、下限の形と同じ PR で直す**。**本番は成果物の入れ替えのみ**（コードは変えていない）。**v1.72（2026-09-23）PR J を J1（学習側・W5）と J2（配信側・W6）に分けた（D-31。W5 プラン §6.10・§9 の契約 29。§13.1）。** **推論がプロファイルを読む部分は、v1 を配るまで何も買わない**——配っている B3 は `prof_*` を 1 列も読まないので、読んでも確率は変わらない。それでも読み始めれば**下り月 24 GB（込み 250 GB の約 1 割）とメモリの山 ＋53 MB（余りは約 360 MB）**を効く日より 3 週間早く払う（所見 175・176）。**学習側には期限がある**（天気の 30 日保持が 10/07 に 09-07〜09-11 を抜け、v1 の学習窓が 10/09 に揃う）。**J1**：`build_day` に `prof_*` 7 列（v4・全 68 列）、`build_now` も 68 列を出すが**プロファイルを渡さなければ `prof_*` は NULL・`prof_n_days` は 0**、ゴールデン（プロファイルを渡したとき 68 列一致）、**B3 の確率が `prof_*` に左右されないことを検査で固定**、`sum_dailies` の入口と実証（D-28）、**09-07 以降の作り直しを J1 のマージと同じ日に**（v3 と v4 が混ざった窓は当てはめが拒む。契約 8）——**9/24 の当てはめ直し（下限 3）を v3 のまま先に終えてから**。**J2**：推論が前日のプロファイルを読む、`build_profiles` の並び替え（契約 28）と検査 3 本、`profile_load_ms`・`model_load_ms`・`rss_mb`——**v1 の `shadow` の前に §12 の 176（成果物の書式）と一緒に。176 を先に入れて余りを空けてから J2 を載せる**。**並び替えを J2 に置いたのは、部分読みが J2 からで、J1 で並べ替えると恩恵の無いまま置き場が 3 倍（15 → 44 MB/日）になるため**。**契約 29**：J1 と J2 のあいだ配信の `feature_set` は v4 だが `prof_*` は NULL——**`prof_*` を使うモデルは J2 まで `shadow` にも `active` にもしない**（J2 で機械で守る）。あわせて **#122・#123 を 9/23 にマージしたので、当てはめ直しを 9/24（`profile(09-23)` の後）に前倒した**（9/25 は予備。**学習窓の 5 日が連休**なので 9/25 朝に新しい版の平日の成績を見る）。**本番は何も変えていない**（文書だけ）。**v1.71（2026-09-23）PR K を判断した：校正器は入れない。気候値の下限を 2 → 3 日に上げる（W5 プラン §6.11・§12 の 177・178。§7.1・§7.4・§13.1、§15 の D-29・D-30）。** **本番で配った確率の全数**（09-14〜09-22 の 9 日・約 5.4 億行 × 2 ターゲット）を、日次評価と同じ関数（ラベル・除外・バケツ）に通し、**確率の値ごとの件数と陽性数に畳んで**測った（`x1000` は 0〜1000 の整数なので ECE も Brier も等頻度の区切りも厳密に再現できる。**`model_daily_metrics` と 20 ファイル × 280 組で一致**を確かめてから使った）。**① ECE は 0.02 を超えていた**（配信中の版・祝日で最大 0.053、40 組中 23 組）が、**歪みは全部、難所のバケツ（0 台・1 台）にある**——3 台以上のバケツは 0.0002〜0.0064 で、**B1 単独ならどの水平でも 0.02 を超えない**。バケツ 1 の中でモデルは 0.40〜0.96 に散らしているのに実際は 0.77〜0.89 にしか散らず、**「40%」と言った行の実際は 81%**だった。**原因は B3 の B2 の重み `w₂` が全バケツ・全セルで 1 つ**であること（所見 178）——難所で効きすぎ、**混合は学習で「薄い B2」を一度も見ていない**（土日の行は leave-one-out で B2 が落ちていた）。バケツ 1 の中の校正の傾き（1 が理想）は、**16 組すべてで、B2 が 2 日の祝日のほうが 5 日の平日より 1.7〜3.5 倍小さい**。**② 校正器は同じ種別の日どうしでしか効かない**：平日→平日は 0.02 以上 6 → 0、**平日→土日は 3 → 11・Brier は 40 組中 21 組で悪化**。バケツ別の isotonic・ロジスティックも、**どれもどこかで 20 組以上を悪化させた**。配信は全曜日種別に同じ成果物を出すので、**§7.4 の手順 3「特徴量に戻す」に当たる**（D-29）。**v1 の採用基準に「バケツ 0・1 の ECE と傾き」「曜日種別別の ECE」を足した**（§7.1）。**③ 下限の反実仮想**：B2 が引けないセルの B3 は `(system, 水平, バケツ)` だけで決まるので、配った確率をその値に置き換えて同じ実測で測り直せる。**祝日（B2 2 日）は落とすと Brier −7.59%**（09-21 は −12.1%、09-22 は −6.0%、両系統とも同じ向き）、**平日（B2 5 日）は落とすと +1.22%**——**境目は 2 日と 5 日のあいだ**で、**3 に上げる**（D-30。同じ `2` が 2 か所に在るのでまとめてから）。**④ 9/21 の昇格記録の「祝日に `confidence = 3` が 97.2%」は、B2 が使われていることを数えていて、効いていることは数えていなかった**（所見 177）。**⑤ `confidence` は分けない**——鮮度切れ（1）は 2 日とも 0 件で、直すべきは「3 と名乗る下限」だった。**本番は何も変えていない**（判断と文書だけ。下限の変更は別の PR）。**v1.70（2026-09-22）PR J の読み方・`profiles/` の保持・所見 176 の期限を決めた（W5 プラン §6.10・§9 の契約 28・§12 の 176。§7・§13.1、§15 の D-28）。** **① 読み方は「`profile.parquet` の並びを変える」**——配信用に 2 つ目のファイルは作らない。**税の内訳を測り直した**：15.3 → 35.0 MB（**並べ替えそのもので 2.3 倍**）→ 44.1 MB（**行群を 1 セルに揃えて ＋18%**）で、**行群は行数ではなく鍵の変わり目で切らないと効きが 3 分の 2 に落ちる**（1 周期 1.51 → 2.35 MB、月 24 → 38 GB）。**ページ索引は効かない**（pyarrow が読みで使わない）。**1 つのファイルのままにする決め手は W5-01 の原則**——プロファイル表の存在理由は「B2 と `prof_*` は同じ量だから表を 1 つにして両方が引く」で、**配信用に 2 つ目を作るのはその原則を自分で破ること**である（所見 132・142 の「同じ規約が 2 か所に無名で在る」）。**並びに依存する読み手は 1 つも無い**ことをコードで確認した（`roll` → `_fold` は `group_by` ＋ 最後の 1 行が `sort_by`、`profile_climatology.cells` は名前でポート番号を引く、`require_schema` は列名しか見ないので**古い並びの過去ファイルもそのまま読める**）。**並びが動作に効くので契約 28 にして検査を 3 本付ける**（行群の揃え／1 周期の読みが 2 MB 未満／**並べ替えた表から作った成果物がバイト一致**）——崩れると**例外を出さずに下りが 10 倍**になる。**② `profiles/` の保持は D-28**：**`daily.parquet` は無期限（源）、`profile.parquet` は 35 日（派生）**。`profile(D)` は `daily(D-27 … D)` の和で、**`profile.sum_dailies` が既にあり `roll` との一致を検査が保証している**——足りないのは Storage から 28 個読んで渡す入口だけである。**ただし D-22 と同じ作法で、作り直せることを確かめるまで掃除は入れない**。44 MB/日 × 35 日 ＝ **1.5 GB 定常**（無期限なら年 16 GB）。**バケットは増やさない**（接頭辞で掃除できる）。**③ 所見 176（成果物の書式）の期限は PR K ではなく W6 の `shadow`**。**`shadow` は 1 回の呼び出しで 2 つのモデルを読む**（§7）のに、**プランは所要しか検討しておらずメモリを一度も見ていなかった**——本番はいま P95 1.64 GB / 枠 2 GB で、**その 96% がベースラインの成果物を開く 1 行**である。**PR K は強制しない**（校正器は水平グループ 3 × ターゲット 2 の単調写像で節点は数千。1,200 万 × 2 の配列とは桁が違う）が、**PR K が「入れる」なら書式を 2 度触らないために 176 を先にやる**。直し方は**新しい依存なし**——`b2_keys` を `int32`、`b2_rate` を `float32` の生バッファにして `np.frombuffer` で読み、`b1`・`b3` は JSON のまま残す（成果物が目で読める性質を保つ）。`FORMAT_VERSION` 1 → 2 で古い版も読めるまま。見込み **1.58 GB → 0.5 GB 前後・8.2 秒 → 1 秒前後**。**本番は何も変えていない**（文書だけ）。**v1.69（2026-09-22）PR J の前に測ったら、配信の関数が 2 GB の枠の 82% に達していた（W5 プラン §6.10・§12 の 175・176。§15 の D-27、§13.1）。** PR J（`prof_*`・`FEATURE_SET` v4）を実装する前に、**配信がプロファイルを読む費用**を測った。**時間は問題にならない**（読んで引くのに 0.28 秒）。問題は 2 つだった。**① 下り**：`profile.parquet` は 15.3 MB で、いまの並び（`system_id, station_id, dow_type, slot15`）では**6 つの行群すべてが全 96 枠・全 3 曜日種別を含む**ため、絞っても**ファイル全部を落とす**（読んだバイトを数えて確認）。5 分ごと 2 系統なら **月 247 GB** で、**Supabase Pro の込み 250 GB をこれ 1 つで使い切る**。**1 周期で要るのは表の 2.8%** しかない——`t + h`（10 水平）が落ちる 15 分枠は**どの時刻でも 8 個**で、曜日種別は普通 1 つである（種別が変わる日の 21:00〜23:50 だけ 2 つ。35 / 288 周期）。`dow_type, slot15, ポート` に並べ替えて **1 行群 ＝ 1 セル**にすると **1 周期 1.51 MB・月 24.3 GB** になる（置き場は 15.3 → 44.1 MB/日に増える。**辞書ではなく並びのせい**——いまの並びは「同じポートの 96 枠」が続いて値が似ており圧縮が効くが、枠で切ると「同じ枠の 2 万ポート」が並ぶ。**ポートを番号に畳んで文字列を捨てても直らない**）。**§5.4 が成果物について 1 度解いた問題を、PR J が裏返していた**（§12 の 175）。**② メモリ**：本番の P95 が **1.64 GB / 2 GB**（Average 416 MB・P75 271 MB。**4 分の 3 の回は 280 MB 以下**で、山は冷えた起動に立つ）。**余りは約 360 MB。** 手元で**成果物を開くだけ**のプロセスは `peak memory footprint` **1.58 GB** で本番とほぼ一致し、**`ru_maxrss` は 0.59 GiB と 2.6 倍低く出る**——**§12 の 168 で保留にした「どちらの計器を信じるか」が、本番の数字で決着した**。山の内訳は gz を解いて 0.41 GB、**`json.loads` が ＋0.89 GB**、numpy へ移して ＋0.23 GB で、**枠の 96% を「28.5 MB の gzip JSON を Python の辞書に開く」ことが使っている**（鍵 1,200 万個・率 1,200 万個をいったん Python の `int`/`float` にするため。numpy に移せば 105 MB で済む）。**成果物を生のバッファで配れば 0.2 GB 前後に落ちる見込み**で、**PR K（校正器の同梱）と W6（LightGBM の配信）の前には要る**（§12 の 176）。**この 2 つから、プロファイルの読み方は「並べ替えて要る行群だけ落とす」しか残らない**——全体を開く案は一時に 373 MB、抱える案は常駐 373 MB を要し、**どちらも 360 MB の余りを超える**。あわせて **`prof_n_days` を特徴量に入れると決めた**（D-27）。`prof_*` は **7 列**、v4 は全 **68 列**で、§6.10 の完了条件が「65 列（61 ＋ 4 群）」と**群の数を列の数と取り違えていた**のを直した——決め手は §12 の 174 の実測で、**土日祝のセルは 2 日しか無く B2 は 13 個の値しか取らない**（平日は 230 個）ため、**`n_days` が無ければ木は薄いセルと濃いセルを区別できない**。**本番は何も変えていない**（測定と文書だけ）。**v1.68（2026-09-21）⭐ 当てはめ直して昇格した。土日祝の気候値が初めて立った（W5 プラン §8.3・§12 の 174。§13.1）。** 5 連休の 3 日目、**`sat` と `sun_holiday` の 2 日目が profiles に揃った最初の朝**に 1 度だけ当てはめ直した（`baseline-b3-v0-20260920`、学習 09-14〜09-20 の 7 日・722 万行）。**`sat` と `sun_holiday` のセルが 0 → 1,981,992 / 1,982,037**、`weekday` も 1,980,225 → 2,036,100、ポートは 20,663 → **21,258**。**09:38 JST に入れ替え、祝日に `confidence = 3` が 97.2%**（前日まで全ポートが 2）、相異なる確率は **7〜12 → 22〜59**、成果物に無いポートは **0**、推論の p95 は **17.3 秒（ドコモ）/ 29.0 秒（HELLO）**で失敗 0、`forecast_log` は ok。**成果物が 8.6 → 28.5 MB になるので、切り替える前に本番で試し打ちした**（`?model=`。ドコモ 33.7 秒 / HELLO 11.9 秒、どちらも 200。手元で開いて数えるまで 7.3 秒・ピーク 1.89 GB）。**相異なる確率が 9/14（平日・旧版）の 30〜108 より少ないのは、版ではなく日の性質だった**（§12 の 174）——プロファイルは 1 日 3 格子点で、`sun_holiday` は `MIN_CELL_DAYS` ちょうどの 2 日しか無い。**1 セル最大 6 点なので B2 が取りうる値は `k/n`（n ≤ 6）の 13 個ちょうど**（実測と一致）で、B1 の 6 バケツとの混合の**上限は 6 × 13 = 78**。`weekday` は 230 個で上限 1,380 であり、**同じ成果物を平日発で引くと丸め前 258〜665**（旧版 120〜276）と 7 倍の粒度が出る。**合格ラインの「丸め前 ≥ 60」は平日で決めた数**だったので、§1.2 に「平日発で測る」と明記し、§8.3 の「いつ測るか」の表に**その日の上限** `6 × (3·n_days + 1)` の列を足した（**9/26 の `sat` も上限 78。60 未満でも不合格と読まない**）。**本番の DB・Storage・Cron は触っていない**——昇格は `promote_model_version` の 1 行のみ。**v1.67（2026-09-20）気候値の下限を「信じてよいか」で決め直した（§12 の 167。§7.1・§13.1）。W6 の障害は 3 つとも片付いた。** **同じ「2」が違うものを数えていた**——プロファイルは 1 セルに **1 日 3 点**の格子点が入るが、抽出済みの学習サンプルは **1 セル 1 日 0.3 行**しか来ない。2 行しかないセルの率は **0・0.5・1 の 3 通り**しか取れず、気候値ではなく**その 2 行そのもの**である。**定数を 2 つに割った**：`MIN_CELL_POINTS`（格子点。2 のまま）と `MIN_CELL_ROWS`（行。**30**）。**30 は標準誤差から決めた**——n 行の率の標準誤差は高々 `0.5/√n` で、n = 25 で 0.1、**n = 30 で 0.091**。**掃引で確かめた**（一様 1% に作り直した 09-07〜09-15）：学習 6 日で下限 2 なら B3 は **0.04156**、**下限 5 でもっと悪い 0.04215**（B1 は 0.04117）、**セルが 0 になって初めて 0.04112 と B1 を下回る**。**「少し厳しくする」では直らない。** いまの抽出では 30 行に届くセルは立たない（6 日で 0 個）ので、これは**「プロファイルが無い日は気候値を作らない」**と言っているのに等しく、**それが正しい状態である**（抽出率が上がれば同じ規則のまま効き始める）。**測り方を 1 つ間違えた**のも残した：「セルの行数ごとに B2 単体と B1 を比べる」は**本番のプロファイル経路まで 3.5 倍悪く見せる**——B2 は「いまの台数」を知らない周辺確率なので、**効いているかは混合（B3）で見る**。**本番の経路は触っていない**（セル 198 万・落下 1.5%・係数 B1 +0.876 / B2 +0.075 で縮退なし）。報告書と `model_versions.metrics.climate` には**下限が出る**（`学習サンプル（features/、下限 30 行 2 日）`）。検査は 927 → **932 件**、故意の破壊 6 通りはすべて捕まった。**v1.66（2026-09-20）2 段目を Linux で検算した。§12 の 168 は閉じた（§7.2・§13.1・§4.4）。** マージののち、**同じ場所で・同じ既定の 4 水準**を測り直した（run 35480903448）。**8 日＝28 日ぶんは 常駐 11.30 GiB・退避 0** で、前回の**実需要 15.39 GiB**（常駐 14.67 ＋ 退避 0.96）から **−27%**。**機械（15.61 GiB）に対する余りは 0.22 → 4.31 GiB（1.4% → 28%）**。**傾きは 0.834 → 0.575 GiB/百万行**で、**落ち幅 278 B/行は読んだ表（292 B/行）とほぼ一致する**——削れたのは狙ったところである。**的（28 日・一様）は 11.44 GiB**（余り 27%）、**PR J（66〜69 列）でも 12.07〜12.54 GiB**（余り 20〜23%）で、**今度は PR J を入れても載る**。**尾にも出た**：行列を捨てたあとの常駐は、前は日数に比例して伸びていた（3.83 → 7.64 GiB）が、いまは `Samples` のぶんだけ（2.58 → 5.15 GiB）である。**所要は前回と比べられない**——4 本とも 6〜52% 長いが、**いじっていない尾まで 12〜67% 伸び、水準ごとにばらつく**（4 本は別々のランナー）ので機械の速さが違う（同じ日の 4 本では `6.80 分 ＋ 1.220 分 × 百万行`、的は 27 分）。**副産物として精度も読めた**（報告書が記録に残るようになったため。PR #115）：**h ≥ 45 分は 4 組すべてで門（10%）を越え、h ≤ 30 分は docomo の bike を除いて越えない**——**合計 21 / 40、境目は 45 分**。**ただし学習日は層化のまま**なので、判定は 10/09 以降にやり直す。**v1.65（2026-09-20）当てはめが読んだ表を抱えるのをやめた（§12 の 168 の 2 段目。§7.2・§13.1）。** `load_days` が全日を `pa.concat_tables` で 1 つにして返すのをやめ、**3 つに分けた**（`jobs/window.py`）：**日ごとに取って天気の 4 列だけで被覆と行数を数える**（中身は**圧縮のまま**持つ。28 日ぶんで 0.46 GB ＝ 解いた表の 25 分の 1。**2 度目の取得をしない**）／**日ごとに開いてサンプルにし、語彙を合流させて番号を振り直す**／**先に場所を確保し、1 日読んでは行列の区間を埋めて捨てる**。**山はもう 1 つあった**——`to_samples` が `system_id` と `station_id` を `to_pylist()` で **Python の文字列**にしており、1,800 万行では数 GB になる（**`matrix._encoded` で直したのと同じ形が、隣に残っていた**）。**8 日の回の山が 1.9 分＝読んだ直後に立っていた**のはこれである。**答えは変わらない**——手元で同じ窓を当てはめ直し、**報告書は生成時刻の 1 行を除いて完全に一致**した（Brier も ECE も 1 つも動かない）。山は **5.85 → 4.47 GB**、常駐は **2.11 → 1.45 GB**、所要は 763.7 → 751.3 秒。**手元の窓では読んだ表が 1.66 GB しかない**ので、**文字列のぶんが効くのは行が 1,800 万になったとき**である。**Linux の検算はマージのあと**——`training` 環境は **`main` だけ**を許可する設定で、ブランチのコードでは起動できない（4 ジョブとも 1 秒で失敗）。検査は 908 → **927 件**。故意の破壊 12 通りはすべて捕まったが、**2 つは最初すり抜けた**（日ごとの行数が同じだと検証と学習の取り違えが見えない／「止まること」しか見ていないと門の順番が見えない）——**行数を日ごとに変え、開けないバイト列を渡す**検査を足した。**v1.64（2026-09-20）当てはめの山を `ubuntu-latest` で実測した（§12 の 168 の検算。§7.2・§13.1・§4.4）。** **学習 2・4・6・8 日の 4 本**を同時に走らせた（run 35473387062。**何も書いていない**——`--upload` も `--register` も渡らず、成果物 0 件）。**8 日＝28 日ぶん**（学習 16,607,174 行・読む表 18,415,395 行、的の 98.4%）**は載った。ただし 1.9 分の時点から退避を使っている**——**常駐（`ru_maxrss`）14.67 GiB ＋ 退避 0.96 GiB**、機械は **`MemTotal` 15.61 GiB・`SwapTotal` 3.00 GiB**で、**余りは 0.22 GiB ＝ 1.4%** である。**`ru_maxrss` だけを見ると 0.94 GiB 余って見える**——**Linux の `ru_maxrss` も退避に出たページを数えない**ので、168 が macOS について書いた「計器が低く出る」は**こちらでも起きた**（**天井に当たった回ほど低く出る**——いちばん知りたい回で、いちばん嘘をつく）。**退避を使っていない 3 点の回帰は 山 ＝ 1.883 GiB ＋ 0.834 GiB × 百万行**で、**的（28 日・一様、16,876,720 行）は 15.96 GiB ——機械を 0.35 GiB 超える**。**PR J（62 → 66〜69 列）なら 16.9〜17.6 GiB。** **macOS からの外挿（14.2 GB）は 1.10〜1.20 倍低かった**が、**外れ方は一定**だった。**所要は 26.3 分**で、退避のぶん見込み（21.3 分）より **+23%**。**だから 2 段目（`load_days` を日ごとに流して「読んだ表」4.59 GiB を外す。約 11.4 GiB、PR J 後も約 12 GiB で余裕 23〜27%）は「足りなければ」ではなく、やる。** **公表値も確かめた**——「16 GB」に対して実際に使えるのは **15.61 GiB**（16 GiB の 97.6%）で、**予算の分母は公表値そのものではない**（§4.4）。**測る側も 1 つ直した**：当てはめの報告書と機械の諸元を `GITHUB_STEP_SUMMARY` にしか出しておらず、**要約欄は API からも HTML からも取れない**ため**画面を開いた人にしか読めない数字**になっていた（この回の精度の表がそれ）。**`tee` と `cat` で記録にも流す**ようにした——**要約欄に書くものは、記録にも書く。****v1.63（2026-09-19）当てはめの山を Linux で測る場所を作った（§12 の 168、§7.2、§13.1）。** 「28 日ぶんは 14.2 GB」は**手元 macOS（8 GB、スワップしながら）の 2 点からの 5.6 倍の外挿**で、**週次の再学習が載る `ubuntu-latest` では一度も測っていない**。`bikechance_ml.jobs.measure_fit`（当てはめを**子として起こし**、`ru_maxrss` で山を測り、**1 分ごとに常駐を記録に書く**——OOM で殺されると要約は書けないので、**記録に残った最後の 1 行が唯一の証拠**になる）と `.github/workflows/measure-fit.yml`（`workflow_dispatch` のみ、水準ごとに別のランナー、`fail-fast: false`）を置いた。**何も書かない**——引数を組み立てるのは 1 か所で、`--upload` も `--register` も入らない（**禁じている名前が当てはめ側に実在すること**も別に留めた。旗の名前が変われば、定数は綴りの違う文字列を探し続けて**いつまでも緑になる**）。**いちばん効いたのは、日を複製せずに済んだことである**——本番の在庫を数えると、**09-15 までは層化で 1 日 207 万行、09-16 からは一様で 60.3 万行**。**層化の日が 3.4 倍濃い**ので、**09-08〜09-18 の 11 日が 28 日ぶんの 98.6%** になる（学習 16,607,174 行 / 読む表 18,415,395 行）。**日付をずらした写しを作れば、プロファイルの引き算も曜日も歪む**ところだった。**計器も対照した**：同じ窓・同じコードで macOS の `ru_maxrss` は **0.59 GiB**、`peak memory footprint` は **3.96 GiB**——**6.7 倍違う**。**Linux に圧縮は無い**ので、168 が使った footprint と比べられるのは Linux の `ru_maxrss` である。**件数も訂正した**：28 日は 1,643 万行ではなく **1,688 万行**（以前は 09-12 の一様版 587,270 行で計算していた。実際の一様な日は 602,740 行）。Python の検査は 882 → **907 件**、故意の破壊 12 通りはすべて捕まった——**うち 1 つは仕込みが無意味で最初すり抜けた**（山をたまたま等間隔の点に置いていた。「壊す仕掛けも壊れる」）。**Linux の数そのものはまだ無い**——`workflow_dispatch` は既定ブランチのワークフローしか起動できないので、**マージのあとで測る**。**v1.62（2026-09-19）W6 の障害 3 つのうち 1 つ半を片付けた（§13.1・§7.2）。** **§12 の 166 を直した**——`fit_lightgbm` が `harness.run` に `climate` を渡しておらず、B2（気候値）が**本番と違う作り方**（学習サンプル）で作られていた。**決める場所を `jobs/climate.py` の `source_for` 1 つにまとめ、`harness.run` の `climate` を必須引数にした**——既定値を消した瞬間、`mypy --strict` が欠陥そのものを**70 ファイル中ただ 1 件の誤り**として名指しした。**同じ条件は 3 か所に書かれていて**（`fit_baseline`・`evaluate_baselines`・`harness.run` の既定値）、**そのうち 1 つだけが違う答えを出していた**ので、**複製そのものを検査で留めた**（`FromSamples()` を書いてよいのは 1 ファイルだけ）。**道具が単独で出す数字が、9/17 に手で差し替えた数字と完全に一致する**（ベースライン 160 マス・LGBM 40 マスで差 0）——**判定は予告どおり反転し**、学習 3 日で門（10%）を越えた水平は **2〜3 → 0**、40 のバケツでは **16 → 5** になった。消えたのはすべて h ≥ 30 で、**気候値が効くところではベースラインも強くなる**という筋がバケツの単位でも出ている。**報告書は最初から §11 に「気候値（B2）の作り方：学習サンプル（features/）」と印字していた**——**書いてあったが、読んでいなかった。** **§12 の 168 は 1 段目まで**（9/19、23.7 → 14.2 GB）で、**14.2 GB は手元 macOS の 2 点からの外挿**なので、**Linux で検算してから 2 段目（約 9 GB 見込み）を決める**。**167 は未着手**（プロファイルが無い日の退避路）。Python の検査は 877 → **882 件**、故意の破壊 8 通りはすべて捕まった。**v1.61（2026-09-18）W6 に入る前の棚卸しを §13.1 に置いた。** **W6 の「データ / 基盤」列と「バックエンド / API」列はもう空**で、残るのは LightGBM v1 と iOS の 2 画面だけである。**v1 は暦だけで止まっているように見えて、実際には 3 つで止まっている**——**28 日ぶんの当てはめが 25.1 GB**（§12 の 168。手元 8 GB にも `ubuntu-latest` 16 GB にも 載らない）、**`fit_lightgbm` の気候値が本番と違う**（同 166。h=180 の改善が +13.8% → +11.2% に 縮み、学習 3 日では門を越えた水平が 2 → 0 に反転する）、**一様 1% では `MIN_CELL_SAMPLES = 2` が 低すぎる**（同 167）。**どれも日付に縛られていないので先に潰す**——学習窓が揃う 10/09 を待ってから気づくと W6 が丸ごとずれる。**暦で止まっているのは PR K（9/23、週末を含めて測れる初日）だけ**で、**PR J（9/24）はデータ待ちではなく連休を避けているだけ**なので実装は先行できる。**M4 の判断を v1 を作り込む前に置いた**——§4.5 の実測（numpy $71 / C 版 $52 / 木半分＋スキップ規則 $38）は**どれもクレジット $20 に収まらず**、進め方は W4-11・W4-20 で 9/12 に決着済み（W6 まで numpy、① 特徴量の経路 → ② 大きさ → ③ 評価器）。**残るのは「v1 を active にするか」そのもの**で、決めた瞬間に ① が走り出すので当てはめる前に置く。§7.2 に測り直しで分かった制約を、§13 の W5・W6 の行を今日の状態に更新した。**Apple Developer Program は登録済み**（M5 を縛る最長の外部要因が解消）。**v1.60（2026-09-13）iOS のお気に入りを作った（§9.2・§9.4、W5 の PR H）。** 登録・削除・並べ替えができ、**アプリを落としても残る**（JSON 1 ファイル。SwiftData は使わない）。**20 件で要求はちょうど 20 回**（`/v1/stations/{system}/{id}` を 1 件ずつ。上限が 20 件なのはこのため）。**「いつもの時刻」の確率は、曲線の点をそのまま読む**——詳細の応答は補間していない生の 10 点なので（契約 16）、選択肢を `HORIZONS_MIN` の部分集合にすれば**端末に補間を書かずに済む**（契約 26）。近い点を選んだことは**その点の絶対時刻**で画面に出るので隠れない。**オフラインでは前の値を「○分前の観測」つきで出す**（§9.4）——`observed_at` から作るので**値そのものが自分の古さを持つ**。キャッシュは**受け取ったバイトのまま**持つ（型に写して書き戻すと、サーバーが足した欄が端末で消える）。**App Group の権限はまだ宣言していない**——読む相手（Widget）は W9 で、それまで実機の署名を壊す取引をしない。置き場所を決める関数はコンテナを先に試すので、**W9 で権限を足せば 1 行も変えずに移る**。壊して確かめた 18 通りのうち **4 通りが最初「落ちなかった」**——**すべて検査の穴**で、**検査の道具自体にも競合があった**（20 回のうち 17 回しか記録されなかった。W5 プラン §12 の 161）。Swift のテストは 193 → **225 件**。**v1.59（2026-09-13）低ズームで格子のセルを返せるようにした（§8.3、W5 の PR E、migration 0049）。** **東京駅中心 0.10 度（約 11 km）でいままで 400 だったものが 200 になる**——1,091 ポートが **108 セル**、0.5 度（8,805 ポート）は **94 セル**にまとまる。**`zoom` は受けず、矩形の辺で決める**（同じ矩形に 2 つの答えを作らない）。**どちらを返したかは `aggregation` に書く**（契約 25）——「`cells` が無ければポート」という暗黙の約束を作らないためで、そのぶんポートの応答が **＋24 バイト**になった（中身は完全に一致し、既存の 52 件はそのまま通る）。**代表値は平均ではなく最大**（W5-12。1 台でもあれば答えは「借りられる」）。**集約は SQL、補間は TypeScript**——`interpolateForecast` を 2 つ持たないために、SQL は「水平ごとの最大を取った曲線」までを返す。**`max` と補間は交換しない**ので上限になるが、実測で **89.25% のセルは単独のポートで説明でき（誤差 0）、残りの上界も 0.7 ポイント**（表示の刻み 5% より小さい）。**1,000 件の 400 は構造として起きなくなった**——ポートを返す経路が見る最大の矩形は 0.08 度で、全国でいちばん密な窓でも **824 件**（1,000 を超える窓は 0）。判定は残した。**iOS は 1 ファイルも変えていない**（知らない欄は読み飛ばす）。壊して確かめた 19 通りのうち **4 通りが最初「落ちなかった」**——3 つは測り方のほうが壊れていた（W5 プラン §12 の 159）。**v1.58（2026-09-13）iOS の行程チェック画面を作った（§9.2、W5 の PR G）。** `/v1/trip-check` は W4 から在ったので**画面だけ**——`BikeChanceCore` に判断層（`TripPlan`・`TripOutcomeState`）を置き、View は並べるだけにした。**`p_trip` を出す条件を端末側で 1 段厳しくした**（契約 23）：サーバーの契約 10（両端の予測が非 null）に加えて、**端末が両端の確率を実際に出していること**を要る。フィードが滞って片側を伏せたときに、**隠したはずの数から作った掛け算だけが残る**のを防ぐ。**注記の無い `p_trip` は作れない**（`TripDisplay` はメンバーワイズ初期化子を公開せず、生成口が注記を検査する。W4-24 を型で表した）。**`MKDirections.calculateETA(.cycling)` が効いた**——虎ノ門 → 信濃町でサーバーの概算 14 分に対し **18 分（1.29 倍）**で、§8.3 の見立て（直線の 1.2〜1.4 倍）の内側。叩くのは 1 行程 1 回だけ（端末単位でスロットリングされる）。**本番のバグを 1 つ直した**（契約 24、W5 プラン §12 の 158）：`/v1/trip-check` の代替候補に**別事業者のポートが混ざっていた**——虎ノ門の行程に**厚木（約 40 km）が「80 m 先」として**返っており、原因は近傍の索引を `station_id` だけで作っていたこと（`station_id` はシステムをまたいで 313 件衝突する）。**サーバーで直し、端末でも落とす**。壊れた応答はフィクスチャとして残し、端末側の防御を実データで検査する。Swift のテストは 152 → **193 件**。**v1.57（2026-09-13）当てはめ直しを 1 行にし、成果物が古びたら鳴るようにした（§11.2、W5 の PR M、migration 0048）。** `fit_baseline` は**期間を書かなくても走る**（`--to` の既定は**昨日**、日数の既定は **7 日**。`--from` と `--days` は排他）——**PR D が「窓を延ばす理由」を消した**ので、`features/` を読むのは B1（120 セル）と B3（係数 4 つ）だけになり、1 日 205 万行で 1 セルあたり 1.7 万行になる。**`check_model_freshness()` を `monitor_jobs` の 7 つ目の検査**として足し、`active` な版の `train_days` の最終日が **`model_stale_days`（既定 8 日）**以上前なら 6 時間ごとに 1 回鳴らす（`active` が無いときも別の鍵で鳴る）。**閾値は「異常の定義」ではなく「鳴った日の行動」から決めた**——当初プランの 3 日では**正常な週次運用でも毎週 4 日ぶん鳴る**（W5 プラン §12 の 155）。**昇格は 1 つも自動化していない**（CLAUDE.md §6）。`promote_model_version` を**呼ぶ形**がコードにもワークフローにも現れないことを **`ci.yml` の 3 段**で見張る（**名前の不在では測れなかった**——散文で境界を説明している行が 2 つある。**置き場所も `apps/ml` の pytest では足りなかった**——`python.yml` は `apps/ml/**` が変わった PR でしか走らない。§12 の 156）。**`--upload` は同じ名前の成果物を上書きする**——当てはめ直しが 1 行になったぶん踏みやすくなった落とし穴で、3 か所に書いた（§12 の 157）。**v1.56（2026-09-13）ポート詳細のエンドポイントを実装した（§8.3・§9.2、W5 の PR F、migration 0045〜0047）。** **`capacity_est` / `capacity_days` を `/v1` に出した**——W4-09 の宿題（「ドコモの 5,837 ポートに大きさを表す数が 1 つも無い」）が閉じた（実測：`capacity` は null のまま `capacity_est = 35`）。**値は Storage にしか無かった**ので、**同じジョブ（`build_reference`）が DB にも写す**形にした（SQL で数え直すと 900 万行の unnest になり、同じ量を 2 つの実装が持つ。W5 プラン §12 の 150）。**直近 24 時間は毎時 1 回まとめる表にした**——素直なビューは**本番で 1 ポート 1 日ぶん 616 ms**で、お気に入り（PR H）が 20 件叩くと 1 画面 12 秒になる（§12 の 151）。**集計 3.5 秒／時・表 63 MB（26 時間）・詳細の応答 115〜204 ms**。**`capacity_est = 0` は「分からない」ではなく観測だった**（実測 37 件。0 と NULL を分ける約束。§12 の 152）。**外れ値（1000 以上が 12 件・最大 9999）はそのまま通す**——丸めるなら計算するところで、**モデルも同じ数を読んでいる**ので影響を測ってから（W6。§12 の 153）。契約テストは**本番の実応答 3 本**で固定した。**v1.55（2026-09-13）配信を `baseline-b3-v0-20260912` に入れ替えた（11:50 JST。migration 0044）。** **気候値をプロファイルから作った最初の配信版**で、学習 6 日・ポート 20,663・B2 の使えるセル 198 万（平日のみ）。**成果物は 3.36 → 8.6 MB**（開くピークメモリ 303 → 405 MB、上限 2 GB）。**推論は伸びていない**（HELLO 15,612 ms・ドコモ 20,636 ms。上限 60 秒）。**成果物に無いポートが HELLO 45 → 0・ドコモ 11 → 1**。**9/13 には確率も `confidence` も変わらない**——日曜で、成果物の `sun_holiday` のセルが 0 だからである（W5 プラン §12 の 149）。**効き始めるのは平日の 9/14 から**で、土日祝は **9/21 に当てはめ直して**から。`station_forecasts.confidence` の列コメントを 0044 で直した（0026 の「気候値が無いので最大 2」は W3 段 8 の文言）。**昇格の前に本番で試し打ちした**（`?model=`。どこにも書かない経路）。**v1.54（2026-09-13）B2（気候値）をポートプロファイルから作るようにした（§7.1、W5 の PR D）。** **配信の分岐は 1 つも増えていない**——変えたのは表の作り方だけで、`climatology.predict` はそのままである。 **B2 が B1 に落ちた割合は 30.5% → 4.2%**（平日の検証日。土曜なら 100% のまま。2 日目は 09-19）、**B3 の Brier は B1 を下回った**（hello/bike 総合 0.03342 → 0.02971、B1 は 0.03003。BSS は +1.4% → +12.3%）、**ECE も半分以下**（docomo/bike h=5 で 0.01921 → 0.00648）。**台数バケツ別はほぼ全面的に符号が変わった。** **プランの「`predict_leave_one_out` をそのまま使う」は成立しなかった**——抽出重みは 25 と 100 で、プロファイルの 1 セルは高々 84 点なので分母が負になる（W5 プラン §12 の 143）。**引く量に `Own` という名前を付け、プロファイルからは「その行の日ぶん」を引く**（配信は「その日を 1 点も含まないプロファイル」で予測するため）。**「気候値が引けたか」を確率の一致で判定するのをやめた**（プロファイルの率は53.1% がちょうど 1.0 で、B1 の 120 セルにも 1.0 が在る。§12 の 144）——これで `confidence` の 2 が**「そのポート・その時刻の履歴が足りない」だけ**を意味するようになった（W5-06。**平日昼の 3 は 99.78%**）。**ポートの並びを `Samples.ports` の 1 か所にした**（§12 の 142。§12 の 132 と同じ形が隣に在った）。成果物は **3.36 MB → 8.6 MB**（読み込み 2.71 秒）。**相異なる確率は丸め前で 120〜276**、`x1000` 後は 30〜165 で、**上限を決めているのはもうモデルではなく 1/1000 の刻み**である（§12 の 146）。**v1.53（2026-09-13）測り方を 1 か所に決めた（§7.3・§7.4、W5 の PR C）。** **ECE の判定を「等頻度 15 区間」に揃えた**——プランは最初からそう書いていたのに、実装は **20 等幅**だった（W5 プラン §12 の 133）。**等幅も残して報告書に 2 つ並べる**（§8.1 が等頻度、§8.2 が等幅）——2026-09-13 より前の記録はすべて等幅である。**同じ成果物・同じデータで測り直したら、v0 の ECE は 0.03589 → 0.03568 で判定は動かなかった**（どちらも基準 0.03 を超える）。**差が出るのは LightGBM ではなく B3 のほう**だった（最大 0.01562）——確率が上端に固まっているほど等幅で潰れる。**Brier の値は 1 つも変わらず**、乱数の種の固定（§7.5）も同時に確かめられた。**曜日種別と到着時刻の時間帯**をスライスに足した（鮮度とプロファイルの日数は列が無いので足さない。**軸を足しても既存の数字は動かない**）。**曜日種別の番号の正を `DOW_TYPE_ORDER` 1 つにした**——学習と配信が別々に `sorted()` を呼んでいて、片方を「直す」と過去の成果物が別のセルを指す状態だった。** **v1.52（2026-09-13）ポートプロファイルを作った（§6.3・§8.2・§4.5b、W5 の PR B、migration 0043）。** **気候値（B2）と `prof_*` は同じ量だった**ので、表を 1 つにして両方が引く形にした（W5-01）。**`station_profiles` は DB ではなく Storage の `profiles/date=…/`**（推論が 5 分毎に 200 万行を引かないため。W5-04）。**「9 月では日数が足りない」は誤りだった**——痩せていたのは抽出のせいで、**全格子から数えれば 2 日目に下限を満たすセルが 44.35% → 99.72%** になる（実測）。**28 日を待たず**、`n_days` を持って下限は読む側が決める（W5-03）。作り方は `profile(D) = profile(D-1) + daily(D) - daily(D-28)` で、**読むのは 3 ファイルだけ**。実測は 1 日 199 万セル・`daily` 2.2 MB・`profile` 7.85 MB（6 日）・所要 20 秒・ピーク 1.01 GB。**休止した時間を分母に入れるかは読む側が決める**（`n_suspended` を列に持つ。**99.68% のセルで差が無く、0.28% で最大 0.93 違う**）。Storage は **5〜6 GB/年**増える（`profile` はキャッシュなので短く保てば 0.8 GB/年）。** **v1.51（2026-09-13）予測ログ（`forecast-log/`）を実装した（D-24・R21 を閉じた。§8.2、W5 の PR A、migration 0042）。** **W6 の最初から前倒した**——`station_forecasts` は 5 分毎に上書きされるので、**記録しなかったサイクルは作り直すしかなく**、作り直しには `weather_hourly`（30 日）が要る。**実測は 1 サイクル 129,281 B・1 日 37.2 MB・1 年 13.6 GB**（W4 の PR N の見込み 37.4 MB と一致）。**`maxDuration` を 120 → 240 秒に上げ**、`COMPACT_MAX_DURATION_S` と `INFER_MAX_DURATION_S` を **`ML_MAX_DURATION_S` の 1 つにまとめた**（`vercel.json` の glob は 1 本しかなく、**同じ物理量に名前が 2 つ付いていた**）。**置けなくても推論は落とさない**が、**失敗が記録にしか出ない経路**ができるので、`check_inference` に 3 つ目の条件を足した（直近 3 時間に置けなかった回があれば 6 時間ごとに通知。**理由と、何が失われるかを本文に書く**）。** **v1.50（2026-09-12）Vercel の使用量の内訳が分かった（§4.5a、W4 プラン §6.3d）。** 請求サイクル（09-05〜10-05）の 7 日で **$5.90**、うち **Build CPU が $4.52（77%）**。Fluid の Active CPU は $0.88 で、**その 70% は推論**（`inference_log` の実測 3.04 CPU 時）。**収集・圧縮・天気・属性同期・公開 API を全部合わせて 1 日 0.19 CPU 時・月 $1.95**——§4.5a の積み上げ「約 $4/月」の半分。**§4.5a が「実力値と見てはいけない」と書いた $11.5/月 は、大半がビルドだった。** **推論 ＋ ビルド以外は月 $14.09 でクレジット $20 の内側**だが、**ビルド $19.37 を足すと $33.47 で超える**。**超える要因は推論ではない**——D を入れてもなお $26.8 で超えるので、W4-11 は「**D を採らない**」で決着。** **v1.49（2026-09-12）予測ログ（`forecast-log/`）の置き場と形を決めた（D-24、§4.5b・§5.2・§8.2・§8.5、W4 の PR N）。** **新しいバケット**に **Parquet**（1 ポート 1 行・確率は配列・zstd・UTC の `date=`/`hour=`）。**JSON 列ごと gz のほうが 28% 小さい**が、**読みが 34 倍速い**ほうを採った——効くのは日次評価ではなく**あとから 1 か月ぶんを測り直したくなったとき**である。**定数は覚え書きではなく列に持つ**（+1.7%）：**1 日ぶんを `concat_tables` した瞬間に覚え書きは消える**。**`active` と `shadow` の両方**が書く。**量は実測で 1 日 37.4 MB・12 か月保持で定常 13.7 GB**（§5.2 の見込み 43 MB は大きめだった）——**Storage の残り時間が約 4 か月縮む**ので、D-22 の見直しが 2028-03 → **2027-11** に早まる。**`shadow` は `maxDuration` 120 秒に収まらない**（HELLO 77.7 → 約 144 秒）ので、W6 の最初に 240 秒へ上げる。§8.5 に「オフラインの評価と比べられる形にする」4 条件を書いた。**R21 の設計ぶんを閉じた。** ** **v1.48（2026-09-12）生 JSON からの再構築を本番で確かめた（§5.2、§12 の復旧演習、W4 の PR M）。** 1 日ぶんを**両システムとも**突き合わせ、**4,164 万個の値が 1 つも違わなかった**。**`--dry-run` は生 JSON を 1 バイトも読んでいなかった**（一覧とパーティションを数えるだけ）ので、読んで・解いて・突き合わせるところを作った。規則は `packages/gbfs-core/src/rebuild-check.ts` に置き、**フィクスチャで CI が毎回通す**。復旧演習の欄に実施日を入れた（次は 2026-11-30 まで）。** **v1.47（2026-09-12）天気を生アーカイブから戻せることを本番で確かめ、`job_runs` を刈ってはいけないことを §5.5 に書いた（W4 の PR L）。** 3 発行を組み直して **57,280 値で食い違い 0**。手順書は `docs/260912_runbook_weather_restore.md`（次に見るのは **2026-10-08** より前——最古の発行がその日に消える）。**戻す入口は `v_weather_files` で、これは `job_runs` の `archive_weather` の記録から作られている**ので、`job_runs` を刈ると生アーカイブが在っても戻せなくなる。行は 1 日 2,100 件・年 77 万行増える。** **v1.46（2026-09-12）通知の本文に「理由」と「取り返せるか」を書くようにした（§5.5、W4 の PR K）。** 理由は `job_runs.detail` の**実際に書かれている 4 つの形**から取り出し、どれにも当てはまらなければ生の断片を返す。**再実行では取り返せないジョブ**には `monitored_jobs.failure_note` の一文を足す（いまは `archive_weather` だけ）。** **v1.45（2026-09-12）PR J を本番で確認し、`schedule` の遅れを実測値にした。** **定時の回は 21:00 UTC の予定に対し 22:55 UTC に始まった（1 時間 55 分遅れ）。** 害は無い（対象日は実行時刻から決まるので、遅れが JST の日付をまたがない限り正しい日を作る。またぐには 18 時間要る）が、**見張りの余裕は 30 時間そのものではない**——日次の素の間隔が 24 時間なので、**遅れに使える余裕は 6 時間**である。§4 の制約表と §11.1 に書いた。**閾値は変えない**（1 回の観測で動かさない）。`features/` は **5 日ぶん**そろった。** **v1.44（2026-09-11）学習サンプルの保持と作る場所を決めた（D-22・D-23、§4.5b）。** **`features/` は当面ためる**——捨てる判断の前提は「必要なら作り直せる」ことで、その確認は **PR L（期限 2026-10-08）**である。**確かめていないものを前提に捨てる決定はできない。** 急ぐ理由も無い（13 GB/年で Storage 全体の 3 割、100 GB まで約 2.4 年。先に効くのは `gbfs-raw` の 24 GB/年で、そちらは Drive へ移す手当てが §5.2 に在る）。**見直す合図は Storage 合計 60 GB、または W6 の週次再学習を組むとき。** **作る場所は GitHub Actions**（毎日 06:00 JST、`training` 環境）——Vercel の枠には収まるが、**5 分毎の推論と同じサービスに 3 分・1.3 GB のバッチを混ぜない**（W4-27）。**R20 を閉じた**（日次化と見張りが入った）。** **v1.43（2026-09-11）「特徴量の版は『値が入っている』ことを語らない」を §6.3 と §11.1 に書いた**（W4 の PR I）。手元の 3 日ぶんは**すべて `v3`** だが、天気 4 列が入っている行の割合は **0.000% / 98.589% / 99.977%** である。**同じ版で中身が違う日を混ぜると、配信では必ず天気が在るのに、学習には「天気を一度も見ていない日」の枝ができる**——例外は出ず、確率だけが静かにずれる。**被覆は Parquet を読んだ人がその場で数え**（`features/coverage.py`）、**学習日と検証日で 3 ポイント以上違えば `fit_lightgbm` は成果物を書かない**（`--allow-mixed-weather` で通せるが、`model_versions.metrics.weather` に `spread_pp` と `max_spread_pp` が並ぶ）。§11.1 の再学習の表に「学習データの素性」の行を足した。** **v1.42（2026-09-11）**D-12 を「公開」に書き換えた**。計画時は「私有」と決めていたが、**実態は最初から公開**で、確認のうえ公開を正とした。**費用**：公開リポジトリでは **GitHub Actions が無制限・無料**（2,000 分/月の制約が消える）。**秘密**：全 164 コミットの履歴を走査して**混入 0 件**、`.env` も未コミット。**制約**：秘密を使うワークフローは **`schedule` / `workflow_dispatch` だけ**で起動する（`pull_request` では動かさない）。§4 の制約表と §4.5 の費用表も揃えた。あわせて W4 の **PR H** を実装した（推論の記録に診断 3 つ。W4 プラン §6.8）。** **v1.41（2026-09-11）**費用の計算を実測の Active CPU で出し直した**。Vercel が課金するのは**実時間ではなく Active CPU** で、推論の 1 周期は**実時間 31.4 秒に対し CPU 18.3 秒**（`inference_log.detail.cpu_ms`。`time.process_time_ns()`）である。差は PostgREST から読む待ち時間で、**実時間で計算すると 1.7 倍に見積もる**。§4.5b と §8.2 の表を直した：**いまの使用量は $24 → $14**（クレジット $20 の内側）、LightGBM は $81 → **$71**、C に戻して $61 → **$52**。**結論は変わらない**（LightGBM はクレジットを超える）が、**いまは超えていない**。§6.3c の観測値『$0.38/日＝月 $11.5』と整合するのは直した後の数字である——**照合できる数字が手元にありながら使っていなかった**。あわせて **D-12 の「私有」が実態と食い違っている**ことを記録した（リポジトリは公開。**Actions は無制限・無料**になるが、PR J で Secrets を入れるならワークフローの起動条件に注意が要る）。履歴の秘密スキャンは **0 件**。** **v1.40（2026-09-11）点検で出た宿題を **W5 ではなく W4 のうちに**閉じることにした（W4 プラン §6.8、PR H〜N）。9/12 の Usage と 9/16 の測り直しを待つあいだに並行して進める——**日付で決まっているものが 3 つある**（`weather_hourly` の 30 日保持で **2026-10-08**、`status_snapshots` の 60 日保持で **2026-11-30**、そして毎日積むもの）ことと、**W5 は「貯まったものを使う」側**（校正・履歴プロファイル）なので貯める側の欠けを先に閉じるほうが安いため。§13 のロードマップを W4 に戻し、R20 と R21 の対応先を PR J・PR N にした。**W4-27：学習サンプルの日次生成は GitHub Actions**——実測 41〜65 秒・ピーク 1.28 GB でVercel の枠（300 秒・2 GB）には収まるが、**推論と同じサービスに 3 分・1.3 GB のバッチを混ぜない**。**W4-28：予報の齢は特徴量に足さない**（実効的な先読みはほとんど `h_min` で決まり、齢が伸びるのは 82 発行に 1 回。全部作り直す理由にならない）。** **v1.39（2026-09-11）W4 完了時点の点検（W4 プラン §8.5）を反映した。§8.2 の「1 サイクルの処理」を**実装に合わせて書き直した**——アドバイザリロック（同じ節の構成表と矛盾していた）・校正器・`station_profiles`・一時テーブルへの COPY・**予測ログ `forecast-log/`** の5 点がずれていた。**`forecast-log/` は W6 の `shadow` の前提**なので、W6 の最初の PR に置くと明記した。§5.6・§14 の**再構築スクリプトのパスを実在するものに直した**（`rebuild_hot_store.py` は無い。実装は `scripts/rebuild-snapshots.ts`。期限 2026-11-30 は生きている）。§4.5b に**容量の実測**（DB 定常 約 1.3 GB／Storage 約 41 GB/年。**`features/` の保持を決めていない**）。§13 の W5 に「点検で出た宿題を先に片づける」、W6 に `forecast-log/` を足した。§14 に **R20〜R22** を追加し、**R15（LightGBM の互換性問題）を「実際に起きた」に更新**した。** **v1.38（2026-09-11）W4 の PR G（`/v1/trip-check`）を実装した（migration 0039）。§8.3 の行程チェックの行を実装に合わせ、**`system` を必須にした**（事業者をまたぐ行程は成立しないので表現できなくする。W4-21）。**到着（出発 ＋ 乗車）も水平の範囲で見る**——見ないと「180 分の確率」を 210 分の答えとして黙って返す。**代替候補は確率の高い順に 5 件**（距離順だと 6 番目の 90% が 5 番目の 40% に隠れる）。所見 129：**`lat` / `lon` を「非 null」と宣言していた根拠（bbox で引くから）が、ID で引いた瞬間に崩れた**——本番に座標の無いポートが10 件あり、うち 1 件は予測を持っていた。** **v1.37（2026-09-11）配信の評価器の判断を洗い直した（W4 プラン §12 の 128、W4-20）。§8.2 に numpy と LightGBM の C の**ネイティブ実測（CPU 秒で 1.5〜1.6 倍）**と「**並列化は Active CPU の秒数を 13〜18% 増やす**」を記録し（再現は `scripts/measure-tree-evaluators.py`）、§4.5 の見積り表に**本番実測からの費用の突き合わせ**を足した。**`libgomp.so.1` を同梱すれば `lightgbm` は動く**が、C に戻しても使用量は $61 で**クレジット $20 には届かない**。**効く順は ①行数と木の本数 → ②特徴量の経路 → ③評価器**——特徴量の組み立てだけで月 $13、いまの推論費用の 83% を占める。** **v1.36（2026-09-10）W4 の PR E′（木を numpy で歩く）を実装し、**W4-15 を取り消した**（W4-19）。§8.2 の依存とモデル成果物、§7.2 の v0 の実装、§15 の D-05 を直した。**`lightgbm` は Vercel の Python ランタイムでは読み込めない**——ホイールが要求する `libgomp.so.1` が無く `import lightgbm` が `OSError` で落ちる（W4 プラン §12 の 126）。**ビルドもバンドルの大きさの検査も通る**ので、「入れても動く」は**本番と同じ土台のコンテナで `import` して**確かめる規則にした。配信は木を numpy で歩き、**一致は当てはめ時の照合（2,070,413 行で差 9.992e-16）とゴールデン 21 件と、`lightgbm` を import できない子プロセスでの検査**の 3 段で担保する。本番の依存は 334 → **212 MB**。**マージ後に本番で試し打ちして通ることを確認した**（PR E で `OSError` になったところ）。**ただし速さの見込みが外れた**：クラウドは手元の 1.5〜2 倍ではなく **3.0 倍**遅く、1 サイクルは HELLO **77.7 秒**。`maxDuration` には収まるが、**配ると推論費用が月 $16 → $65 になり $20 のクレジットを超える**（W4 プラン §12 の 127）。**W6 の昇格前に決める。** **v1.35（2026-09-10）W4 の PR E（LightGBM v0 ＋ `model_versions`）を実装した（migration 0038）。§5.3 の DDL に実装との差分を書いた（`kind` は `baseline` / `lightgbm`、`train_days text[]`、**`active` と `shadow` は 1 つだけ**、**昇格の関数には誰にも grant しない**）。§8.2 のモデル成果物の行を実装に合わせ、**環境変数 `BASELINE_MODEL_VERSION` をやめた**。§7.2 に v0 の実装（62 列・カテゴリ 6・語彙はコードの定数・早期終了なし）を書き足した。**`lightgbm` を `apps/ml` の本番依存に入れた**：上限は **500 MB**（Python の関数）で、入れても 334 MB に収まる（W4 プラン §12 の 123。`pyproject.toml` の「250 MB」は Node の数字だった）。** **v1.34（2026-09-10）W4 の PR D（`weather_hourly` と天気の特徴量）を実装した（migration 0037）。§5.3 の DDL に実装との差分を書いた（**`cluster_id` は無く 595 格子の整数添字、時間帯は配列、`wind_ms` ではなく `wind_kmh`**）。§6.2 のリーク防止を「`hour_epoch + 1 時間`（保守的な既定）」から「**`available_at <= t`（厳密版）**」に置き換えた。§6.3 の天気の行を実装に合わせ、**`precip_prob_fcst` と湿度は作れない**ことを明記した（`jma_msm` は降水確率を返さず、湿度は取得していない）。§4.5a の容量に `weather_hourly`（**5.2 MB/日・保持 30 日で約 152 MB**、実測）を足した。** **v1.33（2026-09-10）W4 の PR C（推論側の特徴量経路）を実装した。**推論も学習と同じ `features/` を通り、57 列が完全に一致する**ことをゴールデンで固定した（W4 プラン §6.3a）。§6.2 の train/serve skew に 1 件を足す：**`minutes_since_last_change` は読んだ窓の長さで値が変わる**唯一の列だったので、上限 180 分を入れて閉じ、`FEATURE_SET` をv2 に上げた（W4-10）。除外規則は `features/exclude.py` の 1 実装になり、`jobs/infer.py` の `is_predictable` は消えた。推論の実測は HELLO 10.7 秒・ドコモ 14.0 秒。** **v1.32（2026-09-10）§5.5 に「**ジョブを足したら `monitored_jobs` にも足す**」を書いた。日次の参照スナップショット（`/ml/reference`）が `job_runs` に何も書かず、**止まっても気づけない**状態だった（W4 プラン §12 の 116。migration 0036 で解消）。あわせて W4 §3 の確認を実施し、**`rebuild_geo` は 6.61 秒で定常**・**参照スナップショットの cron は初回から成功**（`capacity_days` が 4 に到達）を確認した。** **v1.31（2026-09-09）W4 の PR B′ を実装した（migration 0035）。§8.3 の `capacity` を「実装予定」から「実装済み」に直した。**ビューが動的な系統の `capacity` を NULL にする**ので、「容量 5・借りられる 12」という矛盾は公開の面から消えた。iOS も**サーバーが数を送ってきても出さない**（古いキャッシュへの備え）。** **v1.30（2026-09-09）§8.3 に「**`capacity` は固定のラック数を意味する列にする**」を足した（W4-09。実装は W4 の PR B′）。ドコモの `capacity` は日次同期の瞬間の `bikes + docks` が凍結された値で、`/v1` がそのまま渡していたため **628 ポート（10.8%）で「容量 5・借りられる 12」のような矛盾**が画面に出ていた（W4 プラン §12 の 115）。動的なシステムでは NULL を返し、ポートの大きさは W5 に `capacity_est` を別の欄として足す。** **v1.29（2026-09-09）W4 の PR B（iOS の到着時刻と確率、M3）を実装し、§9.3 に表示の規則を 4 つ書き足した：**到着は 5 分格子の絶対時刻で送り、絶対時刻で表示する**（相対だと CDN に留まった応答が別の到着を指す）、**借りる／返すは片方だけを出す**（切替では取りに行かない）、**現在値の色と予測の色を同時に出さない**、**5% 刻みと「100% と言わない」は上の端でぶつかり 99% だけが刻みから外れる**。詳細画面の 10 水平の曲線は W5（`/v1/stations` は補間した 1 つしか返さない）。** **v1.28（2026-09-09）§8.3 に「**予測が指す時刻は `generated_at ＋ forecast_in_min`**」を足した（W4 の PR A′）。水平の起点は推論を回した時刻で、PR A はそこに `in_min` をそのまま渡していたため、**利用者の到着より中央 2.1 分早い時刻の確率**を返していた（30 分先で 2.62% が表示の刻みを跨ぐ）。サーバーは行の年齢を足して補間するようにし、CDN 由来の最大 3 分はクライアント側の表示で吸収する。** **v1.27（2026-09-09）W4 の PR A を実装し、§8.3 に `/v1` の予測の規約を書いた。**予測は `at` / `in_min` があるときだけ、補間した 1 到着時刻ぶんを `forecast` に入れて返す**（10 水平の曲線は W5 の詳細画面の持ち場。W4 プラン §9 の契約 4）。**鮮度の判定は TypeScript に置き、SQL には 900 を書かない**（正を 2 つ作らない）。`in_min` の範囲外は張り付けずに 400。本番の実測：予測の付くポートは **99.15% / 99.59%**、結合の増分は **5.4 → 9.1 ms**（468 ポート）、15 分より古い行は 17 時間で **9 件**（0.04%）。** **v1.26（2026-09-09）W4 の実装プラン（`docs/260909_week4_serving_and_model_v0.md`）を作成し、§13 から参照を張った。**W3 が 10 日早く終わったので W4 を 9/10〜9/19 に前倒し**、M3（予測が地図に出る）は 9/17 見込みになった。§13 の W4 の行をプランに合わせて書き直した：天気は **`jma_msm` 一本**（水平 180 分に対し覆域 76 時間で余り、30〜40 度では差が 0.00%）、LightGBM v0 は **`candidate` 止まり**（shadow は予測ログの置き場が要るので W6）、`/v1` は **`base_observed_at` が 15 分より古い予測を返さない**（`FORECAST_STALE_AFTER_S` は定義済みで未使用だった）。**推論側の特徴量経路（`build_now`）と、学習との一致のゴールデンが W4 の本体**である。** **v1.25（2026-09-09）0031 の検知が初回で実物を捕まえたが、**その行は自分では閉じない**ため通知が鳴り続けることが分かった。日次の保守が 1 時間超の `running` を `failed`（`error = 'never_finished'`）にして閉じるようにした（0032。W3 プラン §14.8）。**検知を入れたら解消の経路も一緒に作る。** あわせて `cpu_ms` の初実測（HELLO は実時間 5,907 ms のうち CPU 2,218 ms＝37.5%、ドコモは 1,756 ms のうち 269 ms＝15.3%）を得た。§4.5a の費用の見積りを詰めるときの材料になる。** **v1.24（2026-09-09）W3 の §14.2 の 4・5 を実装した（0031）。`inference_log` の保持を**90 日・`ok` のみ**にし（失敗と二重抑止と閉じられなかった行は残す）、**閉じられずに `running` のまま残った行**の検知を `check_inference` に足した（W3 プラン §12 の 111）。§5.3 の `cpu_ms` は**列にせず `detail` に入れる**ことにし、§5.5 の「所要 CPU は記録していない」を解消した。** **v1.23（2026-09-09）日次の参照スナップショットを実装し、`build_features` を切り替えた（`FEATURE_SET` v0 → v1。W3 プラン §14.3・§14.6）。§3 の `capacity_est` は**計画どおり過去 7 日**になり、ビルド窓から独立した。同じ日を新旧で作って突き合わせたところ、**動いたのは 65 列中 5 列**で全部 `capacity` の下流、**抽出と除外は 1 件も変わらなかった**。副産物として `is_over_capacity` が意味のある列になった（true が 0.07% → ドコモの 2.7%。旧定義では当日の観測を含む累積最大なので定義上ほぼ超えられなかった）。** **v1.22（2026-09-09）W3 の残りの実装を切り分けた（W3 プラン §14）。**日次の参照スナップショット**（`reference/date=YYYY-MM-DD/` に `stations` と `neighbors` の Parquet を 1 組）を導入し、§6.2 の参照データの as-of（§13.2）と §3 の `capacity_est`（§13.3）を一度に閉じる。**学習も推論も「基準時刻の前日の版」を読む**規則にする。`inference_log` の保持は W7 から W3 の残り期間へ前倒した。`/v1` の鮮度切りと推論側の観測履歴は W4 のプランへ送る。** **v1.21（2026-09-09）W3 完了時点の点検を反映した（W3 プラン §13・§14）。§6.2 のリーク防止に「**参照データは『いまの値』を使っている**」を追加（属性は現行行だけ、近傍は履歴が無い、行政区画は上書き。目的変数の漏洩ではないが再現性が担保されない）。§3 の `capacity_est` を実装に合わせて注記（**計画は 7 日、実装は約 49 時間**の累積最大）。§5.3 の `inference_log.cpu_ms` は**実装に列が無い**ことを明記し、§5.5 の監視の記述も直した。§13 の W7 に `inference_log` の保持を足した。** **v1.20（2026-09-09）W3 の段 8 の手順 5（推論のウォッチドッグ）を実装し、§8.2 のウォッチドッグの行を実装に合わせた。発火の条件に「**まだ推論していない観測がある**」を足した（収集の停滞で叩いても進まないため。あちらは `monitor_feeds` の持ち場）。鮮度は `inference_log` ではなく `station_forecasts` で測る（推論が失敗していても `inference_log` には行が増える）。`generated_at` に索引は張らない（HOT 更新率 100% が崩れる）。** **v1.19（2026-09-08）W3 の段 8（`/ml/infer` の配線）を実装した。§8.2 の冪等の記述を直した：**PostgREST 越しにアドバイザリロックは効かない**ので、`inference_log` の一意制約に行を入れられたかどうかで掴む（W3 プラン §12 の 103）。モデル成果物の指定を、`model_versions` がまだ無い段階では環境変数で行うことにした。§5.3 の予測テーブルは実装の台帳（`(system_id, station_id)`）に合わせ、台数の予測区間と `model_versions` は作っていない（W3 プラン §5.10）。** **v1.18（2026-09-08）W3 の段 7（ベースライン B0〜B3 と評価ハーネス）を実装し、§7.1 を実測に合わせて更新した。**採用基準を「バケツ別 ＋ Brier ≥ 0.001 の絶対下限」に変えた**（W3-16。総合値だけだと、価値のある 0〜2 台の領域で負けていても合格する）。B1 のバケツは当てる対象と同じ側の台数で切ると明記し（W3 プラン §12 の 100）、B2 のセルの下限に**日数**の条件を足した（同 101）。ベースラインは §6.1 の除外規則を通した集合の上でだけ測ることも明記した（W3-15）。結果は `docs/260908_eda_02_baseline.md`。** **v1.17（2026-09-08）W3 の段 6（特徴量パイプライン v0）を実装した。§6.2 の重点サンプリングについて実測を追記：難所を「`bikes ≤ 2` または `docks ≤ 2`」と定義すると母集団の **83.9%**（HELLO 86.9%・ドコモ 76.5%）になり、**層化がほとんど効かない**（1 日 2,053,383 行・34.8 MB。見積り 140 万行に対し ＋47%）。重みは厳密なままなので v0 は定義どおりにし、閾値の見直しは W4 でベースラインの評価と一緒に判断する（W3 プラン §12 の 99）。あわせて HELLO の as-of が常に約 205 秒古いことを実測した（同 98）。** **v1.16（2026-09-08）W3 の段 5（`pref_code` / `muni_code` と近傍リスト）を実装し、本文を実装に合わせた。行政区域の出どころを国土数値情報 N03 から**総務省「全国地方公共団体コード」への住所の前方一致**に変え（§6.4）、§5.3 の DDL コメントを「座標から導出」から「住所から導出」に直した。**LightGBM のカテゴリ特徴量から `pref_code` を外し `muni_code` に替えた**（§7）：近傍が 1 つも無いポート（行政区画コードが実際に働く唯一の場面）で `pref` は全体平均より MAE が 0.0102 悪く、`muni` は 0.0259 良い。計測は `scripts/measure-geo-fallback.sql` で再現できる（W3 プラン §10.2a）。** v1.0（2026-09-04）初版。**v1.1（2026-09-04）Vercel Pro を前提に推論・スケジューリング・収集器の配置・費用・商用化の考慮を再設計（§4〜§5、§8、§11〜§15）。「AI と人間で別 API を作らない」原則を CLAUDE.md と本書から削除し、API 層の採用理由を技術的根拠に書き直した。§15 の推奨を決定として確定し、Vercel Pro に伴う新規論点 D-17〜D-21 を追加。** v1.2（2026-09-04）D-17〜D-21 を決定として確定。CLAUDE.md をプランに沿って改訂。v1.2a（2026-09-06）§15 の D-04 に ODPT の認証方式とレート制限の実測値を追記（決定内容は変更なし）。v1.2b（2026-09-06）§15.1 に W1 実装での決定（W1-01〜W1-31）を転記し、§5.3 の DDL と実装の食い違いを明記。v1.2c（2026-09-06）§5.2 に本番での容量実測を追記（生データは見積りどおり、配列は見積りの半分以下）。v1.2d（2026-09-07）§5.2 の容量を 24 時間観測で確定（DB 22.2 MB/日、60 日で約 1.3 GB）。ドコモの実更新周期を 80.9 秒と確認。v1.2e（2026-09-07）§8.3 に「属性の無いポート」と `geo_suspect` の扱いを追記し、§5.3 の実装差分に `geo_suspect` の復活と `capacity_is_dynamic` を追記。v1.2f（2026-09-07）§8.3 の「属性の無いポート」の根拠を本番の実測に合わせて訂正（恒常的に 11 件ではなく、新しいポートが最大 1 日のあいだ持たない）。**v1.15（2026-09-08）天気アーカイブの `forecast_days` を 2 → 3 に広げた。窓は取得時刻によらず JST 当日 0 時から始まるため先の覆いが時刻とともに縮み、`2` では夜のファイルが約 25 時間先までしか無かった。`jma_msm` が値を返すのは当日 0 時から 76 時間ちょうどで、**3 日（72 時間）が欠けない最大**である。費用は容量 +45%（4.4 → 約 6.4 MB/日、年 2.3 GB）、所要 +6%、呼び出し回数は不変。§4.5a の容量を更新した。あわせて 2 モデルの食い違いが緯度で決まることを 371,280 値で確認し（30〜40 度で 0.00%、沖縄・北海道北部で 21〜48%）、`constants.ts` の「バイト単位で同値」というコメントとデータ辞書 §8.3 を直した。** **v1.14（2026-09-08）W3 の実装プラン（`docs/260909_week3_dataset_and_baseline.md`）を作成し、§13 から参照を張った。あわせて §13 の W3 の行を実装プランに合わせて整理した：**`job_runs` の保持を W7 へ移し**（`archive_weather` の行は天気予報の入手時刻の記録そのもので、素直に「N 日より古い行を消す」と書くと W3 の④が使う材料を消してしまう）、**履歴プロファイルの日次ジョブを W4〜W5 へ**（過去 28 日の集計で、9 月の時点では日数が足りない）、行政区域は HELLO の住所から `pref_code` と `muni_code` を埋めるところまでとした（ドコモ向けの N03 ポリゴン照合は見送り。実測で、地理の特徴量は近傍リストのほうが市区町村より当たり〔LOO R² 0.201 対 0.132〕、近傍の残差に市区町村が足すのは 1.49%、N03 で埋まるのは「ドコモかつ 1 km 以内に近傍 0 件」の 136 ポート〔0.66%〕だけだった。根拠は W3 プラン §10.2a）。** **v1.13（2026-09-08）§13 の W3 に、点検で見つけた 4 件の実装順を書き入れた（詳細は W2 プラン §14）。** v1.12（2026-09-08）W2 完了時点の点検を反映。§6.2 のリーク防止に 2 つの規則を追加（as-of 結合は `fetched_at` で切る／天気の予報ファイルは `hour_epoch + 1 時間` から使う）。§6.3 の `gap` は生 JSON でなく DB の値で計算してよいと緩和（丸めは 0.038%）。§5.5 に「監視できていないもの」を明記し、§14 に R19（ジョブの失敗と Parquet の欠落が無監視）を追加。§13 のロードマップを W2 の実績に更新。** v1.11（2026-09-08）EDA #1（`docs/260908_eda_01.md`）の結果を反映。§6.1 の除外規則に「実在しないポート」（ドコモ `5753`）を追加し、§3.4b に「毎分収集の価値はシステムで違う」（HELLO 0.0% / ドコモ 14.8%）を追記した。** v1.10（2026-09-08）`docs/data_dictionary.md`（データ辞書）を新設し、§6.7 から参照を張った。実測にもとづく訂正 2 件：`parking_type` / `parking_hoop` は分散ゼロで特徴量にならない（W4 で見る必要は無い）、`is_charging_station` は 1.8% が true で使える。** v1.9（2026-09-08）§4.5 に「見積りと実測の突き合わせ」（§4.5a）を追加。呼び出し回数・所要・課金・容量を本番の実測と並べ、圧縮の見積りが 1 桁大きかったこと（40 秒想定に対し 3.3 秒）を記録した。§14 に R18（Spend Management の自動停止で収集が止まる）を追加。** v1.8（2026-09-08）§8.3 に W2 の PR E で確定した `/v1` の規約を反映（bbox はサーバー側で 0.01 度に量子化、件数の上限 1,000 件、Problem Details の書式、DB 不達時の扱い、`stale` の意味）。** v1.7（2026-09-08）M2 達成に伴い §5.2 の学習用アーカイブを実測で確定（4.05 MB/日、年 1.5 GB、0.61 / 0.26 B/行）。§5.6 の毎時圧縮ジョブの入力を「生 JSON を S3 互換で読む」から「`status_snapshots` を PostgREST で読む」に訂正（実装に合わせた）。§12.1 の `vercel.json` を `services.<name>.functions` の形に訂正。** v1.6（2026-09-08）§6.7 に「配列長は時系列で単調とは限らない」を追記（再構築を挟むと逆転する）。 v1.5（2026-09-07）§13 から W2 の実装プラン（`docs/260907_week2_archive_and_serving.md`）へ参照を張った。** v1.4（2026-09-07）§13 の W1 を実績に更新し、W2 の着手順（天気バックフィルの検証 → 通知先の設定 → Parquet 化 → 再構築）を明記。§5.5 を W1 の実装に合わせ、通知先が未設定であることを記録。** v1.3（2026-09-07）W1 完了時点で蓄積データを学習の観点から点検し、§6.1 の除外規則を書き直し（`is_active` と `capacity=0` による除外をやめた）、§6.3 を実測のフィールドに合わせ、§6.7「データの棚卸し」と R16・R17 を追加、§5.6 に再構築の期限（2026-11-30）を明記。**
...
| **GitHub Actions** | **公開リポは無制限・無料**（私有なら 2,000 分/月。D-12）。スケジュール最短 5 分、**遅延あり（実測 1 時間 55 分。2026-09-12）**。**60 日無活動で `schedule` は自動停止**。ジョブ最長 6 時間。**`ubuntu-latest` の実測（2026-09-20）：`MemTotal` 15.61 GiB・`SwapTotal` 3.00 GiB**（**公表の「16 GB」に対して使えるのは 15.61 GiB ＝ 16 GiB の 97.6%**。退避があるので「足りない ＝ 即失敗」ではなく「遅くなる」） | CI・**学習サンプルの日次生成**（06:00 JST。D-23）・週次再学習・週次 Drive ミラー。**秘密を使うワークフローは `schedule` / `workflow_dispatch` だけで起動し、鍵は `training` 環境に置く**。**週次再学習（28 日）の山は 2026-09-20 に 11.30 GiB（8 日＝28 日ぶんの実測、退避 0）**。的（28 日・一様）は 11.44 GiB の見込みで**余り 27%**、PR J 後でも 12.07〜12.54 GiB（§13.1） |
...
**どれもクレジット $20 に収まらない。** ~~判断は W4-11（9/12〜13 の Usage）と W4-20（評価器）で行う。~~ → **どちらも 2026-09-12 に決着した**：**W6 まで numpy を維持し**、判断は **① 特徴量の経路 → ② モデルの大きさ → ③ 評価器**の順で行う（W4-20）。**「LightGBM を配る」と決めた瞬間に ①（月 $8）が最優先になる**（W4-11）。**残っているのは「v1 を active にするか」そのもの**で、§13.1 の (d) 3 に置いた。

**2026-09-24 に測り直した**（`docs/260924_m4_cost_and_schedule.md` §1。D-32）。**上の表の 4 行目（木半分＋スキップ規則）は前提が外れていた**——スキップ規則は全行の 0.81% しか当たらない（同 §3 の 181）。**代わりに、本数と配る範囲で下げられる**と分かった：

| 配信しているもの（**使用量・公開後の定常**） | 森（秒/周期） | `hnd1` | `iad1` |
|---|---:|---:|---:|
| B3 だけ（いま。**開発中は $18.9**） | 0 | **$16.8** | $12.7 |
| v1 の設計どおり（300 本・全水平） | 104 | $75.7 | $50.1 |
| 300 本・門をセル単位で当てる（森を歩く行 62%） | 65 | $53.5 | $36.0 |
| 100 本（学習率 0.15）・セル単位 | 21 | $28.6（＋B で $25.9） | $20.2（＋B で **$18.5**） |
| 50 本（学習率 0.15）・セル単位 | 10 | $22.3（＋B で **$19.6**） | **$16.2** |

**100 本で 300 本の改善の 96%、50 本で 93%**（予備の当てはめ。門を越える組の数は同じ）。**「$20 の内側で v1 を配る」は、本数を予算から逆算すれば成り立つ。** 単価は Vercel の地域別料金（2026-09-14 更新）で、`hnd1` と `iad1` の差は −36.6%。

**W6 の変更ぶん**（書式 v2・J2・shadow・合成器・特徴量の経路 B）の費用と容量は、**`docs/260924_week6_model_v1_and_release_prep.md` §4.9 に先に書いた**（CLAUDE.md §6）。**要点は 3 つ**：①書式 v2 で成果物が 20.2 → 2.3 MB になり（9/25 に実装で測った。ほぼ埋まった成果物でも 2.5 MB）、冷えた起動の下りは月 17 → 約 1.4 GB、②**J2 で下りが月 +24 GB**（1 周期 1.51 MB）、`profiles/` の `profile.parquet` は 15 → **44 MB/日**（35 日の掃除は W7。§4.5b の `profiles/` の行は W7 に直す）、③shadow の 4 日は全行ぶんの森（100 本なら約 $2.6）
...
| Storage `features/`（**当面ためる**。下の段） | ~~36 MB~~ → **14.9 MB** | ~~13 GB/年~~ → **5.4 GB/年** | — |
...
| Storage 合計 | — | ~~約 46~~ → **約 38 GB/年 ＋ 定常 13.6 GB** | 100 GB（~~約 1.9 年~~ → **約 2.3 年**） |

> **`features/` は 2026-09-16 に 3 分の 1 になった**（D-25、W5 の PR I）。層化（難所 4%・他 1%）をやめて一様 1% にしたため、**1 日 40.9 MB → 14.9 MB**（実測）。**行数の比は 0.28 なのに容量の比が 0.36 なのは**、層化した標本のほうが同じポート・近い時刻の行が固まって並び、辞書符号化がよく効くため（19.7 B/行 → 25.4 B/行）。**表の「36 MB」は 2026-09-11 の実測**で、直前の実測は 40.9 MB だった。
...
**急ぐ理由も無い。** `features/` は **5.4 GB/年**で Storage 全体（約 38 GB/年）の **1.4 割**、100 GB まで**約 2.3 年**ある。**先に効くのは `gbfs-raw`**（24 GB/年）で、そちらは「12 か月超は Drive へ移す」という手当てが §5.2 に元から在る。**貯めておけば「あるモデルを作った正確なバイト列」が残る**という利点もある。
...
- 母集団：ポート 21,149 × 1 日 288 基準時刻 × 10 水平。除外を通った後で **1 日 5,875 万サンプル**（2026-09-16 に 9 日ぶんの逆抽出確率で推定）。全件は不要かつ非効率。
- **サンプリング（v2。2026-09-16 に層化をやめた。§15 の D-25）**
  1. **一様 1%。**（ポート, 基準時刻, **水平**）の**行ごと**に乱数を 1 つ引き、`SAMPLE_RATE = 0.01` の内側なら採る（**約 59 万行/日**）。~~ペアの 1% を引いて 10 水平すべてをラベル化~~ → **実装は最初から行ごとだった**：1 ペアに入る水平は平均 **1.19 本**で、10 本そろうペアは 1 つも無い（W5 プラン §12 の 164）。**散らばっているほうが推定の分散は小さい。**
  2. ~~難しい領域の重点サンプル：`bikes ≤ 2` または `docks ≤ 2` を追加で 3% 抽出~~ → **やめた**（2026-09-16）。**難所は稀ではない**——母集団の **84.3%** が該当する（当初の見積りは「全ペアの 4 割前後」だった）。層 4%・他 1% が同じ行数の一様抽出より多く取れる難所は **+13.3%** しかなく、`weight` と `stratum` が 27 ファイルを通る代償に見合わない。**閾値を下げても足りない**（`≤ 1` で 69.2%、`≤ 0` でも 43.3%）。測り方と 3 案の比較は W5 プラン §6.9。
  3. 各サンプルに **逆抽出確率の重み**（`1 / SAMPLE_RATE` ＝ 100）を付け、LightGBM の `weight` に渡す（確率推定の不偏性を保つ）。校正・評価は重み付きで行う。**一様なので値は定数だが、列は残す**——2026-09-15 以前の日は層化（25 と 100）で作ってあり、混ぜて読むときに行ごとの重みが効く。
- 合計 **約 59 万行/日**、**60 日で約 3,500 万行**。LightGBM の内部ビニング（1 特徴 1 バイト）で約 **2.2 GB** となり、**GitHub Actions（2 コア / 7 GB）でも 60 日窓のまま週次再学習できる**（層化時は 8,500 万行・約 5 GB で、抽出率を半分にするか 8 週窓に縮める必要があった）。容量は **14.9 MB/日 ＝ 5.4 GB/年**（層化時は 40.9 MB/日 ＝ 14.9 GB/年。どちらも実測）。特徴量は日次で Parquet 化（`features/date=YYYY-MM-DD/`）し、再学習時は必要期間だけ読む。
- **リーク防止の原則**
...
| B2 | 気候値（Climatology） | `P̂(y=1 ∣ station, dow_type, slot15(t+h))`。セルは **下限（件数・日数）を満たすものだけ**使い、足りなければ B1 に落として**その割合を報告する**（件数だけで数えると 1 日で満たせてしまう。W3 プラン §12 の 101）。**件数の下限は作り方で違う**（2026-09-20。W5 プラン §12 の 167）——**プロファイルから作るときは 2 点**（1 セルに 1 日 3 点入る）、**学習サンプルから作るときは 30 行**（1 セル 1 日 0.3 行しか来ず、2 行の率は 0・0.5・1 の 3 通りしか取れない）。**同じ「2」が違うものを数えていた**のがその所見である。**セルは §6.3 のポート履歴プロファイルから作る**（2026-09-13。同じ量を 2 か所で作らない。W5-01/02）——**抽出した学習サンプルからではない**：1 セル 1 日あたり 1.62 行に対し**全格子なら 3 点**で、B1 に落ちる割合が **30.5% → 4.2%** に下がる（実測。W5 プラン §6.4）。**学習期間の行に当てはめるときは、その行の日ぶんを丸ごと引く**（配信は「その日を 1 点も含まないプロファイル」で予測するため）。文献では 1 時間超の水平で持続予測を上回り、2〜3 時間を超えると現在台数は情報を持たなくなる（§8.1 の 3 時間境界の根拠） |
...
- **ECE もバケツ別と曜日種別別に見る**（2026-09-23 に足した。W5 プラン §12 の 178、`docs/260923_eda_07_calibration.md`）。**水平別の全体の ECE は、難所の歪みを隠す**——配信中の B3 は全体の校正の傾きが 0.84〜1.23 でほぼ正常なのに、**バケツ 1 の中では 0.08〜1.32 とばらつき、B2 が薄い祝日ほど小さかった**（0.08〜0.52）。**v1 は ①バケツ 0・1 の ECE と校正の傾き ②`weekday` と `sat`/`sun_holiday` を分けた ECE も並べて判定する**。**v1 に求める理由がある**：B3 のこの欠陥は「B2 の重みがバケツも日数も知らない」ことから来ており、**木はバケツ × `prof_*` × `prof_n_days` の掛け合いを持てる**（D-27）
...
> **2026-09-17 に測り直した**（学習 1〜6 日、当てはめ 12 本。`docs/260917_eda_05_lightgbm_remeasure.md`）。
> **上の 3 行はそのまま使えない。** `fit_lightgbm` は気候値をプロファイルから作らないので、
> **相手が本番より弱い**（W5 プラン §12 の 166）。本番と同じベースラインに直すと、学習 6 日で
> 門を越えるのは **18 / 40**（system × ターゲット × 水平）、水平だけなら **4 / 10** である。
> **ECE はもう門ではない**——12 本すべて・全 40 水平で 0.03 未満だった（学習 6 日の中央値 0.0036）。
> **28 日の見込みは 7〜8 / 10 水平**で、**h = 5 分と 10 分は届かない**（外挿）。
...
- ハイパーパラメータ初期値：`num_leaves 127`、`learning_rate 0.05`、`min_data_in_leaf 1000`、`feature_fraction 0.8`、`bagging_fraction 0.8`、`lambda_l2 10`、早期終了は時系列検証セットで 100 ラウンド。木の本数は推論 CPU 予算（§8.2）を考慮して 300〜500 本に制限し、必要なら `num_leaves` を増やす。
  **→ 2026-09-24：300〜500 本は予算に合わない**（300 本で使用量 $54〜76。§4.5）。**本数は費用の上限から逆算し（約 50〜120 本）、学習率を本数に合わせて上げる**（D-32 の (c)）。予備の当てはめ（学習 8 日・v4）で、**100 本・学習率 0.15 は 300 本・0.05 の改善の 96%、50 本で 93%**。**学習率を 0.05 のまま 100 本で止めると ECE が 0.0311 で基準を割る**——本数と学習率は組で決める。**葉は 127 のまま**（葉 63 は 1 本 22% 安いが、同じ費用で比べると差が無い）。`docs/260924_m4_cost_and_schedule.md` §1.4
- サンプル重み：§6.2 の逆抽出確率。
...
**v0 を測り直して分かった、v1 に効く制約（2026-09-17〜18）。** 学習 1〜6 日の 12 本を
同じ検証行の上で当てはめた（`docs/260917_eda_05_lightgbm_remeasure.md`）。**設計そのものは
変えないが、v1 を作る前に片付けなければならないことが 3 つ出た**（§13.1）。

- **28 日ぶんの当てはめは 25.1 GB 要る。** 12 本の実測を回帰すると
  **山 ＝ 2.37 GB ＋ 1.39 GB × 百万行**（残差は最大 0.25 GB）で、28 日 ＝ **16.88M 行**
  （2026-09-19 訂正。以前は 16.4M と書いていたが、それは 09-12 の一様版 587,270 行で
  計算したもので、**実際の一様な日は 602,740 行**である。読む表は 31 日で 18.68M 行）。
  **行列そのものは 7.96 GB**（62 列 × float64）しかなく、**山が 3 倍あるのは持ち方による**
  ——`fit_and_score` が検証行の行列と照合用の写しを**同時に持ったまま**学習の行列を作る。
  並列にしても直列と **0.5% しか変わらない**ので、重いのは往復ではない。
  **手元（8 GB）にも `ubuntu-latest`（16 GB）にも載らない**（§12 の 168）。
  → **2026-09-19 に 23.7 → 14.2 GB まで下げた**（実測。同じものを 2 つ持つのをやめ、
  行列を float32 にした）。**載るが余裕は 1.8 GB しかない**ので、次の梃子
  （`load_days` を日ごとに流して「読んだ表」4.81 GB を外す。見込み約 9 GB）を残してある。
  → **同じ日に測る場所を作った**（`.github/workflows/measure-fit.yml` と
  `bikechance_ml.jobs.measure_fit`）。**28 日ぶんの規模は今日そのまま実データで測れる**
  ——学習サンプルは 09-15 までが層化（約 207 万行/日）で **3.4 倍濃い**ので、
  **09-08〜09-18 の 11 日が 28 日ぶんの 98.6% になる**（学習 16,607,174 行 /
  読む表 18,415,395 行）。**日を複製する必要は無い**
  → **2026-09-20 に `ubuntu-latest` で 4 水準（学習 2・4・6・8 日）を測った。**
  **8 日（＝28 日ぶん）は載ったが、退避を 0.96 GiB 使っている**：常駐（`ru_maxrss`）
  **14.67 GiB**・実需要（常駐＋退避）**15.39 GiB** に対し、機械は
  **`MemTotal` 15.61 GiB・`SwapTotal` 3.00 GiB**（**公表の「16 GB」に対して使えるのは
  15.61 GiB ＝ 16 GiB の 97.6%**）。
  **余りは 0.22 GiB＝1.4%。** 退避を使っていない 3 点の回帰は
  **山 ＝ 1.883 GiB ＋ 0.834 GiB × 百万行**で、**的（28 日・一様、16,876,720 行）は
  15.96 GiB ——機械を 0.35 GiB 超える**。**PR J（62 → 66〜69 列）で 16.9〜17.6 GiB**。
  **macOS からの外挿（14.2 GB）は 1.10〜1.20 倍低かった**（計器の差ではなく、
  **Linux の `ru_maxrss` も退避に出たページを数えない**）。所要は **26.3 分**で、
  退避のぶん見込み 21.3 分より **+23%**。**したがって 2 段目（`load_days` を日ごとに
  流す。約 11.4 GiB、PR J 後も約 12 GiB）はやる**——「足りなければ」ではない
  → **2026-09-20 に 2 段目も実装した**（`jobs/window.py`）。`load_days` が全日を
  1 つの表にして返すのをやめ、**日ごとに取り・天気の 4 列だけで被覆を数え・
  1 日読んでは行列の区間を埋めて捨てる**形にした。**山はもう 1 つあった**
  ——`to_samples` が `system_id` と `station_id` を `to_pylist()` で Python の
  文字列にしており（`matrix._encoded` で直したのと同じ形）、**8 日の回の山が
  1.9 分＝読んだ直後に立っていた**のはこれである。**手元の A/B では報告書が
  生成時刻の 1 行を除いて完全に一致**し、山は 5.85 → 4.47 GB（小さい窓）。
  → **2026-09-20 に Linux で検算した（run 35480903448）。載った。退避は 0。**
  8 日（＝28 日ぶん）は **実需要 15.39 → 常駐 11.30 GiB**（機械 15.61 に対し
  余り 0.22 → **4.31 GiB ＝ 28%**）。傾きは **0.834 → 0.575 GiB/百万行**で、
  落ち幅 278 B/行は**読んだ表 292 B/行**とほぼ一致する。**的（28 日・一様）は
  11.44 GiB**（余り 27%）、**PR J（66〜69 列）でも 12.07〜12.54 GiB**（余り 20〜23%）
  ——**今度は PR J を入れても載る**
- **`fit_lightgbm` の B2・B3 は本番より弱かった。** `harness.run` に `climate` を渡して
  いないので気候値が**学習サンプルから**作られていた。本番と同じプロファイルの気候値に
  直すと、学習 5 日の h=180 の改善は **+13.8% → +11.2%**、学習 3 日では門を越えた水平が
  **2 → 0** に反転する。**「同じ関数を 2 か所から呼ぶなら既定値で分岐させない」**（§12 の 166）
  → **2026-09-19 に直した。** B2 の作り方を決める場所を `jobs/climate.py` の `source_for`
  1 つにまとめ、`harness.run` の `climate` を**必須引数**にした。**道具が単独で出す数字が、
  手で差し替えた数字と完全に一致する**（ベースライン 160 マス・LGBM 40 マスで差 0）。
  **以後、報告書と `model_versions.metrics` に「B2 をどう作ったか」が残る**
- **一様 1% では `MIN_CELL_SAMPLES = 2` が低すぎる。** 学習サンプルから作る気候値は
  **セルが埋まるほど悪くなり**（B2 の Brier 0.041 → 0.092）、3 日目から B0（持続）より悪い。
  2 件しかないセルの率は 0・0.5・1 の 3 通りしか取れない（§12 の 167）。
  **166 を直したので本番の経路は通らなくなった**が、**プロファイルが無い日は今でも
  ここに落ちる**（`--no-profile` の退避路）。**未着手**

**見通し（外挿。当たる保証は無い）。** 本番と同じベースラインを相手にすると、学習 6 日で
門（10%）を越えるのは **18 / 40**（system × ターゲット × 水平）。28 日への外挿は
**7〜8 / 10 水平**で、**h = 5 分と 10 分は届かない**。
→ **2026-09-20 に 28 日ぶんの規模で当てはめて読んだ**（メモリの測定の副産物。
W5 プラン §12 の 168）：**h ≥ 45 分は 4 組すべてで門を越え、h ≤ 30 分は
docomo の bike（+12.3%）を除いて越えない**——**合計 21 / 40**。
**境目は 45 分**である。**ただし学習日は層化のまま**（本番の v1 は一様 1%）で
検証日も違うので、**判定は 10/09 以降にやり直す**。**「長い水平は LightGBM、短い水平は
ベースライン」という配信の切り分け**が現実的な着地である。**ECE は門になっていない**
（12 本すべて・全 40 水平で 0.03 未満）。
→ **2026-09-24 に v4（`prof_*`）・一様 1% で当てはめ直した**（学習 09-08〜09-15 の 8 日、
検証は同じ 09-17〜18。`docs/260924_m4_cost_and_schedule.md` §1.4）：**31 / 40**。
**h ≤ 30 分も 11 / 20 が越える**——越えないのは HELLO の bike の h ≤ 30 分（+3〜10%）と
短い水平の一部だけ。**上がったのは `prof_*` による**（抜くと 18 / 40、配るときだけ抜くと
6 / 40）。**「長い水平は LightGBM」より広く配れる**見通しで、判定は変わらず 10/09 以降。
...
**配信中の B3 には校正器を入れない**（2026-09-23、W5 の PR K。D-29。`docs/260923_eda_07_calibration.md`）。**ECE は 0.02 を超えていた**（祝日で最大 0.053、40 組中 23 組）が、**手順 3 に当たった**——歪みは**全部、難所のバケツ（0 台・1 台）にあり**、3 台以上のバケツは 0.0002〜0.0064 だった。**校正器は種別をまたぐと壊す**：平日で当てはめた isotonic は平日を直す（0.02 以上 6 → 0）が、**土日に当てると 3 → 11 組に増え、Brier は 40 組中 21 組で悪化**する。配信は全曜日種別に同じ成果物を出すので採れない。**直すのは v1 の特徴量**（§7.1 の追記、D-27）。**あわせて気候値の下限を 2 → 3 日に上げる**（D-30）——2 日の B2 は祝日の Brier を 7.59% 悪くしていた
...
>
> **⚠️⚠️ メモリは一度も検討していない**（2026-09-22 に気づいた。W5 プラン §12 の 176）。**`shadow` は 1 回の呼び出しで 2 つのモデルを読む。** 本番の推論はいま **P95 1.64 GB / 枠 2 GB**（余り約 360 MB）で、**その 96% はベースラインの成果物（28.5 MB の gzip JSON）を Python の辞書に開く 1 行**が使っている。**`shadow` に入る前に成果物の書式を直す**（鍵と率を生のバッファにする。見込み 1.58 → 0.5 GB 前後）。**これは所要の話ではなく、枠に入るかの話である。** **同じ期限で PR J2（推論がプロファイルを読む。山 ＋53 MB、下り月 24 GB）も入る**（D-31）——**176 を先に入れて余りを空けてから J2 を載せる**。
...
- **費用の抑え方**（超過が続く場合に順に適用）：(1) スキップ規則：`bikes ≥ 8 ∧ docks ≥ 8` かつ水平 ≤ 30 分のような「ほぼ確実」な組は B1 参照表で埋め、モデル呼出を 20〜30% 減らす（境界は評価で決める）。(2) 木の本数・葉数を減らす（300 × 63 で約 2 割減）。(3) 遠い水平（90〜180 分）は 10 分毎に更新。(4) **ML サービスだけ米国東部 `iad1` に置く**（単価 $0.128/$0.0106 で約 35% 減。Supabase 東京との往復は 1 サイクル数十回のクエリで数秒の追加。D-21）。
  **→ 2026-09-24 に 4 つとも測り直した**（`docs/260924_m4_cost_and_schedule.md` §1。D-32）。**(1) は効かない**：両方 8 以上かつ水平 ≤ 30 分は**全行の 0.81%** で、しかもそこは門を越えない水平なので**もともと B3 が配られる**（同 §3 の 181）。**代わりに、門を台数バケツ単位で当てれば「ほぼ確実」なセルは B3 に回り、森を歩かない**——規則を足さずに同じことが起きる。**(2) は本数のほうが効く**：学習率を上げて 100 本にすると、300 本の改善の 96% を 3 分の 1 の費用で保つ（葉 63 は 1 木あたり −22%）。**(4) は関数単位の `regions` として公式にある**が、Services（ベータ）の Python で効くかはプレビューで確かめる
...
3. 3 日間の実運用 Brier（§8.5）で `shadow` が `active` を下回らなければ `active` に昇格、旧版は `retired`。~~悪化時は自動ロールバック。~~ → **悪化時は人が前の版に戻す**（2026-09-24、D-36。自動ロールバックは CLAUDE.md §6 と W5 の契約 22 に反する。手順は `docs/260924_week6_model_v1_and_release_prep.md` §8.10）。**代わりを立てずに shadow を下ろす口（`retire_model_version`）を W6 で足す**
...
| 永続化 | ポート台帳（約 1 MB JSON）と直近の予測を App Group コンテナにキャッシュ（Widget から読める）。**お気に入りは JSON 1 ファイル**（SwiftData は使わない。20 件の配列に問い合わせも移行も要らない）。**置き場所は App Group を先に試し、無ければ `Documents`**——権限を宣言するのは W9（Widget）で、それまで実機の署名を壊さない。**最後に取れた応答は `Caches`**（作り直せる） |
...
| **行程チェック（Trip Check）** | 出発ポート（ポート詳細から）→ 目的地検索（`MKLocalSearchCompleter` → `MKLocalSearch`）→ 目的地最寄りポート候補 → 出発時刻（5／15／30／60／120 分後）→ **両方うまくいく可能性（主表示・注記つき）**、両端それぞれの確率と現在値、代替候補 | `/v1/trip-check`、MapKit ETA（`MKDirections.calculateETA(.cycling)`。**1 行程 1 回だけ**） |
| **お気に入り** | 登録ポートの現在値と「いつもの時刻」の確率（**曲線の点をそのまま読む**。契約 26）。**上限 20 件**・並べ替え・借りる／返すの切替。**オフラインは前の値を「○分前の観測」つきで**。**0 件のときの画面がある**。Widget（W9）の元 | `/v1/stations/{system}/{id}` を**件数ぶん**（束ねる口はまだ無い） |
...
- 行程チェックでは「借りる」「返す」の両確率を並べ、全体は「両方うまくいく確率 ≈ 積」を**画面のいちばん大きい字**で出す（W5 の PR G で実装、2026-09-13。**補助ではなく主表示にした**——利用者が知りたいのは片側ずつではなく行って返せるかである）。**独立仮定の注記を同じ欄に置く**（W4-24）。**出す条件は 3 つ**：端末が両端の確率を**両方出している**・サーバーが `trip` を返している・**注記が空でない**（契約 23）。1 つ目はサーバーの契約 10 より強く、**フィードが滞って片側を伏せたときに掛け算だけが残るのを防ぐ**。
...
- オフライン時は最後に取得した値を「○分前」の注記付きで表示（**お気に入りで実装、W5 の PR H**）。**「○分前」は取得の日付ではなく `observed_at` から作る**——値そのものが自分の古さを持つので、バックアップから戻した端末でも嘘にならない。**取れなかった行は消さない**（19 件取れていて 1 件落ちただけで何も出せない、を作らない）。
...
| 昇格 | candidate → shadow（3 日） → active（§8.4）。~~自動ロールバック~~ → **切り戻しは人が行う**（D-36） |
...
| W5 | **9/13〜9/26**（W4 が早く終わったぶん前倒し。実装プランは `docs/260912_week5_probability_and_screens.md`） | 予測ログ `forecast-log/`（PR A、**W6 から前倒し**）、`maxDuration` 240 秒 | ✅ 曜日種別の気候値（PR D）、✅ 校正の物差しを等頻度 15 区間に（W5-07）、✅ **抽出を一様 1% に**（PR I、D-25）、**PR K（校正の判断）は 9/23**（週末を含めて測れる初日） | ✅ 予測曲線・低ズームの格子集約（PR E）、✅ **日次評価 `/ml/evaluate`**（PR L、**W6 から 9 日前倒し**）→ **9/18 に GitHub Actions へ移設**（D-26） | ✅ Trip Check 画面（PR G）、✅ お気に入り（PR H） | **実運用 Brier 計測開始 ✅**（09-13〜09-17 の 5 日ぶん・1,680 行を 9/18 に記録） |
| W6 | 10/12–10/16 → **9/25–10/16**（前提づくりを前倒し。**実装プランは `docs/260924_week6_model_v1_and_release_prep.md`**） | ~~`forecast-log/` を実装する~~ → **✅ 9/13（PR A）**。~~`maxDuration` を 240 秒~~ → **✅ 9/13** | **LightGBM v1**（学習 4 週）→ ゲート → shadow。~~**未着手。§13.1 の 3 つを先に片付ける**~~ → **当てはめの側の 3 つ（166・167・168）は 9/20 に片付いた。配る側の前提が 6 つ残る**（§12 の 176・J2・shadow の経路・v1 を置くワークフロー・合成器、上限しだいで特徴量の経路 B と `iad1`）。**日程は W6 プラン §7**（shadow 10/09〜・active 10/13。~~M4 の文書 §2.3〜2.4~~ から移した） | ~~予測ログ・日次評価 `/ml/evaluate`~~ → **✅ 9/16（PR L）** | クレジット/免責画面、プライバシーマニフェスト、~~Developer Program 登録~~ → **✅ 登録済み（2026-09-18 確認）**。**加えて iOS のセル応答のバグ・予測時刻の表示の漏れ・暗号化の申告**（W6 プラン §6.9〜6.11）。アイコンと署名は利用者の作業 | **M4 LightGBM v1 active**（**費用の判断が先**。§13.1）→ **2026-09-24 に整理した**（D-32。`docs/260924_m4_cost_and_schedule.md` §1） |
...
### 13.1 W6 に入る前に片付けるもの（2026-09-18 の棚卸し）

**W6 の「データ / 基盤」列と「バックエンド / API」列はもう空である。** 残っているのは
**ML の中核（LightGBM v1）と iOS の 2 画面**だけで、うち v1 は**暦だけで止まっている
ように見えて、実際には 3 つの障害で止まっている**。

#### (a) いま何が v1 を塞いでいるか

| # | 中身 | 状態（2026-09-20） |
|---|---|---|
| **§12 の 168** | **28 日ぶんの当てはめが 25.1 GB**（実測の回帰。山 ＝ 2.37 GB ＋ 1.39 GB × 百万行） | **✅ 1 段目を 9/19 に実施**（23.7 → 14.2 GB）、**✅ 測る場所も 9/19**（`measure-fit.yml`）、**✅ 9/20 に Linux で実測**（4 水準）。**結果は「載るが足りない」**：8 日＝28 日ぶんで**常駐 14.67 GiB ＋ 退避 0.96 GiB**、機械は **15.61 GiB**（余り 1.4%）。**的（28 日・一様）は 15.96 GiB で 0.35 GiB 超過**、**PR J 後は 16.9〜17.6 GiB**。**→ 2 段目（日ごとに流す）を 9/20 に実装し、同じ日に Linux で検算した**：**実需要 15.39 → 常駐 11.30 GiB・退避 0**、余りは 1.4% → **28%**。**的は 11.44 GiB、PR J 後でも 12.07〜12.54 GiB で載る**。**これで 168 は閉じた** |
| **§12 の 166** | `fit_lightgbm` が `harness.run` に `climate` を渡さず、B2 が**学習サンプルから**作られる | **✅ 9/19 に直した。** 決める場所を `jobs/climate.py` の `source_for` 1 つにまとめ、`harness.run` の `climate` を必須引数にした。**道具が単独で出す数字が、手で差し替えた数字と完全に一致**（200 マスで差 0）。**判定は予告どおり反転**（学習 3 日で門を越えた水平 2〜3 → **0**） |
| **§12 の 167** | `MIN_CELL_SAMPLES = 2` が一様 1% では低すぎ、**セルが埋まるほど B2 が悪くなる** | **✅ 9/20 に直した。** 定数を 2 つに割り（`MIN_CELL_POINTS` ＝ 格子点 2 / `MIN_CELL_ROWS` ＝ **行 30**）、退避路の下限を**標準誤差から**決めた（n = 30 で 0.091）。**到達しうるセルを入れると必ず B3 が悪くなる**ことを掃引で確かめてある（下限 5 でも 0.04215 対 B1 0.04117）。いまの抽出では**実質すべて B1 に落ちる**——それが正しい状態である |

**どれも日付に縛られていなかった。** **3 つとも 9/19〜9/20 に片付いた**（166 → 168 の
1 段目 → 検算 → 2 段目 → 検算 → 167）。**当てはめの側で v1 を塞いでいるものは、もう無い**
——残るのは**暦**（学習窓が揃う 10/09）と、**M4 の費用判断**である。
**→ 2026-09-24：配る側に前提が 6 つ残っていた**（§12 の 176・J2・**shadow の経路**・
**v1 を置くワークフロー**・合成器・特徴量の経路 B。太字の 2 つはどこにも書かれていなかった）。
**`docs/260924_m4_cost_and_schedule.md` §2.3 に積み、M4 の費用判断も同 §1 で整理した**。

**ただし、配る側で 1 つ出た**（2026-09-22。PR J の下調べで見つけた）。

| # | 中身 | 状態 |
|---|---|---|
| **§12 の 176** | **配信の関数が 2 GB の枠の 82%（P95 1.64 GB）に達しており、その 96% は「28.5 MB の gzip JSON を Python の辞書に開く」ことが使っている** | **未着手。** `json.loads` の 1 行が ＋0.89 GB（鍵 1,200 万個・率 1,200 万個をいったん Python の `int`/`float` にする。numpy に移せば 105 MB）。**余りは約 360 MB**で、**ポートが 2 割強増えれば枠に当たる**。**直すのは「成果物を生のバッファで配る」**（見込み 0.2 GB 前後・所要 8.2 → 1 秒未満）。**PR J は避けて通れる**（並べ替え案の山は 53 MB）。**期限は W6 の `shadow`**——`shadow` は **1 回の呼び出しで 2 つのモデルを読む**ので、ベースライン単体で山の 96% を使っている状態では入らない（§7）。**PR K は強制しない**（校正器は節点が数千で桁が違う）が、**PR K が「入れる」なら書式を 2 度触らないために先にやる** |

#### (b) 暦で止まっているのは 2 つだけ

| 日 | やること | 何を待っているか |
|---|---|---|
| ~~9/23~~ | ~~PR K（校正の判断）~~ | **✅ 済：入れない**（D-29）。あわせて**気候値の下限を 3 日に**（D-30）。**v1 の採用基準に「バケツ 0・1 の ECE」と「曜日種別別の ECE」を足した**（§7.1） |
| **10/09 以降** | **LightGBM v1 の学習窓が揃う** | **学習 28 日 ＋ パージ 1 日 ＋ 検証 2 日 ＝ 31 日**。起点は **09-08**（09-07 は天気が 0% で `refuse_mixed_weather` が撥ねる）なので 31 日目は **10-08**、その日の学習サンプルが作られるのが **10-09 の 06:40 JST** である |

**PR J（9/24）はデータ待ちではない。** W5 プラン §6.0 が「9/19 と 9/24 の差は 5 日ぶん
＝ 15 分しかない」と書いているとおりで、9/24 なのは**推論がプロファイルを読むようになる
＝配信の形が変わるため連休に重ねない**（原則 5）という暦の都合である。
**実装と PR 作成は先に済ませておける。**

**→ 2026-09-23 に 2 つに分けた**（D-31、W5 プラン §6.10）。**推論がプロファイルを読む部分（J2）は、v1 を配るまで何も買わない**——配っている B3 は `prof_*` を 1 列も読まないのに、下り（月 24 GB）とメモリ（＋53 MB）だけが先に来る。**J1（学習側：`prof_*` 7 列・v4・09-07 以降の作り直し）は W5 のまま、10/07 より前に済ませる**（天気の 30 日保持）。**J2 は W6、v1 の `shadow` の前に §12 の 176 と一緒に入れる。**

#### (c) 日付で決まっているものの一覧

| 日 | やること | 性質 |
|---|---|---|
| 9/20（日） | `check_model_freshness` が初めて鳴る | **観測だけ。** 最終学習日 09-12 ＋ 8 日。鳴るのが正常 |
| ⭐ 9/21（月・祝） | 08:30 JST 以降に当てはめ直して昇格 | **✅ 09:38 JST に済んだ**（`baseline-b3-v0-20260920`）。**祝日に `confidence = 3` が 97.2%**（前日まで 0%）、`sat` と `sun_holiday` のセルが **0 → 198 万ずつ**。W5 プラン §8.3 |
| 9/22（火・休） | 予備 | 遅れを吸う。**当てはめ直さない**（9/21 の版のまま。同じ日に 2 度当てはめると成果物を上書きする） |
| ~~9/23（水・祝）~~ | ~~PR K（校正の判断）~~ | **✅ 済：入れない**（D-29）・**下限を 3 日に**（D-30）。**9/24（`profile(09-23)` の後）に当てはめ直して昇格すれば、9/26（土）は B1 に落ちる**（いまの成果物の `sat` は 2 日。#122・#123 は 9/23 にマージ済み。9/25 は予備） |
| ~~9/24（木）~~ | ~~⭐ 当てはめ直して昇格~~（下限 3。D-30）→ **PR J1 に着手** | **✅ 当てはめ直しは 09:48 JST に済んだ**（`baseline-b3-v0-20260923`。`sat` 0＝正しい・`sun_holiday` 5 日・20.2 MB。W5 プラン §8.3）。**評価で「3 日ぶんの B2 も祝日にはまだ害」と分かった**（落とすと −10.40%。害の約 8 割は「混合が見ていない B2」。W5 プラン §12 の 179）。**✅ J1 を実装し、13:58 JST にマージ、14:16 JST に 09-07〜09-23 の作り直しまで済んだ**（PR #126。17 / 17 日が合格。W5 プラン §6.10）。**PR J は 9/23 に J1・J2 に分けた**（D-31） |
| **9/25 以降** | **→ 9/25〜10/16 は W6 プラン（`docs/260924_week6_model_v1_and_release_prep.md`）§7、10/17 以降は `docs/260924_m4_cost_and_schedule.md` §2 に移した**（2026-09-24。M4 の費用判断と合わせて、申請（11/27）と再構築の期限（11/30）まで 1 か所にまとめた。**正を 2 つ作らない**ので、下の行は当時の記録として残す） | — |
| ~~9/25（金）~~ | ~~新しい版の平日の成績（日次評価）~~ | 移した（§2.2） |
| ~~9/26（土）~~ | ~~W5 完了時点の点検（§14 の 9 問）~~ | 移した（§2.2）。**当てはめ直さない** |
| ~~**9/28（月）**~~ | ~~9/27（日）の実測で下限の形を決め、所見 180 の直しと 1 つの PR に~~ | 移した（§2.2）。**当てはめ直しは 9/29〜9/30**（鮮度の警報 10/01 の前。§2.3 の 1） |
| ~~10/05（月）~~ | ~~W4-29（ビルド本数）・9 月の Vercel 使用量~~ | 移した（§2.3。**画面を見るのは利用者**） |
| ~~10/07（火）~~ → **10/07（水）** | ~~09-07〜09-11 の天気が `weather_hourly` の 30 日保持を抜ける~~ | **J1 の作り直し（9/24）で解消済み**。**曜日も違っていた**（10/07 は水曜） |
| ~~10/08（木）~~ | ~~「必要なら作り直せる」の実証（D-22 の前提）~~ | **✅ 9/12 に済んでいた**（W4 の PR L。生アーカイブから 3 発行を組み直した。§4.5b の最後の段落）。**この行が古かった** |
| ~~11/30（月）~~ | ~~再構築スクリプトの実証＋9 月パーティションの期限~~ | 移した（§2.5・§2.6） |

#### (d) 進める順番

1. **✅ §12 の 168（メモリ）の 1 段目**（9/19 完了）——**同じものを 2 つ持つのをやめ、
   float32 にした**。**23.7 → 14.2 GB**。float32 は**当てはめて確かめた**（LGBM の Brier の
   差 0.05%、門は 39 / 40 で同数、ベースラインは完全一致）。**大きいランナーは買わずに済んだ**
2. **✅ §12 の 166（気候値）**（9/19 完了）——決める場所を `jobs/climate.py` の
   `source_for` 1 つにまとめ、`harness.run` の `climate` を必須引数にした。
   **これで v0 の報告書の数字が読める**（本番と同じベースラインを相手にしている）
3. **✅ §12 の 168 の検算（9/20 完了）→ 2 段目をやる** ——`ubuntu-latest` で
   **学習 2・4・6・8 日の 4 本**を測った（run 35473387062。**何も書いていない**）。
   **8 日＝28 日ぶんは載ったが、1.9 分の時点から退避を使っている**——
   **常駐 14.67 GiB ＋ 退避 0.96 GiB、機械は 15.61 GiB で余りは 0.22 GiB（1.4%）**。
   退避ゼロの 3 点から引くと **山 ＝ 1.883 GiB ＋ 0.834 GiB × 百万行**で、
   **的（28 日・一様）は 15.96 GiB ＝ 0.35 GiB 超過**、**PR J（66〜69 列）で
   1.3〜1.9 GiB 超過**。**macOS からの外挿は 1.10〜1.20 倍低かった。**
   → **✅ 2 段目も 9/20 に実装し、同じ日に検算した**（`jobs/window.py`。`load_days` が
   全日を 1 つの表にして返すのをやめ、日ごとに読んで区間を埋める）。**答えは変わらない**
   （手元の A/B で報告書は生成時刻の 1 行を除いて完全一致、ベースラインの成果物は
   `created_at` を除いて完全一致）。**Linux の実測は 15.39 → 11.30 GiB・退避 0**、
   余りは 1.4% → **28%**。**的は 11.44 GiB、PR J 後でも 12.07〜12.54 GiB。**
   **168 は閉じた**（§12 の 168 の「2 段目を Linux で測った」）
4. **✅ §12 の 167（下限）**（9/20 完了）——プロファイルが無い日の退避路。**定数を
   2 つに割った**（`MIN_CELL_POINTS` ＝ 格子点 2 / `MIN_CELL_ROWS` ＝ **行 30**）。
   **30 は標準誤差から決めた**（n 行の率の標準誤差は `0.5/√n`。n = 30 で 0.091）。
   掃引で**到達しうるセルを入れると必ず B3 が悪くなる**ことを確かめてある
   ——**セルが 0 になって初めて B3 が B1 を下回る**。**本番の経路（プロファイル）は
   触っていない**（そちらは格子点を数えるので 2 のまま）
5. **M4 の費用判断** ——**v1 を作り込む前に決める**。**見積りは §4.5 に在り、2026-09-10 に
   本番の 1 周期を測って作ってある**：いま配っている B3 は 18 秒 CPU/周期で **$20**
   （クレジット内）だが、**LightGBM は numpy で 121 秒 → $71**、C 版で 88 秒 → $52、
   **木を半分にしてスキップ規則を入れても 63 秒 → $38**。**どれもクレジット $20 に
   収まらない**と §4.5 が既に書いている。

   **だから「作ってから決める」は順番が逆である。** 収まらないことは 9/12 に分かっていて、
   **どう進めるかも既に決めてある**：W4-20 が「**W6 まで numpy を維持する。判断は
   ① 特徴量の経路 → ② モデルの大きさ → ③ 評価器の順で行い、③ を先に決めない**」、
   W4-11 が「**『LightGBM を配る』と決めた瞬間に B（特徴量の経路、月 $8）は最優先になる**」。

   **残っているのは「v1 を active にするか」そのものである。** 決めた瞬間に ① が走り出す
   ので、**当てはめる前に置く**——決めないまま v1 を作ると、①〜③ を飛ばした成果物が
   できて作り直しになる。選択肢は**木を減らす／水平を絞る／`iad1` へ移す（D-21）／
   shadow までで止める**で、**どれを選ぶかで v1 の学習の目標が変わる**

   **→ 2026-09-24 に整理した**（`docs/260924_m4_cost_and_schedule.md` §1、D-32）。
   **止まっていた問い（木を減らすと何を失うか）は、手元の予備の当てはめで答えが出た**
   ——100 本（学習率 0.15）で 300 本の改善の 96%、50 本で 93%、門を越える組は同じ。
   **上限は「公開後の定常で $20 の内側」に決まった**（2026-09-24、利用者が選んだ）。
   `hnd1`＋特徴量の経路 B なら約 50 本、`iad1`＋B なら約 120 本を置ける。本数・配る単位・
   リージョンはそこから連動する（D-32）。**あわせて、v1 を配る前提が 6 つ残っていると分かった**
   （§12 の 176・J2・shadow の経路・v1 を置くワークフロー・合成器・特徴量の経路 B。
   同 §2.3）——**「当てはめの側で v1 を塞いでいるものは、もう無い」は当てはめの側に
   ついてだけ正しかった**（同 §3 の 184）
6. **9/20・9/21** ——鮮度の警報を観測し、当てはめ直して昇格
7. **9/23** ——PR K（校正）。**実運用の ECE が長い水平で 0.03 を超えている**という材料が
   既にある（`model_daily_metrics` の 1,680 行。h=180 で `hellocycling/dock` が 0.0319）
8. **9/24** ——~~PR J をマージ（実装は 3・4 と並行して先に済ませておける）~~ → **✅ 当てはめ直して昇格した**（09:48 JST。D-30）。**✅ PR J1 を実装・マージし、同日に作り直しまで済んだ**（`prof_*` 7 列・v4。W5 プラン §6.10 の J1）。**J2 は W6、v1 の `shadow` の前に §12 の 176 と一緒に**（D-31）
9. **9/28** ——9/27 の実測で**下限の形**を決め（所見 179）、**leave-one-out の直し**（所見 180）と 1 つの PR にしてから次の当てはめへ。**`sat` が 3 日になる当てはめはその後**

**この順にする理由は 1 つ。** v1 は「データが揃う 10/09」で待っているように見えるが、
**実際には 168 で止まっている**。データを待ってから着手すると、揃った日に「載らない」ことが
分かって W6 がそこでずれる。先に潰しておけば、**揃った瞬間に当てはめられる**。

**9/20 までに 1・2・3 の検算が済んだ。** 166 が片付いたことで**測る土俵が本番と揃い**、
以後の当てはめは報告書の数字をそのまま門の判定に使える。**そして検算は、待っていたら
W6 の初日に分かったはずのことを先に出した**——**外挿は 1.1〜1.2 倍低く、的は載らない。**
**残る障害は 168 の 2 段目と 167 の 2 つ**で、**どちらも日付に縛られていない。**

#### (e) この棚卸しで消えた宿題

| | |
|---|---|
| Apple Developer Program の登録 | **✅ 登録済み**（2026-09-18 確認）。M5 TestFlight（W7、10/19〜）を縛る最長の外部要因だったが、解消した |
| Supabase のシークレットキーのローテーション | **対応不要と判断**（2026-09-18） |
...
| R4 | Vercel 使用量が $20 クレジットを超過（東京単価は米国東部の約 1.6 倍。他プロジェクトとクレジットを共用） | 中 | 低〜中 | Spend Management アラート、スキップ規則・木数削減・遠い水平の間引き、ML サービスの `iad1` 配置（D-21）。超過は月数ドル規模。**→ 2026-09-24 に測り直した**（`docs/260924_m4_cost_and_schedule.md` §1、D-32）：**B3 だけで開発中 $18.9／公開後 $16.8**、**v1 を配ると形しだいで $15〜$78**（本数・配る単位・リージョン）。**Spend Management の「Pause Production Deployments」は使わない**——チームの全プロジェクトの本番デプロイを止めるので、**収集器ごと止まる**（同 §3 の 185）。予算額は通知（50・75・100%）のためだけに置く |
...
| D-22 | 学習サンプル（`features/`）の保持 | **当面ためる**（~~13~~ → **5.4 GB/年**。§4.5b。2026-09-16 に一様抽出へ変えて 3 分の 1 になった。D-25）。**捨てる前提が揃っていない**——「必要なら作り直せる」の確認が PR L（期限 2026-10-08）で、**確かめていないものを前提に捨てる決定はできない**。急ぐ理由も無い（100 GB まで約 2.3 年、先に効くのは `gbfs-raw` の 24 GB/年）。**貯めておけばモデルを作った正確なバイト列が残る** | N か月で捨てて必要なら作り直す | **決定（2026-09-11）**。見直す合図は Storage 合計 **60 GB**、または W6 の週次再学習を組むとき。**2026-09-24 追記**：「必要なら作り直せる」の確認（PR L）は **9/12 に済んでいた**（§4.5b の最後の段落）。§13.1 (c) の 10/08 の行と、この行の「期限 2026-10-08」は古かった |
...
| D-25 | 学習サンプルの抽出（層化するか） | **一様 1%**（`SAMPLE_RATE`）。**層化はやめる。** 母集団の **84.3%** が `bikes ≤ 2 または docks ≤ 2` で**難所は稀ではなく**、層 4%・他 1% が同じ行数の一様抽出より多く取れる難所は **+13.3%** にすぎない（`4 / (1 + 3p)`）。`weight` と `stratum` は**列として残す**——2026-09-15 以前の日は層化してあり、混ぜて読むときに行ごとの重みが要る | 閾値を下げる（`≤ 1` で 69.2%・`≤ 0` で 43.3%）／`fill_ratio` の両端（39.4%）で層化／いまのまま | **決定（2026-09-16）**。W5 の PR I。**9 日・5.29 億行の推定で測ってから決めた。**行数 207 万 → **59 万/日**、容量 40.9 → **14.9 MB/日**（5.4 GB/年）。**学習を 3.5 分の 1 にしても、ベースラインのバケツ別 Brier は最大 0.72% しか動かなかった**（難所は ±0.4% 以内。B1 の 120 セルは全部埋まる）。~~**LightGBM は W6 の v1 で測り直す**~~——木は行数を使うので、ベースラインの結果では代われない。→ **2026-09-17 に測った（`docs/260917_eda_05_lightgbm_remeasure.md` §8）。判断は変わらない。** 同じ日数で層化が買えるのは **+0.3〜0.7 ポイント**だけで行は 3.5 倍要り、**同じ容量に直すと逆転する**（一様 5 日 74.5 MB が層化 2 日 81.8 MB に全水平で勝った）。**気候値も速く埋まらない**——`MIN_CELL_DAYS = 2` は日数で縛るので、行を 3.5 倍にしても 1 日は 1 日である。**2026-09-24 追記（W5 の PR J1）**：J1 の作り直しで全日が一様になり、`weight` は定数になった——**それでも列は残す**。評価も当てはめも重み付きで書いてあり（定数なら重み無しと一致する）、**抽出率を上げる・層化に戻すのは定数 1 行**なので、列を落とすとそのとき**版をもう 1 つ上げる**ことになる |
| D-26 | 日次評価（`/ml/evaluate`）を走らせる場所 | **GitHub Actions**（`evaluate-daily.yml`、06:40 JST）。**Vercel の枠に入らない**——2026-09-16 を実測して**メモリの山 2.26 GB**（枠は 300 秒・2 GB）。並列でも直列でも 0.5% しか変わらず、重いのは往復ではなく**持ち方**である。W4-27 が `build_features`（1.28 GB）について決めた「5 分毎の推論と同じサービスに重いバッチを混ぜない」と同じ規律で、**こちらは収まってすらいなかった** | Vercel のまま持ち方を削る（`pa.concat_tables` の二重取りを避ける等）／関数のメモリを上げる（**枠が 2 GB なので上げ代が無い**） | **決定（2026-09-18）**。W5 の PR L の直し。**本番で 2 回続けて死んでから決めた**——09-17・09-18 の 04:40 がどちらも `FUNCTION_INVOCATION_FAILED` で、`model_daily_metrics` は 0 行のままだった（W5 プラン §12 の 171）。**持ち方を削るのは別の仕事として残す**：16 GB のランナーなら当面通るが、**日数が増えれば同じ壁に当たる** |
| D-27 | `prof_n_days` を特徴量に入れるか | **入れる**（`prof_*` は **7 列**、`FEATURE_SET` v4 で全 **68 列**）。**同じ `prof_p_bike = 1.0` でも「2 日から」と「28 日から」では意味が違う。** `n_days` が無ければ木はそれを区別できず、**薄いセルの率を濃いセルの率と同じ重さで使う**ことになる。材料は既に `confidence` のために持っている（W5-06）ので、**足すのは 1 列ぶんだけ**である | 入れない（§6.3 が当初書いていた 6 列のまま。全 67 列）／下限を満たすセルだけ `prof_*` を出して `n_days` は捨てる | **決定（2026-09-21）**。W5 プラン §6.10。**実測が決め手になった**——2026-09-21 に配った成果物では、`sat` と `sun_holiday` のセルは `MIN_CELL_DAYS` ちょうどの **2 日**しか無く、**B2 が取りうる値は 13 個**（`k/n`、`n ≤ 6` の既約分数）しかない。同じ表の `weekday` は **230 個**である（W5 プラン §12 の 174）。**「どれだけの日から出た率か」は、率そのものと同じだけ情報がある**。**2026-09-24 に実装した**（`FEATURE_SET` v4。W5 の PR J1）——`prof_n_days` は **NULL にしない**（0 ＝ 履歴が無い） |
| D-28 | `profiles/` の保持 | **`daily.parquet` は無期限、`profile.parquet` は 35 日**（28 ＋ 余白）。**源は `daily`、`profile` は派生**である——`profile(D)` は `daily(D-27 … D)` の和で、**`profile.sum_dailies` が既に実装され、`roll` と一致することを検査が保証している**（`tests/test_features_profile.py`）。足りないのは Storage から 28 個読んで渡す入口だけ。`daily` は 1.4〜2.3 MB/日（年 0.73 GB）、`profile` は W5 の PR J で並びを変えると 44 MB/日になるので、**35 日なら 1.5 GB 定常・無期限なら年 16 GB** | 両方とも無期限（`gbfs-parquet` の方針どおり）／`profiles/` を別バケットに分けて保持を切る | **決定（2026-09-22）**。W5 プラン §6.10 の「先に測った」⑥。**ただし順番がある**——**D-22 と同じ作法で、「作り直せる」を確かめるまで掃除を入れない**：①いまは入れない ②PR J の作り直しのついでに 1 日ぶんを `sum_dailies` 経由で作り直してバイト一致を確かめる ③確かめてから 35 日で掃除する。**バケットは増やさない**——`forecast-log` が分かれたのは「寿命も作り直し方も違うものを混ぜない」ためだが（W3 §12 の 106）、`profiles/` は接頭辞で分かれており掃除も接頭辞単位でできる。RLS・掃除・監視を 1 セット増やす価値はいま無い。**見直しの合図は D-22 と同じ「Storage 合計 60 GB」**。**2026-09-24 追記（W5 の PR J1）**：**②を済ませた**——`build_profiles --from-dailies --compare` が**本番の 3 日（09-07・09-13・09-23。`daily` 1・7・17 日ぶん）でバイト一致**した。**③（35 日の掃除）は Storage の削除なので、確認してから別の PR で入れる** |
| D-29 | 配信中の B3 に校正器を入れるか | **入れない**。`confidence` の鮮度と履歴も**分けない** | 水平グループ別（≤30／≤90／≤180 分）の isotonic を成果物に同梱（§7.4 の手順 2）／バケツ別の isotonic・ロジスティック | **決定（2026-09-23）**。W5 の PR K。`docs/260923_eda_07_calibration.md`。**本番で配った全数（9 日・約 5.4 億行 × 2 ターゲット）で測った。** ECE は 0.02 を超えていた（祝日で最大 0.053）が、**歪みは難所のバケツ（0・1 台）に集中**し、**校正器は種別をまたぐと壊す**（平日→平日は 0.02 以上 6 → 0、**平日→土日は 3 → 11・Brier は 21/40 組で悪化**）。§7.4 の手順 3「特徴量に戻す」に当たる。**原因は B3 の B2 の重みがバケツも日数も知らないこと**（W5 プラン §12 の 178）で、**v1 の木が掛け合いで持つ**。`confidence` は、**鮮度切れ（1）が 2 日とも 0 件**で、直すべきは「3 と名乗る下限」だった（D-30 で直る）。**v1 では「履歴」の定義を `prof_n_days` から作り直す**（LightGBM では「本来の情報で出せたか」が常に真になるため）|
| D-30 | 気候値（B2）の下限の日数（`MIN_CELL_DAYS`） | **2 → 3 日** | 2 日のまま／5 日（測って効いていた最小） | **決定（2026-09-23）**。W5 の PR K と一緒に決めると W5 プラン §6.4 が予告していた。**反実仮想を全数で出した**——B2 が引けないセルの B3 は `(system, 水平, バケツ)` だけで決まるので、**配った確率を B2 を落とした値に置き換えて同じ実測で測り直せる**。**祝日（B2 2 日）は落とすと Brier −7.59%**・ECE 0.02 以上 19 → 3 組（09-21 は −12.1%、09-22 は −6.0%。両系統とも同じ向き）、**平日（B2 5 日）は落とすと +1.22%**（悪化）。**境目は 2 日と 5 日のあいだ**で、3・4 日は未測定。**3 にするのは、測って害だった 2 だけを外す最小の一歩**だからで、**次の当てはめ（9/24。遅くとも 9/25）には 3・4 日のセルが生まれない**（`sun_holiday` 5 日・`sat` 2 日）——「3 で足りるか」は `sat` が 3 日になる当てはめまでに測る。**同じ `2` が `baselines/climatology.py` と `features/profile.py` の 2 か所に在る**ので、1 つにまとめてから変える。**2026-09-24 追記（EDA #8、W5 プラン §12 の 179・180）**：**3 日も、祝日にはまだ害だった**（B2 を落とすと −10.40%。2 日のときは −7.59%）。**害の約 8 割は日数ではなく「下限ちょうどの曜日種別は、混合が学習で一度も見ていない」ことから来ていた**（leave-one-out が 1 日引くので下限を割る）——学習側の下限だけ 2 にすると B1 との差 +11.98% → +2.57%。**次の当てはめで `sat` が 3 日になる前に、下限の形**（配るセルを混合が見たセルに揃える／下限を上げる）**を 9/27 の実測で決める**（9/28。未決）→ **決め方は D-37**（2026-09-25） |
| D-31 | PR J（`prof_*`・`FEATURE_SET` v4）を 1 本で入れるか | **2 つに分ける**。**J1（学習側）**：`build_day` に `prof_*` 7 列（全 68 列）、`build_now` も 68 列を出す（**プロファイルを渡さなければ `prof_*` は NULL・`prof_n_days` は 0**）、ゴールデン、`sum_dailies` の入口と実証（D-28）、**09-07 以降の作り直し**——**W5、10/07 より前**。**J2（配信側）**：推論が前日のプロファイルを読む（契約 28 の並びで要る行群だけ）、`build_profiles` の並び替えと検査 3 本、`profile_load_ms`・`model_load_ms`・`rss_mb`——**W6、v1 の `shadow` の前に §12 の 176 と一緒に** | 1 本のまま 9/24 に入れる（W5 プラン §6.10 の当初の形） | **決定（2026-09-23）**。W5 プラン §6.10。**推論がプロファイルを読む部分は、v1 を配るまで何も買わない**——配っている B3 は `prof_*` を 1 列も読まず、読んでも確率は変わらない。それでも読み始めれば**下り月 24 GB（込みの約 1 割）とメモリの山 ＋53 MB（余り 360 MB）**を先に払う（W5 プラン §12 の 175・176）。**学習側には期限がある**（`weather_hourly` の 30 日保持が 10/07、v1 の学習窓が 10/09）。**並び替え（契約 28）は J2 に置く**——部分読みは J2 からで、J1 で並べ替えると恩恵の無いまま置き場が 3 倍になる。**あいだの約束（契約 29）**：J1 と J2 のあいだ、配信の `feature_set` は v4 だが `prof_*` は NULL——**`prof_*` を使うモデルは J2 まで `shadow` にも `active` にもしない**（版は列が在ることしか語らない。§6.3） |
| D-32 | M4（LightGBM v1 を active にする）の形と費用の上限 | **(a) 上限：公開後の定常で Vercel の使用量 $20 の内側**（開発中のビルドぶんは文書だけのビルドを飛ばして吸う）。**(b) 配る単位：§7.1 のとおりセル単位**（system × ターゲット × 水平 × 台数バケツ）。門の表は当てはめのときに決めて**成果物に焼き付ける**。**(c) 本数は上限から逆算**（`hnd1`＋特徴量の経路 B で約 50 本、`iad1`＋B で約 120 本）し、**学習率を本数に合わせて上げる**（50 本で 0.15〜0.20）。10/09 に上限の本数と 300 本を並べて当てはめ、**配る形の改善の差が 1 ポイント以内**なら上限の本数で配る。超えたら利用者に諮る。**(d) 評価器は numpy のまま**（W4-20 の戻す条件に当たらない）。**(e) まず `hnd1`**。`iad1` はプレビューで確かめておき、(c) で 1 ポイントを超えて失うとき、または W9 の `/ml/profiles` で余りが足りなくなったときに移す（D-21 の条件を具体にした） | ② 月 $10 の超過まで許す（`hnd1` のまま 100 本、使用量 $26〜29）／③ 設計どおり 300 本（$36〜54）／④ shadow までで止め、B3 のまま申請する | **決定（2026-09-24。(a) は利用者が ① を選んだ）**。`docs/260924_m4_cost_and_schedule.md` §1。**根拠は予備の当てはめ**（学習 8 日・一様 1%・v4、検証は平日 2 日）：**100 本（学習率 0.15）で 300 本の改善の 96%、50 本で 93%、門を越える組は同じ 31 / 40**。学習率を上げずに減らすと ECE が 0.03 を割る。**`prof_*` を抜くと 18 / 40、配るときだけ抜くと 6 / 40**（J2 と契約 29 の門が前提）。**スキップ規則は全行の 0.81% しか当たらず効かない**（§8.2 の (1)）。**門の粒度は CLAUDE.md（水平）と §7.1（水平, バケツ）で違い、同じ費用なら水平単位のほうが改善が大きい**——変えずに 10/09 の本番の窓で比べて諮る |
| D-33 | v1 をどの形で配るか | **合成器（`model_versions.kind = composite`）**。セル（system × ターゲット × 水平 × 台数バケツ）ごとに、**門を越えたセルは LightGBM、他は B3**。**成果物は自己完結**（B3 の中身・LightGBM の森・門の表を 1 つの gzip JSON）。**門の表は当てはめのときに決めて焼き付ける**（コードに書かない）。**`train_days` は 2 つの和**（B3 の週次で鮮度を見張る）。**LightGBM 単体は active にしない** | LightGBM と B3 を別々に登録して参照でつなぐ（2 つの登録を同時に正しく保つ必要があり、配ったバイト列が 1 つに定まらない）／水平単位で切り替える（10/09 の本番の窓で比べてから諮る） | **決定（2026-09-24）**。W6 プラン §4 の W6-08・09・11・12、PR H。0038 の `kind` の制約を広げる移行が要る。**`train_days` を LightGBM の日だけにすると、10/13 に入れた瞬間に鮮度の警報が鳴る**（学習の最終日 10-05 から 8 日。W6 プラン §12 の 199） |
| D-34 | ベースラインの成果物の書式 | **版 2：gzip JSON のまま、B2 の「使えるか」（1 セル 1 ビット）と率（6 桁に丸めて ×10^6 した整数・`<u4`）を base64 の生バッファで持つ。読み手は版 1 と 2 の両方** | Parquet（MIME の追加が要り、小さい欄を JSON のまま読めない）／版 1 のまま／**B2 の鍵を昇順の差分・`<u4` で持つ**（9/24 に決めた形。ほぼ埋まった成果物で開く山が 0.76 GB。W6 プランの所見 200） | **決定（2026-09-24）、直した（2026-09-25）**。W6 プラン §4 の W6-01、PR B。**配信中の成果物から作って測った**：20.2 → 2.4 MB、開く山 1.23 → 0.52 GB、6.3 → 1.2 秒、**中身は完全一致**（W6 プラン §13.2）。**9/25 の実装で鍵をビット列に替えた**：同じ成果物で 2.26 MB・0.35 GB、ほぼ埋まった成果物で 2.46 MB・0.39 GB、どちらも版 1 とビット一致。**率の整数は `round(x, 6)` を通してから作る**（W6 プランの所見 201）。W5 プラン §12 の 176 の答え |
| D-35 | 推論が読むプロファイルの版 | **前日の版を読み、まだ無ければ 1 つ古い版（最大 2 日前）。読んだ版の日付を `profile_date` に必ず残す**。**Range 要求で要る行群だけを取る**。プロファイルは **00:40 JST の枠でも作る**。学習は必ず前日の版（W5 の契約 12 を W6 の契約 32 で置き換える） | 前日の版が無ければ `prof_*` を NULL で配る（毎朝、合成器が B3 に落ちる）／プロファイルを丸ごと落とす（1 周期 15〜44 MB） | **決定（2026-09-24）**。W6 プラン §4 の W6-03・04、PR C・D。**本番の Storage が Range に応える**ことを 9/24 に確かめた（206、末尾の範囲も可）。朝の穴は参照スナップショット（W4 プラン §12 の 118）と同じ手当て |
| D-36 | 悪化したときの切り戻し | **人が前の版を `promote_model_version` で戻す。自動ロールバックは作らない。** 代わりを立てずに shadow を下ろす口 **`retire_model_version`**（active は拒む・誰にも grant しない）を足す | §8.4・§11.1 の「悪化時は自動ロールバック」 | **決定（2026-09-24）**。W6 プラン §4 の W6-07・24、PR E。**自動ロールバックは CLAUDE.md §6（active の切替・ロールバックは確認してから）と W5 の契約 22 に反する**（W6 プラン §12 の 192） |
| D-37 | B3 の気候値の下限の形（D-30 の続き） | **学習側の下限は、どの曜日種別も配る側より 1 日低く**（配るセル ＝ 混合が学習で見たセル。**プロファイルから作るとき**——学習サンプルから作る退避路は、引くのが行 1 つで日数が減らないので下げない）。**自分のぶんは到着日で引く**。**平日の配る側は 3**。**土日祝の配る側は 3・4・5・配らない のどれか**を、9/27 の同じ行で 3・4・5 日の版を並べて測り、W6 プラン §6.1 の基準で選ぶ（全体で「B2 を落とした確率」より当たり、ポート単位のブートストラップの 95% 区間の下限も正、4 組のうち 3 組以上。k 以上がすべて効く最小の k）。**後から効かないと分かったら上げる**（1 日の行では下げない） | 9/27 に 5 日ぶんだけを測り、効けば「学習側 − 1」だけ（配る側は 3 のまま）／土日祝の下限を先に上げる／曜日種別をまたいで補う（W6-27 で B3 には入れない） | **決定（2026-09-25）。値は 9/28 に決まる**（測れなければ「配らない」）。W6 プラン §4 の W6-34、PR A。**見せても 3 日の祝日の B2 は、B2 を落とした確率より悪かった**（EDA #8 §0.4 C：B1 との差 +1.75% 対 +0.33%、4 組中 3 組）。9/27 に 5 日ぶんだけを測っても、9/29 の当てはめから配られる 3 日の土曜がよいかは分からない（W6 プランの所見 202）。**1 日の行でも差は測れる**（09-24 の平日で「落とすと」+2.38%、95% 区間の半幅 0.19%。W6 プラン §13.7） |
