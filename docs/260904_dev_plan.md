...
- **28 日ぶんの当てはめは 25.1 GB 要る。** 12 本の実測を回帰すると
  **山 ＝ 2.37 GB ＋ 1.39 GB × 百万行**（残差は最大 0.25 GB）で、28 日 ＝ 16.4M 行。
...
| # | 中身 | 状態（2026-09-19） |
|---|---|---|
| **§12 の 168** | **28 日ぶんの当てはめが 25.1 GB**（実測の回帰。山 ＝ 2.37 GB ＋ 1.39 GB × 百万行） | **✅ 1 段目を 9/19 に実施。23.7 → 14.2 GB**（同じものを 2 つ持つのをやめ、float32 にした）。`ubuntu-latest`（16 GB）に**載るが余裕は 1.8 GB**。**2 段目（日ごとに流す、見込み約 9 GB）は未着手**——先に **Linux で実測して 14.2 GB を検算する**（いまの値は手元 macOS 8 GB の 2 点からの外挿） |
...
3. **§12 の 168 の検算 → 2 段目** ——**14.2 GB は手元 macOS（8 GB、スワップしながら）の
   ...
   **1.8 GB しかない**ので、**まず実測で検算する**。足りなければ 2 段目
   （`load_days` を日ごとに流して「読んだ表」4.81 GB を外す。見込み約 9 GB）
