...
| W6 | 10/12–10/16 | ~~`forecast-log/` を実装する~~ → **✅ 9/13（PR A）**。~~`maxDuration` を 240 秒~~ → **✅ 9/13** | **LightGBM v1**（学習 4 週）→ ゲート → shadow。~~**未着手。§13.1 の 3 つを先に片付ける**~~ → **当てはめの側の 3 つ（166・167・168）は 9/20 に片付いた。配る側の前提が 6 つ残る**（§12 の 176・J2・shadow の経路・v1 を置くワークフロー・合成器、上限しだいで特徴量の経路 B と `iad1`）。**日程は `docs/260924_m4_cost_and_schedule.md` §2.3〜2.4**（shadow 10/09〜・active 10/13） | ~~予測ログ・日次評価 `/ml/evaluate`~~ → **✅ 9/16（PR L）** | クレジット/免責画面、プライバシーマニフェスト、~~Developer Program 登録~~ → **✅ 登録済み（2026-09-18 確認）** | **M4 LightGBM v1 active**（**費用の判断が先**。§13.1）→ **2026-09-24 に整理した**（D-32。`docs/260924_m4_cost_and_schedule.md` §1） |
...
| **9/25 以降** | **→ `docs/260924_m4_cost_and_schedule.md` §2 に移した**（2026-09-24。M4 の費用判断と合わせて、申請（11/27）と再構築の期限（11/30）まで 1 か所にまとめた。**正を 2 つ作らない**ので、下の行は当時の記録として残す） | — |
