...
| ~~9/24（木）~~ | ~~⭐ 当てはめ直して昇格~~（下限 3。D-30）→ **PR J1 に着手** | **✅ 当てはめ直しは 09:48 JST に済んだ**（`baseline-b3-v0-20260923`。`sat` 0＝正しい・`sun_holiday` 5 日・20.2 MB。W5 プラン §8.3）。**評価で「3 日ぶんの B2 も祝日にはまだ害」と分かった**（落とすと −10.40%。害の約 8 割は「混合が見ていない B2」。W5 プラン §12 の 179）。**✅ J1 を実装した**（`prof_*` 7 列・v4。W5 プラン §6.10 の J1 の実装）——**作り直し（09-07〜前日）はマージ当日**（予行演習は 17 / 17 日が合格）。**PR J は 9/23 に J1・J2 に分けた**（D-31） |
...
8. **9/24** ——~~PR J をマージ（実装は 3・4 と並行して先に済ませておける）~~ → **✅ 当てはめ直して昇格した**（09:48 JST。D-30）。**✅ PR J1 を実装した**（`prof_*` 7 列・v4。**作り直しはマージ当日**。W5 プラン §6.10 の J1 の実装）。**J2 は W6、v1 の `shadow` の前に §12 の 176 と一緒に**（D-31）
