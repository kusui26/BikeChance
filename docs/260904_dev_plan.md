...
| **GitHub Actions** | **公開リポは無制限・無料**（私有なら 2,000 分/月。D-12）。スケジュール最短 5 分、**遅延あり**。ジョブ最長 6 時間 | CI・**学習サンプルの日次生成**（W4 の PR J）・週次再学習・週次 Drive ミラー。**秘密を使うワークフローは `schedule` / `workflow_dispatch` だけで起動する** |
