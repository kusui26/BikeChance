...
- **`fit_lightgbm` の B2・B3 は本番より弱い。** `harness.run` に `climate` を渡していないので
  気候値が**学習サンプルから**作られる。本番と同じプロファイルの気候値に直すと、
  学習 5 日の h=180 の改善は **+13.8% → +11.2%**、学習 3 日では門を越えた水平が **2 → 0** に
  反転する。**「同じ関数を 2 か所から呼ぶなら既定値で分岐させない」**（§12 の 166）
...
| # | 中身 | なぜ先に片付けるか |
|---|---|---|
| **§12 の 168** | **28 日ぶんの当てはめが 25.1 GB**（実測の回帰。山 ＝ 2.37 GB ＋ 1.39 GB × 百万行） | **物理的に載る場所が無い。** 手元は 8 GB、`ubuntu-latest` は 16 GB。**データが揃う日を待ってから気づくと W6 が丸ごとずれる** |
| **§12 の 166** | `fit_lightgbm` が `harness.run` に `climate` を渡さず、B2 が**学習サンプルから**作られる | **門の判定が狂う。** 本番と同じプロファイルの気候値に直すと、学習 5 日の h=180 の改善は **+13.8% → +11.2%** に縮む。直すまで v0 の報告書の数字は使えない |
| **§12 の 167** | `MIN_CELL_SAMPLES = 2` が一様 1% では低すぎ、**セルが埋まるほど B2 が悪くなる** | 166 を直せば影響は小さくなるが、プロファイルが無いときの退避路が壊れたままになる |

**どれも日付に縛られていない。** いま着手できる。
...
1. **§12 の 168（メモリ）** ——「28 日ぶんが載る場所」を作る。`models/matrix.py` を
   float32 にする案は**精度への影響を測ってから**決める。照合の写しを持たない案、
   大きいランナーを使う案（費用が増えるので確認が要る）と並べて選ぶ
2. **§12 の 166** ——`fit_lightgbm` にも `evaluate_baselines` と同じ気候値を通す。小さく、
   v1 の門を正しくする。**166 を直さないと 168 を直しても数字が読めない**
3. **M4 の費用判断** ——**v1 を作り込む前に決める**。**見積りは §4.5 に在り、2026-09-10 に
...
4. **9/20・9/21** ——鮮度の警報を観測し、当てはめ直して昇格
5. **9/23** ——PR K（校正）。**実運用の ECE が長い水平で 0.03 を超えている**という材料が
   既にある（`model_daily_metrics` の 1,680 行。h=180 で `hellocycling/dock` が 0.0319）
6. **9/24** ——PR J をマージ（実装は 1・2 と並行して先に済ませておく）
