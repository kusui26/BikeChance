...
| W3 | **9/9〜**（W2 が 4 日早く終わったぶん前倒し可） | 祝日・行政区域（`pref_code` / `muni_code`）の取り込み、近傍リスト、プロファイル日次ジョブ、**バックアップ収集器（Edge Function）**、**① `job_runs` の失敗と Parquet の欠落を監視に足す（R19）**、**② Parquet に `fetched_at` を足す（§6.2 のリーク防止）** | ベースライン B0–B3、特徴量パイプライン v0＋テスト | `/ml/infer` を B3 で稼働（予測テーブルを埋め始める）、ウォッチドッグ | ポート詳細画面（現在値・履歴） | — |
