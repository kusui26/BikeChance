...
| W3 | **9/9〜**（W2 が 4 日早く終わったぶん前倒し可） | 祝日・行政区域（`pref_code` / `muni_code`）の取り込み、近傍リスト、プロファイル日次ジョブ、**バックアップ収集器（Edge Function）**、**① `job_runs` の失敗と Parquet の欠落を監視に足す（R19。最優先）**、**② Parquet に `fetched_at` を足す＋既存を再圧縮（§6.2。費用が時間とともに増える）**、③ 天気ファイルの入手時刻の規約、④ `job_runs` の保持。**順番と理由は W2 プラン §14** | ベースライン B0–B3、特徴量パイプライン v0＋テスト | `/ml/infer` を B3 で稼働（予測テーブルを埋め始める）、ウォッチドッグ | ポート詳細画面（現在値・履歴） | — |
