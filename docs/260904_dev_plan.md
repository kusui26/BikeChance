...
| # | 中身 | 状態 |
|---|---|---|
| **§12 の 176** | **配信の関数が 2 GB の枠の 82%（P95 1.64 GB）に達しており、その 96% は「28.5 MB の gzip JSON を Python の辞書に開く」ことが使っている** | **未着手。** `json.loads` の 1 行が ＋0.89 GB（鍵 1,200 万個・率 1,200 万個をいったん Python の `int`/`float` にする。numpy に移せば 105 MB）。**余りは約 360 MB**で、**ポートが 2 割強増えれば枠に当たる**。**直すのは「成果物を生のバッファで配る」**（見込み 0.2 GB 前後・所要 8.2 → 1 秒未満）。**PR J は避けて通れる**（並べ替え案の山は 53 MB）が、**PR K（校正器を同梱）と W6（LightGBM を配信）の前には要る** |
