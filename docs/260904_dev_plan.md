...
| Storage `forecast-log`（**12 か月保持**。W6 から） | **37.4 MB** | **定常 13.7 GB** | — |
| Storage 合計（W6 以降） | — | **約 40 GB/年 ＋ 定常 13.7 GB** | 100 GB（**約 2.2 年**） |
...
| **予測ログ `forecast-log/`**（旧手順 7） | **W6 の最初** | **`shadow` の前提**。配らずに記録して後から測る運用なので、記録の置き場が無ければ始められない（W3-18 で保留したまま）。**設計は 2026-09-12 に済んだ**（W4 プラン §6.8 の PR N）——下の段 |
...
> **⚠️ `shadow` は `maxDuration` 120 秒に収まらない。** 特徴量の行列は 1 回で済むので**増えるのは木を歩くぶんだけ**だが、LightGBM を `active` にしたうえで別の LightGBM を `shadow` にすると **HELLO は 77.7 → 約 144 秒**になる。**W6 の最初の PR で 240 秒に上げる**（Pro の上限は 300 秒）。3 日ぶんの Active CPU の増分は**約 $6**。
...
| **R21** | ~~**予測ログ（`forecast-log/`）が無いまま W6 の `shadow` に入る**~~ | **設計は解消（2026-09-12）** | **高** | `shadow` は「配らずに記録して後から測る」運用なので、**記録の置き場が無ければ始められない**。W3-18 で保留し W4 でも作らなかった。**設計は 2026-09-12 に済んだ**（W4 の PR N。バケット・形・量・誰が書くかを実測で決めた）。**残るのは実装**——W6 の最初の PR で、あわせて `maxDuration` を 240 秒に上げる |
...
| D-24 | 予測ログ（`forecast-log/`）の置き場と形 | **新しいバケット**に **Parquet**（1 ポート 1 行・確率は `list<int16>`・zstd・UTC の `date=`/`hour=`）。定数は**列**に持ち、**`active` と `shadow` の両方**が書く。**12 か月保持**（§8.2） | `gbfs-parquet` に相乗り／JSON 列ごと gz（**28% 小さいが読みが 34 倍遅い**） | **決定（2026-09-12）**。W4 の PR N。本番の予測で測ってから決めた。実装は W6 の最初 |
