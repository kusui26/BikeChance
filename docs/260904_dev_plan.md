...
- **28 日ぶんの当てはめは 25.1 GB 要る。** 12 本の実測を回帰すると
  ...
  **Linux の検算はマージのあと**——`training` 環境は `main` だけを許可する
...
**7〜8 / 10 水平**で、**h = 5 分と 10 分は届かない**。**「長い水平は LightGBM、短い水平は
...
**どれも日付に縛られていない。** **166 は完了、168 は 1 段目と検算まで完了**
（9/20 の実測で「**2 段目はやる**」が確定した）。**残るのは 168 の 2 段目と、167** である。
...
3. **✅ §12 の 168 の検算（9/20 完了）→ 2 段目をやる** ——`ubuntu-latest` で
   ...
   **的（28 日・一様）は 15.96 GiB ＝ 0.35 GiB 超過**、**PR J（66〜69 列）で
   ...
   → **✅ 2 段目も 9/20 に実装した**（`jobs/window.py`。`load_days` が全日を 1 つの
   表にして返すのをやめ、日ごとに読んで区間を埋める）。**手元の A/B は報告書が
   生成時刻の 1 行を除いて完全に一致**し、山は 5.85 → 4.47 GB（小さい窓）。
   **Linux の検算はマージのあと**——`training` 環境は `main` だけを許可するので、
   ブランチのコードでは測れない（§12 の 168 の「2 段目」）
