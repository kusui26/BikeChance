...
**`feature_set` は v4**（2026-09-24〜）：**ポートプロファイルの 7 列が入った**（W5 プラン §6.10 の PR J1、開発プラン §6.3・D-27）。**引くのは前日の版**（§7.5。`profile.source_day`）の、**目標時刻のセル**である。**前日の版が無い日（2026-09-07）は 6 列が全行 NULL・`prof_n_days` が 0。** **v3 と混ぜて学習しない**——2026-09-07 以降をすべて v4 で作り直す（同 §6.10 の完了条件 5）。
...
> **2026-09-15 までの日は層化してある**（`bikes(t) <= 2` または `docks(t) <= 2` を 4%、それ以外を 1%。重みは 25 と 100、`stratum` は `tight` と `uniform`）。**2026-09-16 に一様へ変えた**——母集団の 84.3% が「難所」で、層化が同じ行数の一様抽出より多く取れる難所は +13.3% しかなかったため（W5 プラン §6.9 の PR I、開発プラン §15 の D-25）。**列の意味は変えていない**ので、新旧の日を混ぜて読んでも重み付きの推定は不偏のままである。
