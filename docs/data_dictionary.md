...
匿名ロールに権限があるのは**この 2 つだけ**。基底テーブルには一切手が届かない（pgTAP `0010_public_views.sql` が 38 項目で固定）。**列の一覧も pgTAP が固定している**（`columns_are`）。読む側は `select *` をせず列を並べるので（`apps/web/lib/api/view-query.ts`）、片方だけ変えると実行時まで気づけない。
...
| `forecast_horizons_min` / `forecast_p_bike_x1000` / `forecast_p_dock_x1000` / `forecast_confidence` / `forecast_base_observed_at` / `forecast_model_version` | `station_forecasts` | **`left join`。予測が無ければ 6 列とも NULL**（0033） |
...
**列の正は `apps/ml/bikechance_ml/features/schema.py` の `SCHEMA`。** **65 列**（特徴量 57・キーとラベルと重み 8）。**`feature_set` は v1**（2026-09-09〜）：列は増えていないが、参照データを日次スナップショットから読むようにしたので `capacity` / `fill_ratio` / `is_over_capacity` / `nb_fill_ratio_mean_500m` の値が変わる（W3 プラン §14.6）。**v0 と v1 を混ぜて学習しない。**
...
| `minutes_since_last_change` | 34.6% | 読み込んだ窓の中で 1 度も変わっていない。**0 で埋めない** |
