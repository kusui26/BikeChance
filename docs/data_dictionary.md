...
| パス | 中身 | 実測 |
|---|---|---|
| `baseline/{version}.json.gz` | B1 の参照表・B2 の気候値・B3 の係数・ポートの並び | 3.2 MB |
