...
**2026-09-13（W5 の PR D）から 2 の意味が絞られた。** 気候値をポートプロファイルから作るようになり、引けない理由が「**そのセルに寄与した日が 2 日に満たない**」だけになった（それまでは抽出が薄く**全ポートが 2** だった）。**列コメント（migration 0026）は W3 段 8 のときのもの**で「気候値が無いので最大 2」と書いてあった——**2026-09-13 の昇格に合わせて migration 0044 で直した**。正はこの表で、コメントはその要約である。
...
- **未処理は `v_weather_pending` に出る**（Storage に何か在るのに、まだ入り切っていない発行）。
  読む側は `available_at` の下限を必ず添える（保持で消えた発行が蘇らないように）
- **分割が欠けた発行は「まだ 1 行も入っていない」あいだだけ出る**（0052）。欠けた分は
  二度と取れないので、入り切るまで出し続けると **`load_weather`（1 回 6 発行・古い順）が
  新しい発行に永久に到達しない**——2026-09-17〜18 に 34 時間ぶんが該当した
- **取り込みは欠けた分割を飛ばす**（0052）。**補間はしない**ので、入らなかったセルは
  行が無いまま残り、`build_features` の天気の被覆にそのまま出る
- **凍結したら鳴る**（0052）。`check_weather_stale()` が `monitor_jobs`（5 分毎）の中で
  最新の `issued_hour` を見て、**4 時間進んでいなければ通知する**。ジョブの見張りとは
  別に要る——2026-09-17 の `load_weather` は 34 時間ずっと `ok`（`rows: 0`）を返していた
  （W5 プラン §12 の 173）
...
- **成果物も同じ規則で置く**（W6-19、契約 38）。`fit_baseline`・`fit_lightgbm` の
  `--upload` は、**同じ名前が `active` / `shadow` なら置かない**。登録簿は 2 度引く
  （名前が決まった直後と、置く直前）。版の名前は学習の最終日なので、回し直すと同じ名前に
  なる——置き直すと、行はそのままで**配る値だけが変わる**（W5 プランの所見 157）
...
| パス | 中身 | 実測 |
|---|---|---|
| `baseline/{version}.json.gz` | B1 の参照表・B2 の気候値・B3 の係数・ポートの並び | **版 2 で 2.3 MB 前後**（2026-09-25 から。使えるセルがほぼ埋まっても 2.5 MB）。版 1 は 3.2 MB（〜09-12）→ 8.6 MB（09-13〜。気候値をプロファイルから作り、使えるセルが 66 万 → 198 万。W5 の PR D）→ 20〜28 MB（09-21〜。日数が増え、使えるセルが 400〜600 万） |
...
**ベースラインの成果物は `format_version = 2`**（2026-09-25、W6 の PR B。D-34）。**版 1 との違いは B2 の 2 欄だけ**で、読み手は**版 1 と版 2 の両方を読む**（契約 30。版 1 を読む道を消すのは、版 1 の成果物が active・shadow・candidate のどこにも無くなってから）。**書くのは版 2 だけ。**

| 欄（ターゲットごと） | 版 1 | 版 2 |
|---|---|---|
| B2 の「使えるか」 | `b2_keys`：使えるセルの番号（昇順、JSON の整数） | `b2_usable_bits`：**1 セル 1 ビット**の列（`np.packbits`、上位ビットから）の base64。長さは**ポート数 × 36 バイト** |
| B2 の率 | `b2_rate`：使えるセルの率を 6 桁に丸めた JSON の数 | `b2_rate_micro_u32`：6 桁に丸めた率 ×10^6 の整数を**リトルエンディアンの `uint32`** で並べた base64。並びは**使えるセルの番号の小さい順** |

- **セルの番号**は `(ポート × 3 ＋ 曜日種別) × 96 ＋ 15 分枠`（`climatology.cell_key`）。ポートは成果物の `ports` の並び、曜日種別は `DOW_TYPE_ORDER` の並び
- **読み出す値は版 1 と同じ倍精度**：率は `k / 10^6` で、これは `round(x, 6)` と一致する（IEEE の割り算は正しく丸める）。k は `round(x, 6)` を ×10^6 して作る（`np.rint(x × 10^6)` から直接作ると、半分の点の近くで 1 違う。W6 プランの所見 201）
- **読み手が止めるもの**（`ArtifactFormatError`）：base64 として読めない（`validate=True`）、ビット列の長さがポート数と合わない、率の列が 4 バイトの倍数でない、立っているビットと率の数が違う、率が 0〜1 の外、版 1 の鍵が昇順でない・表の外。**書き手も 0〜1 の外の率を止める**（`uint32` は負の数を黙って折り返す）
- B1（`b1_*`）と B3（`b3_*`）は版 1 と同じ。B1 は 6 桁に丸めた JSON の数、**B3 は丸めない**
- **ビット列にしたのは、開くときのメモリの山のため**：`…0923`（使えるセル 66.5%）で **20.2 → 2.26 MB、1.23 → 0.35 GB**、ほぼ埋まった `…0920`（98%）で **28.5 → 2.46 MB、1.6 → 0.39 GB**（macOS の `peak memory footprint`。鍵を差分で持つ形だと 0.76 GB。W6 プランの所見 200）

**B2 の下限の形も欄で持つ**（2026-09-25、W6 の PR A。D-37、契約 44）。**書式の版は上げない**——配信が読むのは使えるセルと率だけで、下限は「どう作った B2 か」の記録である。PR B の読み手は知らない欄を読み飛ばすので、PR A の書き手が書いた成果物もそのまま読める（実測：PR A の書き手の成果物を 09-23 の行に当て、両方の読み手の確率 120 万個がビット一致）。

| 欄（ターゲットごと） | 型 | 意味 |
|---|---|---|
| `b2_min_samples` | 整数 | セルを使う最小の件数（プロファイルなら格子点 2、学習サンプルなら行 30）。**前からある** |
| `b2_min_days` | 整数 | **配る曜日種別のうち最も低い日数の下限**（「これより薄いセルはどの種別でも配らない」）。**前からあり、整数のまま**——PR B の読み手は `int` で読む |
| `b2_serve_days` | `{"sat", "sun_holiday", "weekday"}` → 整数か `null` | **曜日種別ごとの配る側の下限**（日）。**`null` は配らない**（その種別の使えるセルは 0 が正しい） |
| `b2_fit_offset_days` | 整数 | **学習の行で下限を何日下げたか**。プロファイルから作ると 1（自分の日を丸ごと引くので、配るセル ＝ 混合が学習の行で見たセル。W5 プランの所見 179）、学習サンプルから作ると 0 |
| `b2_max_days` | 曜日種別 → 整数か、欄ごと `null` | **厚さ**：曜日種別ごとの、セルに寄与した日数の最大（当てはめたプロファイルの厚さ）。候補の版の確かめに使う（W6 プラン §6.1） |

- **欄の無い成果物**（PR A より前）は、全曜日種別に `b2_min_days` を当て、学習の行も同じ下限だった形として読む（厚さは「記録なし」）
- **読み手が止めるもの**（`ArtifactFormatError`）：`b2_serve_days` が 3 種別を持たない・整数か `null` でない（真偽値も止める）・全種別が `null`、学習の行の下限が 1 日を切る、`b2_min_days` が形から出す値（配る種別の最も低い下限）と食い違う、`b2_max_days` が 3 種別の整数でない
...
### 5.5 低ズームのセル（ビューではなく関数。0049）

**`/v1/stations` は矩形の辺で 2 通りの答えを返す**（W5 の PR E、契約 25）。**0.08 度を超えたらポートではなく格子のセル**で、どちらを返したかは応答の `aggregation` に書く。

| 辺（丸めた後） | 返すもの | 刻み | 実測（東京駅中心） |
|---|---|---|---|
| ≤ 0.08 度 | ポート | — | 0.02 度で 49 件 |
| 0.08〜0.25 度 | **セル** | 0.01 度 | 0.10 度：1,091 ポート → **108 セル** |
| 0.25〜0.5 度 | **セル** | 0.05 度 | 0.50 度：8,805 ポート → **94 セル** |

**ビューにしていないのは、刻みを引数で受けるため。** 同じビューに 2 つの粒度を持たせると、読む側がどちらを読んでいるか分からなくなる。実体は §6 の `v1_station_cells`。

**セルの値の約束**：

- `bikes` / `docks` は**観測のあるポートだけ**の合計（`is_present` でない行は、値がいつのものか分からない）
- `p_bike` / `p_dock` は**そのセルの最大**（平均ではない。W5-12）。**上限である**——「水平ごとの最大を取った曲線」を補間するので、隣り合う水平で最大のポートが入れ替わるセルでは真の最大よりわずかに大きい（実測で **89.25% のセルは誤差 0、残りの上界も 0.7 ポイント**）
- `confidence` は**セルの最大**（値も最大なので揃える）
- `stale` は**セルの中に 1 つでも「値をいつのものと言えないポート」があれば true**（フィード全体の鮮度とは別）
...
| `models` | モデルの成果物 | `application/gzip` | 無期限 | **同じ版は二度と作れない**（当てはめの時点が違う）。**配信中（active・shadow）の版の名前では置けない**（W6-19） |
...
**列の正は `apps/ml/bikechance_ml/features/schema.py` の `SCHEMA`。** **76 列**（特徴量 68・キーとラベルと重み 8）。

**`feature_set` は v4**（2026-09-24〜）：**ポートプロファイルの 7 列が入った**（W5 プラン §6.10 の PR J1、開発プラン §6.3・D-27）。**引くのは前日の版**（§7.5。`profile.source_day`）の、**目標時刻のセル**である。**前日の版が無い日（2026-09-07）は 6 列が全行 NULL・`prof_n_days` が 0。** **v3 と混ぜて学習しない**——2026-09-07〜09-23 は **2026-09-24 に v4 で作り直した**（同 §6.10 の完了条件 5）。**いま `features/` に v3 の日は無い。**

- v3（2026-09-10）：**天気の 4 列が入った**（`precip_mm_now` / `precip_mm_target` / `temp_c` / `wind_kmh`。W4 プラン §6.4）。**天気アーカイブは 2026-09-07 15:17 UTC からなので、それより前のサンプルは 4 列とも NULL になる。**
...
- **推論も同じ 68 列を作る**（W4 の PR C・PR D、W5 の PR J1）。出力は `SERVING_SCHEMA` の 72 列（76 から `y_bike` / `y_dock` / `weight` / `stratum` を抜いたもの）で、**学習の 1 点と完全に一致する**ことをゴールデンで固定している（`tests/test_features_parity.py`。**プロファイルを渡したときも渡さないときも**）。**ただし推論は J2 までプロファイルを渡さない**ので、配信の `prof_*` は NULL と 0 である（W5 プラン §9 の契約 29：`prof_*` を使うモデルは J2 まで配らない）
...
| **プロファイル**（v4） | **7** | **目標時刻のセル `(ポート, 目標の曜日種別, 目標の 15 分枠)` を前日の版（§7.5）から引く**。`prof_p_bike` / `prof_p_dock`（`n_bike_ok / n`・`n_dock_ok / n`。**B2 の率そのもの**で、休止した点も分母に入る）・`prof_mean_bikes` / `prof_std_bikes`（`sum_bikes / n`・`sqrt(max(sum_bikes_sq / n − 平均², 0))`）・`prof_rentals_per_hour` / `prof_returns_per_hour`（**60 分の窓の和を `n` で割る**ので 1 時間あたり。引くのは枠）・**`prof_n_days`**（セルに寄与した日数。**NULL にしない**——0 が「履歴が無い」）。**セルが無ければ 6 列は NULL**。水平で値が変わる（目標時刻が動くので引くセルが変わる） |
...
**抽出と重み**：行（`(ポート, 基準時刻, 水平)`）ごとに一様乱数を **1 回**引き、**`SAMPLE_RATE = 1%` の内側なら採る**。重みは `1/抽出率` ＝ **100 で一定**、`stratum` は `uniform` の 1 種類。乱数は `(日付, system_id, station_id)` の blake2b から splitmix64 で導く**決定的な値**で、2 回作れば同じ行が出る。

> ~~**2026-09-15 までの日は層化してある**~~ → **2026-09-24 の作り直し（v4）で全日が一様になった**（層化だった日の行は約 205 万 → 58〜60 万）。以下は作り直す前の記録である。**2026-09-15 までの日は層化してあった**（`bikes(t) <= 2` または `docks(t) <= 2` を 4%、それ以外を 1%。重みは 25 と 100、`stratum` は `tight` と `uniform`）。**2026-09-16 に一様へ変えた**——母集団の 84.3% が「難所」で、層化が同じ行数の一様抽出より多く取れる難所は +13.3% しかなかったため（W5 プラン §6.9 の PR I、開発プラン §15 の D-25）。**列の意味は変えていない**ので、新旧の日を混ぜて読んでも重み付きの推定は不偏のままである。
...
| `prof_*` の 6 列（v4） | **2026-09-07 は 100%**、2026-09-23 は **0.01%**（58 行） | 前日の版が無い日は全行。在る日は、その曜日種別・その枠をまだ観測していないセル（新しいポート、初めての土曜など）。**`prof_n_days` は NULL にならず 0** |
...
**読むのは基準時刻の前日の版**（`features/profile.source_day`）。開発プラン §6.2 のリーク防止で、参照スナップショットとまったく同じ規則である。**v4 の特徴量（§7.4 の `prof_*`）もここから引く**——B2 と同じセル・同じ枠の規則（`profile.target_slot`）で、`prof_p_*` と B2 の率は**ビット単位で一致する**ことを検査で固定している（W5 の PR J1）。

**`profile` は `daily` から作り直せる。** 窓の `daily`（28 日）を素直に足した版（`build_profiles --from-dailies`）が、転がして置いた版と**バイト単位で一致する**ことを本番の 3 日（2026-09-07・13・23）で確かめた（W5 の PR J1、開発プラン §15 の D-28）。
...
> **2026-09-23 に下限を 2 → 3 日にした**（開発プラン §15 の D-30）。上の 99.72% は下限 2 のときの実測で、**いまはセルが使えるのは 3 日目から**である。**2 日ぶんの B2 は配らないほうが当たっていた**（W5 プラン §12 の 177）。下限の持ち主は読む側（`baselines/climatology.py` の `MIN_CELL_DAYS`）1 か所で、プロファイルは切らずに `n_days` を持つ。
>
> **2026-09-25 から、配る側の下限は曜日種別ごと**（`climatology.SERVE_DAYS`。W6 の PR A、D-37）。**平日は 3 日、土日祝は 9/28 に 9/27 の実測で 3・4・5・配らない から選ぶ**（W6 プラン §6.1）。**決まるまでは土日祝を配らない**。`build_profiles` の要約（`job_runs.detail`）の使えるセルもこの形で数え、鍵は `min_days`（整数）から **`serve_days`**（曜日種別 → 日数か `null`）に変わった。**配らない種別の `usable` は、厚くても 0 が正しい**（厚さは `by_dow_type` の `days_max` で見る）。
...
| 履歴プロファイル（`prof_*`）・再配置・天気の特徴量 | ~~**無い**（`features/` の v0 には列ごと入れていない）~~ → 天気は v3（§7.4）、**`prof_*` は v4（2026-09-24、W5 の PR J1）で入った**。**再配置はまだ無い** | 再配置は W8 |
