...
### 4.11 `station_forecasts` — 先回り予測（見込み 20,431 行）
...
- 5 分毎に全件 UPSERT（`fillfactor = 50` で HOT 更新に乗せる）
- **まだ本番に入っていない。** 行数は読み取りのみの試走の値（HELLO 14,663 ＋ ドコモ 5,768）
...
create table public.inference_log (
  ...
  n_rows integer, duration_ms integer, error text,
...
**列の正は `apps/ml/bikechance_ml/features/schema.py` の `SCHEMA`。** v0 は **65 列**（特徴量 57・キーとラベルと重み 8）。
