...
- **変更履歴**：v1.2（2026-09-08）W2 完了時点の点検を反映（`job_runs` の保持規則の記述を訂正、天気ファイルの入手時刻、`fetched_at` の代用）。v1.1（2026-09-08）EDA #1 の結果を反映（実在しないポート `5753`、名前での除外が危険なこと）。v1.0（2026-09-08）初版。W2 の PR F。
...
- 実測：**182 kB/時・4.4 MB/日**、1 時間あたり 6 ファイル
...
|---|---|---|
| `system_id` | string | |
| `station_id` | string | **`idx` は書かない**（再構築で変わり得るため） |
| `observed_at` | timestamp[ms, UTC] | フィードの `last_updated` |
| `bikes` / `docks` / `flags` / `reported_age_s` | int16 | **`-1` は「観測されなかった」。0 と区別する** |

- ソート：`station_id, observed_at`。圧縮：zstd。区間：半開 `[hour, hour+1)`
- **Hive 形式のデータセットとして読むと列が 2 つ増える**：`date`（string、`"2026-09-07"`）と `hour`（**int32**、`4`。ゼロ埋めは落ちる）。ファイル 1 つを `pq.read_table` で読んだときは 7 列のまま
- **配列長より外側の `idx` は行にしない**（未登録と欠損を混ぜない）
- 観測が 0 件の時間帯には**ファイルを置かない**
- 冪等：同じ時間帯を 2 回処理すると同じ行になる（実測ではバイト列まで一致した）
- 実測：**4.05 MB/日・1,047 万行/日**（1 時間あたり約 44 万行）

> **Parquet には `-1` がそのまま入っている**（ビューと違って NULL に開いていない）。学習ローダーが必ず落とすこと（§10.1）。

> **⚠️ `fetched_at` が入っていない。** as-of 結合は `observed_at` ではなく **`fetched_at` で切る**のが正しい（本番の推論が使えるのは取り込み済みのものだけ。§10.3）。Parquet に無いあいだは、生 JSON の `storage.objects.created_at` で代用できる — `fetched_at` との差は**平均 0.4〜0.8 秒・最大 4.4 秒**（実測 2,408 件）で、300 秒の公開周期に対して十分に細かい。`n_stations` と `is_anomalous` は Parquet から導ける（`bikes >= 0` の行数、およびその割合が 0.5 未満か）。
...
[{
  ...
  "hourly": {
    "time": ["2026-09-08T00:00", "2026-09-08T01:00", ...],   ← 48 点
...
配列は**取得日の JST 0 時から 48 時間**を覆う。パスの `hour_epoch` は取得した UTC の時刻なので、両者は一致しない。
...
→ **取得時刻が遅いほど、前向きの予報時間は短くなる。** JST 23 時に取ったファイルは 24 時間先までしか無い。予測の最大水平は 180 分なので**最悪でも十分に足りる**が、「48 時間先まで常にある」と思ってはいけない。
...
実測（1 ファイル・95 地点 × 48 時間 = 4,560 点）：

| 系列 | 2 モデルが一致した割合 | null |
|---|---|---|
| `precipitation` | **96.95%** | 0 |
| `weather_code` | **92.28%** | 0 |
| `temperature_2m` | **90.96%** | 0 |
| `wind_speed_10m` | **90.66%** | 0 |
| `precipitation_probability_jma_msm` | — | **100% null** |
| `precipitation_probability_best_match` | — | 0 |

> **訂正**：W2 プラン §12 の 47 は「日本では決定論的な値は両者一致する」と書いているが、実測では 90.7〜97.0% の一致で**完全一致ではない**。`best_match` は `jma_msm` の別名ではなく、Open-Meteo が地点ごとに選ぶ別のモデル系列である。**どちらを特徴量に使うかは W4 で決める**（決めるまでは両方を保存し続ける）。
...
| **as-of 結合を `observed_at` で切る** | 本番が使えるのは**取り込み済み**のものだけ。`fetched_at <= t` で切る。HELLO は公開遅延が平均 72 秒・300 秒周期なので、**5 分グリッド点の約 24%** で「学習では見えるが本番ではまだ来ていない」値を掴む |
