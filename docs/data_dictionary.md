...
**モデルの成果物**：**`models` バケット**。
`baseline/{version}.json.gz`（B1 の参照表・B2 の気候値・B3 の係数・ポートの並び。実測 3.2 MB）と
`lightgbm/{version}.json.gz`（`Booster.model_to_string()` を 2 つ ＋ 列の並び・カテゴリ・語彙）。
