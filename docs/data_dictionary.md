...
**列の正は `apps/ml/bikechance_ml/features/schema.py` の `SCHEMA`。** **65 列**（特徴量 57・キーとラベルと重み 8）。

**`feature_set` は v2**（2026-09-10〜）：列は増えていないが、**`minutes_since_last_change` に上限（180 分）が入った**（W4 プラン §4 の W4-10）。この列だけが**読み込んだ窓の長さで値が変わる**列で、25 時間読む学習と 3 時間読む推論で必ず食い違っていた。上限を決めて、どちらも同じ値を出せるようにしてある。**v1 と混ぜて学習しない。**
...
- **推論も同じ 57 列を作る**（W4 の PR C）。出力は `SERVING_SCHEMA` の 61 列（65 から `y_bike` / `y_dock` / `weight` / `stratum` を抜いたもの）で、**学習の 1 点と完全に一致する**ことをゴールデンで固定している（`tests/test_features_parity.py`）
