...
- **保持は 90 日、ただし `ok` だけ**（0031）。失敗・二重抑止・閉じられずに `running` のまま残った行は消さない。**5.2 万行・約 31 MB で頭打ち**になる
