...
| ドコモのスナップショット取得数 | 約 1,066 件/日（**実測 80.9 秒周期**。初稿は 80 秒として 1,080 件としていた） | **≥ 1,061 件**（同上） |
| **収集の呼び出し間隔の最大** | 収集周期どおり（60 秒） | **< 収集周期 × 3**（毎分なら 180 秒） |
| 連続欠損の最大長 | 0 | **30 分未満** |
...
判定用の SQL と照合スクリプトは §8 にまとめた。判定は `scripts/verify-m1.sql` が一括で行う。

**取得数の基準は提供側の公開回数に依存する。** ドコモの行を 1,080 → 1,066 に直したのは、24 時間の実測で公開が 1,068 回（86,400 ÷ 1,068 ＝ 80.9 秒）だったため（§5.6 の 43）。**基準を下げたのではなく、期待値の誤りを実測で正した。** 取りこぼしが無かったことは、提供側の挙動に依存しない指標で別に確かめている。「収集の呼び出し間隔の最大」を行に加えたのはそのためで、こちらは我々の責任範囲だけで判定できる。
...
### 5.6 段 7（24 時間観測）で見つかった問題

24 時間の実測が無ければ気づけなかった 2 件。どちらも**壊れて見えないまま静かに効く**種類の問題だった。

| # | 重大度 | 問題 | 修正 | 反映 |
|---|---|---|---|---|
| 42 | 中 | **フィード停滞の閾値に「検知の遅れ」が入っていなかった。** W1-29 の式 `max(期待周期 × 3, 収集周期 × 3)` はフィードが何秒おきに公開するかしか見ていない。しかし `monitor_feeds` が比べるのは `now() - last_observed_at` で、これは「公開の間隔 ＋ その公開を我々が取りに行くまでの時間」になる。後半は最大で収集周期ぶんある。24 時間の実測でドコモの観測の遅れは最大 **244 秒**、閾値は 240 秒で、**1,071 区間のうち 1 回は超えていた**（09/07 20:19:27〜20:23:31 JST）。通知が出なかったのは `monitor_feeds` が 5 分おきで、その瞬間を偶然またいだだけ | 式に `+ collect_interval_s` を足す。ドコモは 240 → 303 秒（実測の最遅に 24% の余裕）、HELLO は 900 → 960 秒。E1 では両システムとも 1,200 秒 | `0013_stall_threshold.sql` |
| 43 | 軽微 | **ドコモの期待周期 80 秒が 1.1% 楽観的だった。** 24 時間の公開は 1,068 回（86,400 ÷ 1,068 ＝ 80.9 秒）。中央値は 80 秒だが、1 周期とばす回が 16 回あって平均は 81 秒になる。`compute_daily_quality` の `n_expected` は 86400 ÷ 期待周期なので、**取りこぼしがゼロでも毎日 98.9% と表示され続ける**。鳴りっぱなしの警報は無視されるようになる | `expected_cadence_s` を 81 に訂正し、§1 の合格ラインを 1,074 → 1,061 に直す。**基準を下げたのではなく期待値の誤りを正した**ことを示すため、提供側の挙動に依存しない指標「収集の呼び出し間隔の最大」を §1 の表に加えた | `0013_stall_threshold.sql`、`verify-m1.sql`、`constants.ts` |

**43 をどう確かめたか**（基準を都合よく緩めていないことの裏づけ）：

- ポーリングは 1,440 分に 1,440 回、**両システムとも 1 分も欠けていない**。呼び出し間隔の最大は 63 秒
- 取得の失敗は 0 件（0 / 2,880）。ウォッチドッグの発火も 0 回
- ポーリング周期（60 秒）がフィードの公開周期（最短 60 秒台）より短いので、**公開を取り逃す経路が無い**
...
**本番での実行結果（2026-09-07）**：**合格**。ベースライン → 日次同期 × 2 の順に実行した。

| 実行 | HELLO | ドコモ |
|---|---|---|
| ベースライン（9/6 の生 JSON） | 14,835 ポート、版 14,835 | 5,796 ポート、版 5,796、`geo_suspect` 1 |
| 日次同期 1 回目（9/7） | 変更 6、版 93、所要 3.6 秒 | 変更 1、版 25、所要 0.8 秒 |
| **日次同期 2 回目** | **変更 0、版 0** | **変更 0、版 0** |

1 回目の「版」が「変更」より多いのは、9/6 のベースラインに無く、その後 status で登録された
ポートが初めて属性を得たため（HELLO 87 件、ドコモ 24 件）。2 回目は両システムとも 0 で、
冪等に動いている。

| 完了条件 | 結果 |
|---|---|
| 2 回目の `n_changed=0` | 両システムとも 0 |
| 有効行数（HELLO 14,861 / ドコモ 5,800 前後） | 14,922 / 5,820 |
| 属性履歴の起点 | 両システムとも **9/6 16:06 JST**（ベースラインを先に入れたため） |
| `job_runs` の記録 | `sync_stations:*` が各 2 回、失敗 0 |
| 収集への影響 | スナップショットの連続性・失敗数・通知いずれも変化なし |

**「status にしか現れないポート」は現時点で 0 件だった**（プランは「ドコモ 11 件」と見込んでいた）。
台帳の 20,742 ポートすべてが属性を持っている。理由は、**入力に無いポートの有効行を閉じない**ため、
属性の網羅は日をまたいで**積み上がる**から。ある日 `station_information` から消えても、
一度でも現れていれば属性は残る。

ただし **0 のままとは限らない**。新しいポートは status に現れた時点で台帳に載り、属性を得るのは
次の日次同期（最大 24 時間後）なので、その間は属性が無い。開発プラン §8.3 の
「属性が NULL でも 200 を返す」設計は引き続き必要で、根拠が
「恒常的に 11 件ある」から「新しいポートは最大 1 日のあいだ属性を持たない」に変わる。
...
**M1 の判定結果（2026-09-07 20:56 JST、窓は 24 時間ちょうど）**：**合格**。

```
psql "$SUPABASE_DB_URL" -v from_at='2026-09-06T11:56:42Z' -v to_at='2026-09-07T11:56:42Z' \
                        -v baseline_bytes=23284883 -f scripts/verify-m1.sql
```

| # | 指標 | 実測 | 合格ライン | 判定 |
|---|---|---|---|---|
| 1 | HELLO のスナップショット取得数 | 288 件 | ≥ 287 | 合格 |
| 2 | ドコモのスナップショット取得数 | 1,067 件 | ≥ 1,061 | 合格 |
| 3 | 連続欠損の最大長 | 5 分 5 秒 | < 30 分 | 合格 |
| 4 | 重複行 | 0 行 | 0 | 合格 |
| 5 | Storage の生 JSON と行数 | ドコモ 1,067/1,067、HELLO 288/288 | 完全一致 | 合格 |
| 6 | DB サイズの増分 | 22.2 MB | ≤ 40 MB/日 | 合格 |
| 7 | 収集の Active CPU | 約 85 ms/回 | ≤ 0.3 秒/回 | 合格 |
| 8 | 収集エンドポイントの失敗率 | 0.00%（0/2,880） | < 1% | 合格 |
| 9 | 配列と生 JSON の照合（§8.6） | 6/6 で不一致 0 | 各システム 3 件 | 合格 |
| 10 | DEFAULT パーティションの行数 | 0 行 | 0 | 合格 |
| 11 | 収集の呼び出し間隔の最大 | 63 秒 | < 180 秒 | 合格 |

ジョブは 24 時間で `watchdog_collect` 1,445 回・`monitor_feeds` 289 回・日次 3 本、**すべて成功で失敗 0 件**。ウォッチドッグの発火は 0 回で、Vercel Cron が 1 分も欠けなかったことを意味する。通知は日次ダイジェスト以外に 1 件も出ていない。

判定の過程で監視の欠陥を 2 件見つけ、`0013_stall_threshold.sql` で直した（§5.6 の 42・43）。

**W1 のマイルストーンはこれで全て達成した。** M0− は 9/6、DB 経路の検証も 9/6、M0 が 9/6 20:07 JST、M1 が 9/7 20:56 JST。当初計画（M0 9/10・M1 9/11）より 4 日早い。残るのは段 8 の PR F（ポート属性の日次同期）。
...
**24 時間の実測（2026-09-06 20:56 〜 09-07 20:56 JST、毎分収集）**：上の推定と突き合わせた。

| 項目 | 上の推定 | 24 時間の実測 |
|---|---|---|
| 生 gzip（HELLO） | 288 個 ＝ 31 MB | **288 個 ＝ 31 MB** |
| 生 gzip（ドコモ） | 1,080 個 ＝ 39 MB | **1,067 個 ＝ 38 MB** |
| 生データ 合計 | 約 70 MB/日 | **約 69 MB/日** |
| 配列（HELLO） | 288 行 ＝ 6.5 MB | **288 行 ＝ 6.3 MB** |
| 配列（ドコモ） | 1,080 行 ＝ 10.7 MB | **1,067 行 ＝ 10.0 MB** |
| 配列 合計 | 約 17 MB/日 | **約 16.3 MB/日** |
| **DB 全体の増分**（`pg_database_size` の差） | — | **22.2 MB/日** |

`pg_database_size` は 23,284,883 → 46,615,699 バイト。**§1 の期待値 22.7 MB/日にほぼ一致した**
（合格ラインは 40 MB/日）。配列との差 5.9 MB がインデックス・行ヘッダ・他テーブルの分で、
配列に対する上乗せは 36%。60 日保持で **約 1.3 GB** の見込みで、Pro の 8 GB と 6 GB 警報に
対して余裕がある。
...
| 項目 | HELLO | ドコモ |
|---|---|---|
| 304 の比率 | **80%**（1,152 / 1,440） | 26%（373 / 1,440） |
| 1 回の所要（200 の中央値） | 1,282 ms | 669 ms |
| 1 回の所要（304 の中央値） | 423 ms | 231 ms |
| フィードの更新間隔（中央値／p95／最大） | 300／302／305 秒 | 80／83／186 秒 |
| 観測の遅れ（中央値／p95／最大） | 367／370／370 秒 | 111／139／**244 秒** |

HELLO の 304 比率 80% は「5 回に 4 回」そのもので、W1-4 の想定どおり。304 の所要が 200 の
3 分の 1 で済んでいることから、条件付き取得は転送量だけでなく CPU も削っている。
**観測の遅れの最大 244 秒は §5.6 の 42 の発端**で、停滞の閾値 240 秒を超えていた。
...
| 天気・祝日の**取り込み** | W4 | 特徴量の作業 |
| 天気バックフィルの**検証**（Open-Meteo Historical Forecast API が `jma_msm` の過去予報を返すか） | **W2** | **先送りできない。** 前提が外れていたら、その日から予報のアーカイブを始めるしかなく、遅れた日数は永久に失われる（§10.1） |
| 近傍リスト・履歴プロファイル | W3 | 特徴量の作業 |
| 公開 API 用の読み取りビュー（`/v1/stations` の現在値） | W2 | `station_status_latest` に `is_present` が入ったので、ビューはその上に作れる |

### 10.1 W1 完了時点の点検と、W2 に持ち越す宿題

収集が本番稼働した時点で、「いま貯めているデータで学習ができるか」を実データで点検した。詳細と数値は開発プラン §6.7 にある。ここには**やらなければならないこと**だけを残す。

**結論**：ODPT 由来のものはすべて作れる。生 JSON を無期限に保全しているので、あとから解釈を変えることもできる。**問題は 2 つで、どちらも期限がある。**

| # | やること | 期限 | 理由 |
|---|---|---|---|
| 1 | **Open-Meteo の Historical Forecast API で 1 日分の過去予報を実際に取得し、`jma_msm` が返ることを確かめる** | **W2 のできるだけ早く** | 天気は「`t` 時点で入手できた予報」を使う決まりで、**過去に遡って観測できない唯一の入力**。前提が外れていたら、その日から予報のアーカイブを始めるしかなく、遅れた日数はそのまま失われる（開発プラン R16） |
| 2 | **生 JSON からの再構築スクリプトを動く状態にする** | **2026-11-30** | 60 日保持の削除で `status_snapshots_y2026m09` が落ちる日。生 JSON は残るので消失はしないが、再構築が動かなければ 9 月分は実質使えない。最初の題材は PR 0 期の積み残し（ドコモ 124・HELLO 34 オブジェクト、DB に行が無い）（開発プラン R17） |

**開発プランを直した点**（実装と実測に合わせた）

| 章 | 直した内容 |
|---|---|
| §6.1 | 除外規則から `stations.is_active` と `capacity=0` を外した。前者は**現在の**状態で過去のサンプルに遡って効き、生存者バイアスになる。後者はドコモの動的容量で、有効行の 4.0%（233 件）が一瞬の 0 で落ちてしまう。休止の判定は `flags` で行う |
| §6.1 | 「ポートが観測されていない」を除外規則に明記した。配列の `-1` と `idx ≥ array_length` の 2 通りがあり、**ゼロ埋めすると新しいポートが登録前の期間ずっと「空」として学習される**。台帳は 1 日に HELLO で +87 増えている |
| §6.3 | `is_charging_station` は HELLO 専用だった。カタログに無かった `parking_type`・`parking_hoop`（HELLO）と `region_id`（ドコモ）を追加。ドコモの `staleness_s` は恒常的に 0 で情報が無いため欠損として扱う |
| §6.3 | `gap` は DB の `docks` から計算すると過小になる。HELLO は定員超過のポートで `-1` を返し、収集器が 0 に丸めているため（1 スナップショットあたり中央値 10 ポート）。生 JSON から計算するか、定員超過を別の特徴量にする |
| §5.6 | 毎時 Parquet 化は「任意の最適化」ではなく**必須**と明記。配列形式は書き込みに最適化されており、ポート単位の時系列を取るには全行の展開が要る |
| §6.3 / §6.7 | **流量の特徴量をフィード本来の周期で計算する**ことにした。5 分グリッドに as-of してから計算すると、ドコモで `|Δbikes|` の総和が 13% 減る（直近 3 時間の実測で 22,704 → 19,741 台）。毎分収集がデータを増やしているのはドコモだけ（288 → 1,067 行/日）で、その増分はここで効く |
| §6.7 | HELLO は公開周期（300 秒）と基準時刻のグリッド（5 分）が同じで、水平 5 分のラベルが基準時刻の状態と同じ観測を指すと情報を持たなくなる。実測では 331 枠中 0 件だったが、位相は月単位でずれ得るので、ローダーで落とす条件を入れておく |
| §14 | R16（天気バックフィル）と R17（保持期間の削除が再構築より先に来る）を追加 |

**点検して問題が無かったもの**：異常スナップショット 0 件、フィードの遅れ（HELLO 中央値 67 秒・最大 226 秒／ドコモ 31 秒・最大 77 秒）は除外規則の 10 分に対して十分な余裕、`-1` の割合は最新スナップショットで HELLO 1/14,922・ドコモ 16/5,820、`reported_age_s` の丸めは 0 件。
