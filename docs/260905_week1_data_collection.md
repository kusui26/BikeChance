...
## 2. 着手時点の状態（2026-09-06、実装を始める前）

> **その後の進捗（2026-09-06 21:00 JST）**：下の「未着手」はすべて完了した。PR 0・A・C・B・D・E1・E2 を
> マージし、マイグレーション 0001〜0012 を本番に適用済み。**収集は毎分で本番稼働中**（M0 達成、§7.1）。
> 残るのは段 7 の 24 時間観測（M1）と段 8 の PR F。
...
実フィードに当てて分かったこと（§5.2）から導いた決定。

| ID | 論点 | 決定 | 根拠 |
|---|---|---|---|
| **W1-25** | 台数・返却枠の負値 | **検証では受け取り、正規化で 0 に丸める。** 丸めた件数を `warnings.clamped_negative_counts` に残す | 仕様どおり非負で弾くと HELLO のフィードが丸ごと取り込めない。`-1` のまま保存すると欠損の予約値と衝突する。0 は「返す枠が無い」という意味でも正しい（§5.2 の 32） |
| **W1-26** | 検証の失敗とデータ保全 | **生 JSON の保存を検証より先に行う。** `gbfs-core` の検証は例外を投げず `{ ok, feed } | { ok, issues }` を返す | 一次ソースの保全が検証の成否に依存してはいけない。例外を投げる設計だと、呼び出し側が try/catch を書き忘れた瞬間にデータを失う（§5.2 の 33） |
| **W1-27** | フィクスチャの出どころ | **PR 0 が本番の Storage に保存した生 JSON を縮約して使う。** `station_status` は縮約せず、`station_information` のみ約 1,000 件に絞る | 同じバイト列がテストにも本番にも流れる。性能テスト（14,835 ポートで 150 ms 未満）とゴールデンテストは実物の規模でないと意味がない。癖のあるポート（重複・BBox 外・capacity=0・文字列容量）は必ず残す |
...
| 11 | PostgREST の `max_rows` は 1000。集合を返す RPC も**テーブルの取得も、警告なく切り詰められる**。`limit=100000` と書いても効かない | RPC は `jsonb` のスカラを返す（PR B）。テーブルを全件取る照合スクリプトは `offset` でページングする（PR D。§5.4 の 37） |
...
### 5.2 PR C の実装中に実データで見つかった問題

`packages/gbfs-core` を実フィードに当てて初めて分かったこと。番号は §5.1 からの続き。

| # | 重大度 | 問題 | 修正 | 反映 |
|---|---|---|---|---|
| 32 | **重大** | §11.1 は「GBFS の台数・返却枠に負値は存在しない」を前提に `-1` を欠損の予約値にしていたが、**この前提が誤りだった**。HELLO は定員超過のポートで `num_docks_available` に **`-1`** を返す（実測 17 件）。そのまま保存すると「返却枠が -1」と「そもそも観測していない」が区別できない。仕様どおり非負で検証すると、今度は **HELLO のフィードが丸ごと取り込めなくなる** | 検証では負値を受け取り、**正規化で 0 に丸める**。丸めた件数を `warnings.clamped_negative_counts` に残す（W1-25） | §4.2d、§11.1、§11.6 |
| 33 | **重大** | §6.6 の手順は「6. 検証 → 7. 生 JSON を保存」の順で、**検証に失敗すると生 JSON が残らない**。生 JSON は一次ソースで、後から何度でも再処理できることが唯一の保険なのに、その保険が検証に依存していた | **手順 6 と 7 を入れ替え、受信したバイト列を先に保存する**。あわせて `gbfs-core` の検証は例外を投げず結果を返す形にし、呼び出し側が try/catch を書き忘れてもデータを失わないようにした（W1-26） | §4.2d、§6.6 |

### 5.3 PR B の実装中に見つかった問題

| # | 重大度 | 問題 | 修正 | 反映 |
|---|---|---|---|---|
| 34 | **重大** | §11.3 は、後退したスナップショットから最新値を守る条件を「行ごとの `p_observed_at > last_changed_at`」としていた。**これでは守れない。** `last_changed_at` は「最後に**変化**した時刻」なので、しばらく値が動いていないポートは、古いスナップショットでも「前回の変化より新しい」と判定されて上書きされる。実際 pgTAP で再現した（値が 3 回変わらなかったポートが、8 分前のスナップショットで書き換わった） | 後退かどうかは**フィード単位の性質**なので、`feed_state.last_observed_at` と比べ、古ければ `station_status_latest` の更新を丸ごと行わない | §11.3、`0007_ingest_rpc.sql` |
| 35 | 軽微 | `now()` は**トランザクション内で固定される**。pgTAP は 1 トランザクションで走るため、`begin_fetch` の「30 秒後に再 claim できる」を間隔 0 で確かめようとしても再現できない | テストでは `last_fetch_at` を明示的に戻す。実運用では毎回別トランザクションなので影響しない | `0004_begin_fetch.sql` |

### 5.4 PR D の実装中に見つかった問題

| # | 重大度 | 問題 | 修正 | 反映 |
|---|---|---|---|---|
| 36 | **重大** | **W1-4 の条件付き取得が、書いたとおりには効かない。** Fetch 仕様は「要求に `If-None-Match` があり cache モードが default なら no-store に切り替える」と定めており、no-store モードでは要求に `Cache-Control: no-cache` が付く。**ODPT はこれを見ると条件付き取得を無視して 200 を返す。** つまり ETag を送っても常に全文が返り、80% の転送削減（155 MB/日 → 31 MB/日）が黙って失われる。エラーもログも出ないので気づけない | `fetch` に **`cache: "no-cache"`** を指定する。このモードだけが `Cache-Control: max-age=0` を送り、ODPT が 304 を返す。実測で確認し、回帰防止のテストを置いた（W1-28） | §4.2e、§6.6、`odpt-fetch.ts` |
| 37 | 中 | 照合スクリプトが PostgREST の `max_rows`（1000）に当たり、**14,835 件の台帳を 1,000 件と誤認していた**。しかもその状態で「1,000 件すべて一致」と表示されるため、**照合が通ったように見えてしまう**。§4.3 の 11 は RPC の話として書いていたが、テーブルの取得でも同じことが起きる | `offset` でページングする。§4.3 の 11 の記述も一般化した | §4.3、`reconcile-*.mjs` |

**実測（2026-09-06、同じ ETag で ODPT に条件付き要求）**

| `fetch` の `cache` | 送られる要求ヘッダー | ODPT の応答 |
|---|---|---|
| 指定なし / `no-store` / `reload` / `default` | `Cache-Control: no-cache`、`Pragma: no-cache` | **200**（全文） |
...
| **E2** | 毎分化 | `vercel.json` の `crons` を `* * * * *` に、`collect_interval_s` を 60 に | **大**（M0） | 15 分 |
...
6. **受信したバイト列を gzip** し、§11.5 のパスに `contentType: 'application/gzip'`・`upsert: false` で保存。409 は成功扱い。**検証より先に保存する**（W1-26）。一次ソースの保全を検証の成否に依存させない。`last_updated` が読めない場合は取得時刻をパスに使う
7. `gbfs-core` で検証・正規化。検証は例外を投げず結果を返す。失敗 → `finish_fetch(error, ok=false)` → 500（**生 JSON は既に保存済み**）。`last_updated` が `last_observed_at` **以下**（同じか、後退している）→ `finish_fetch(unchanged)` → 200。後退したスナップショットは取り込まない
...
**目的**：収集の Cron を毎分に戻し、**本番稼働を宣言する**。差分は 2 行で、巻き戻しも 2 行。

**変更ファイル**

```
vercel.json                                              # crons を * * * * * に戻す
supabase/migrations/<ts>_0012_collect_every_minute.sql   # collect_interval_s を 60 にする
apps/web/vercel-crons.test.ts                            # Cron 式と poll_interval_s の食い違いを CI で検出する
```

W1-29 で監視の閾値を `collect_interval_s` から導出するようにしたため、この PR で触るのは
**Cron 式と設定値の 2 箇所だけ**になった。初稿にあった「ウォッチドッグを 150 秒に戻し、
ドコモの停滞検知を有効化する」マイグレーションは要らない。設定値を 60 にすれば、

| 閾値 | 式 | E1（300 秒） | E2（60 秒） |
|---|---|---|---|
| ウォッチドッグ | `collect_interval_s × 2 + 30` | 630 秒 | **150 秒** |
| フィード停滞 | `max(expected_cadence_s × 3, collect_interval_s × 3)` | 両システム 15 分 | **HELLO 15 分・ドコモ 4 分** |

が同時に締まる。

**適用の順番**：**本番デプロイ（毎分）を先に、マイグレーションを後に。** 逆にすると、収集が
まだ 5 分間隔なのにウォッチドッグの閾値だけ 150 秒になり、毎分ウォッチドッグ経由で収集が
走る。ドコモの停滞検知も 4 分になるため誤報が出る。順序どおりなら、デプロイからマイグ
レーションまでの数分間は監視が緩いだけで害はない。

**Cron 式と設定値の食い違いを機械で防ぐ**：`vercel.json` を速くして設定を戻し忘れると、
「収集は毎分なのにウォッチドッグは 10 分待つ」という、壊れていないように見えて監視だけが
緩い状態に黙ってなる。`apps/web/vercel-crons.test.ts` が Cron 式を
`SYSTEMS[system].poll_interval_s` から導いた式と突き合わせ、片方だけの変更を CI で落とす。

**完了条件（M0）**

- 両システムが期待周期で増え続ける（30 分観測：HELLO 6 件、ドコモ 22 件前後）
- `feed_fetch_log` の `unchanged`（304）の比率が HELLO で 8 割前後になる（ETag が効いている証拠）
- `monitor_feeds` が誤報を出さない（特にドコモの停滞。閾値が 4 分に締まる）
- 収集の Active CPU が 1 回 0.3 秒以下

**巻き戻し**：`vercel.json` を `*/5` に戻して再デプロイし、`collect_interval_s` を 300 に戻す。
急ぐ場合は Vercel の Cron 画面で Disable する（設定値はそのままでよい。閾値が緩む方向）。
...
### 7.1 段 7（24 時間観測）の実施記録

M0 は **2026-09-06 20:07 JST** に達成した（PR #11 のマージで本番デプロイが毎分に切り替わった）。段 7 の観測窓はその約 50 分後、判定に必要な値をすべて記録できた時点から始めた。

| 項目 | 値 |
|---|---|
| 窓の開始 | **2026-09-06T11:56:42Z**（20:56:42 JST） |
| 判定できるのは | **2026-09-07 20:56 JST** 以降 |
| `pg_database_size` のベースライン | **23,284,883 バイト** |

判定はこの 3 つを渡して行う。

```
psql "$SUPABASE_DB_URL" -v from_at='2026-09-06T11:56:42Z' -v baseline_bytes=23284883 \
                        -f scripts/verify-m1.sql
node scripts/reconcile-snapshot.mjs .env hellocycling   # 3 スナップショット分
node scripts/reconcile-snapshot.mjs .env docomo-cycle   # 3 スナップショット分
```

Active CPU は Vercel Observability の Compute で見る（M0 直後の実測は 1 回あたり約 85 ms、基準 0.3 秒に対して 4 分の 1 以下）。

**独立した裏取り**：`compute_daily_quality` は JST の日付で集計するので、9/7（JST）の行が 9/8 の 07:00 JST に自動で書かれる。窓の取り方が違うため、この行と上の判定は完全には一致しない。数字が食い違ったときは、窓の定義の違いをまず疑う。

**観測中に異常が出たら**：`monitor_feeds` が 5 分毎に検知して `alert_state` に記録する（Webhook 未設定でも記録は残る）。合格基準を満たせない場合は §9 の対応を検討し、原因が消えるまで段 8 に進まない。

## 8. 検証の手順と合格判定

段 4（DB 経路の検証）と段 7（24 時間観測）で使う。段 0〜3 の間は Storage のオブジェクト数だけを見る（§6.2）。すべて Supabase の SQL Editor か `psql` で実行できる。

**段 7 の判定は `scripts/verify-m1.sql` にまとめてある。** §1 の合格基準表の 10 項目を 1 回で評価し、項目ごとに PASS / FAIL を出す。手で 10 個のクエリを流すと、窓の指定や時刻の基準を取り違える（§5.5 の 37 と 38 は実際にその類の取り違えだった）。
...
`from_at` を省くと「今から 24 時間前」。`baseline_bytes` は窓の開始時点の `pg_database_size` で、渡すと DB 増分も判定する（出力の末尾に次回用の値が出る）。判定表のあとに内訳が出て、§8.4 の表に書き足す値もそこで揃う。

このスクリプトで判定できないのは 2 つだけ。**Active CPU** は Vercel Observability で見る（§8.4）。**§8.6 の照合**は `scripts/reconcile-snapshot.mjs` を各システム 3 回動かす。
...
Storage の `gbfs-raw/{system}/{YYYY}/{MM}/{DD}/` のオブジェクト数と、同じ UTC 日の `status_snapshots` の行数を突き合わせる。照合スクリプト `scripts/reconcile-raw.mjs`（PR D で追加）が両方を数えて差分を出す。
...
**本番での実測（2026-09-06、PR E2 の直前）**：開発プラン §5.2 の見積りと突き合わせた。

| 項目 | 見積り（開発プラン §5.2） | 実測 | 毎分収集での 1 日 |
|---|---|---|---|
| 生 gzip 1 個（HELLO） | 約 100 KB | **108 KB**（中央値） | 288 個 ＝ 31 MB |
| 生 gzip 1 個（ドコモ） | 約 35 KB | **36 KB**（中央値） | 1,080 個 ＝ 39 MB |
| 生データ 合計 | 約 67 MB/日 | — | **約 70 MB/日** |
| 配列 4 本（HELLO、TOAST 圧縮後） | 40〜60 KB/行 | **22.5 KB/行** | 288 行 ＝ 6.5 MB |
| 配列 4 本（ドコモ、TOAST 圧縮後） | 15〜20 KB/行 | **9.9 KB/行** | 1,080 行 ＝ 10.7 MB |
| 配列 合計 | 30〜40 MB/日 | — | **約 17 MB/日** |

生データは見積りどおり。**配列は見積りの半分以下**で、`smallint[]` に対する pglz が
想定より効いている。1 日あたりの行数はポーリング間隔ではなく**フィードの更新周期**で
決まるため（同じ `last_updated` は重複として弾く）、毎分化で増えるのはドコモの分だけ
（288 → 1,080 行/日）。インデックスと行ヘッダを含む実際の増分は段 7 の 24 時間観測で
測り、この表に追記する。行数の少ない現時点では固定的な確保分が支配的で当てにならない。

**Active CPU の実測（2026-09-06 20:50 JST、M0 直後の 1 時間）**：合計 8 秒 / 呼び出し 94 回 ＝
**1 回あたり約 85 ms**。合格ラインの 0.3 秒に対して 4 分の 1 以下で、月換算 1.6 CPU 時間。
グラフは 0 に近い谷と 400〜650 ms の山を繰り返す。HELLO が 5 分に 1 回だけ全件を解析し、
残り 4 回は 304 でほとんど何もしないため。**条件付き取得は転送量だけでなく CPU にも効く**。
この窓は `*/5` 期を含むので、毎分の定常値はこれよりさらに低い。
...
配列の並び（`idx`）と生 JSON の値が一致していることを、実データで確認する。`scripts/reconcile-snapshot.mjs`（PR D で追加）が、指定した `observed_at` の生 gzip JSON を Storage から取得し、`stations` の `idx` を使って DB の配列と全ポートを突き合わせる。
...
| 列 | 型 | 値 | 欠損時 |
|---|---|---|---|
| `bikes` | `smallint[]` | `num_bikes_available`。**負値は 0 に丸める** | `-1` |
| `docks` | `smallint[]` | `num_docks_available`。**負値は 0 に丸める** | `-1` |
...
**`-1` は欠損（そのポートが登録済みなのに今回のフィードに現れなかった）専用**である。フラグは 0〜7、`reported_age_s` と台数・返却枠は負値を 0 に丸めるため、`-1` が本物の値と衝突することはない。

**台数・返却枠の負値を丸める理由**（v1.2 で追記。§5.1 の 32）：GBFS の仕様では `num_bikes_available` と `num_docks_available` は非負だが、**HELLO は定員超過のポートで `num_docks_available` に `-1` を返す**（2026-09-06 の実測で 17 件）。容量 3 のポートに 4 台停まっていれば `capacity − bikes = -1` になる、という素直な帰結で、開発プラン §3.5 が観測していた「定員超過駐輪」と同じ現象である。そのまま保存すると**「返却枠が -1」と「そもそも観測していない」が区別できなくなる**。0 に丸めるのは意味の上でも正しく、定員を超えて停まっているポートに返す枠は無い。何台超過しているかは W3 の特徴量で `capacity − bikes` から導ける。丸めた件数は `feed_fetch_log.warnings.clamped_negative_counts` に残す。
...
7. `station_status_latest` を更新する。**`p_observed_at` が `feed_state.last_observed_at` 以下なら、この更新を丸ごと行わない**（後退したスナップショットで最新値を汚さない）。行ごとの `last_changed_at` と比べるだけでは足りないことに注意（§5.2 の 34）。`(bikes, docks, flags, is_present)` のいずれかが変わった行だけ書き、`last_changed_at = p_observed_at` にする。今回現れなかったポートは `is_present = false`（異常ガード時は据え置き）
...
`warnings` には正規化で気づいたことを入れる（`gbfs-core` が返す）：`exact_duplicates` / `conflicting_duplicates` / `missing_last_reported` / `clamped_reported_age` / `clamped_negative_counts`。すべて 0 なら何も無かったということなので、そのまま記録してよい。
