...
**重み付きを主、重み無しを従とする**（開発プラン §6.2）。重みは逆抽出確率で、
掛ければ母集団の平均になる。**2026-09-16 からは一様に引いている**ので、その日だけを
読めば 2 つは一致する——**食い違うのは 09-15 以前の日が混ざっているとき**で、
そこは難所を 4 倍濃く取ってある（W5 プラン §6.9 の PR I）。両方を並べて出し、
**一致するはずの窓で食い違ったら抽出か重みを疑う。**
