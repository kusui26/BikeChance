...
def _weighting_block(outcome: Outcome) -> list[str]:
    ...
    return [
        "## 10. 重み付きと重み無し",
        "",
        "**重み付きが主**（開発プラン §6.2）。**2026-09-16 から一様に引いている**ので、"
        "その日だけを読めば 2 つは**完全に一致する**（重みが定数で約分される）。"
        "**食い違うなら窓に 09-15 以前の日が混ざっている**——そこは難所"
        "（`bikes <= 2` または `docks <= 2`）を 4 倍濃く取ってあり、"
        "**重み無しの Brier は難所に引かれて大きく出る**（W5 プラン §6.9 の PR I）。"
        "一致するはずの窓で食い違ったら、抽出か重みを疑う。",
        "",
