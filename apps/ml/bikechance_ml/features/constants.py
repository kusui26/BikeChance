...
#: v4（2026-09-24）：**ポートプロファイルの 7 列を足した**（W5 プラン §6.10 の PR J1、
#: 開発プラン §6.3・D-27）。`prof_p_bike` / `prof_p_dock` / `prof_mean_bikes` /
#: `prof_std_bikes` / `prof_rentals_per_hour` / `prof_returns_per_hour` / `prof_n_days`。
#: 引くのは**前日の版**（`profile.source_day`）の、**目標時刻のセル**である。**推論は J2 まで
#: プロファイルを読まない**ので、配信の表では 6 列が NULL・`prof_n_days` が 0 になる
#: ——**`prof_*` を使うモデルは J2 まで配らない**（W5 プラン §9 の契約 29）。
#: **v3 と混ぜて学習しない。** 2026-09-07 以降の日はすべて v4 で作り直す（同 §6.10）。
#:
#: **版を上げるのは列が変わったときだけ。** 抽出は列を変えないので、2026-09-16 に
#: 層化をやめたときも **v3 のまま**にした（W5 プラン §6.9 の PR I）。推論は全件を出す
#: ——**抽出は学習の都合でしかなく、学習と推論の食い違いを生まない。**
FEATURE_SET: Final[str] = "v4"
...
#:
#: **列は残す。** 2026-09-15 までの `features/` は層化（4% と 1%）で作ってあった。PR J1 の
#: 作り直しで全日が一様になり、重みは定数になった——**それでも落とさない**（W5 プラン
#: §6.10 の J1 の実装）。評価と当てはめは重み付きで書いてあり（定数なら重み無しと一致する）、
#: **抽出率を上げる・層化に戻すのは定数 1 行**（D-25）なので、そのとき列が無いと**版を
#: もう 1 つ上げる**ことになる。定数の列は Parquet の辞書符号化でほぼ場所を取らない。
